...
def _build_load_groups(
    ...
    for source_key in checkpoint_index.keys():
        ...
        for converter in converters:
            _, matched_pattern = converter.rename_source_key(renamed_key)
            if matched_pattern is not None:
    ...
    used_weight_mapping = [
        transform
        for transform in weight_mapping
        if id(transform) in used_transform_ids
    ]
    return tuple(groups.values()), tuple(unexpected_keys), used_weight_mapping
...
class CheckpointManager:
    ...
    def load_checkpoint(
        ...
        *,
        ...
        self.model._weight_conversions = weight_mapping  # pylint: disable=W0212
        report = LoadReport(
