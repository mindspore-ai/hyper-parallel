...
from hyper_parallel import DTensor, HSDPModule
...
def _initialize_materialized_fsdp_state(model: nn.Module) -> None:
    """Refresh FSDP parameters after to_empty replaces module tensors."""
    for module in model.modules():
        if not isinstance(module, HSDPModule):
            continue
        hsdp_state = module.hsdp_scheduler.hsdp_state
        if hsdp_state is not None:
            hsdp_state.lazy_init()


def _move_model_buffers_to_device(model: nn.Module, device) -> None:
    """Keep runtime buffers on the execution device when parameters are offloaded."""
    for module in model.modules():
        for buffer_name, buffer in module._buffers.items():  # pylint: disable=W0212
            if buffer is not None:
                module._buffers[buffer_name] = buffer.to(device)  # pylint: disable=W0212
...
def _tie_model_weights_before_sharding(model: nn.Module) -> None:
    """Materialize declared HF aliases before FSDP captures parameter ownership."""
    tied_mapping = getattr(model, "all_tied_weights_keys", {}) or {}
    if not tied_mapping:
        return
    ...
    broken_aliases = [
        ...
        and source_name in parameters
        and parameters[target_name] is not parameters[source_name]
    ]
    if broken_aliases:
        raise ValueError(
            "Model tie_weights() did not create shared parameter identities before "
            f"distributed wrapping: {broken_aliases}"
        )
...
def apply_model_infrastructure(
    ...
    **kwargs,
    ...
    strategy_config = getattr(distributed_setup, "strategy_config", None)
    if getattr(strategy_config, "offload_policy", None) is not None:
        materialization_device = torch.device("cpu")
    model = _move_model_to_device(model, is_meta_device, materialization_device)
    if is_meta_device and fsdp2_manager is not None:
        _initialize_materialized_fsdp_state(model)
    if materialization_device != device:
        _move_model_buffers_to_device(model, device)
    if is_meta_device:
        if load_base_model:
