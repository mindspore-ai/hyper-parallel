...
from .ops.parallel_ops_register import get_distributed_op



def get_op_name(func, enable_ms=False) -> str:
    """
    从 __torch_function__ 的 func 参数中获取算子名（兼容所有PyTorch版本）
    """
    if enable_ms:
        return func.name
...
class OpDispatcher:
    ...
    def __init__(self):
        ...
        for op_name, config in self.layout_infer_ops.items():
            ...
            module_name = "dist_parallel.spmd.ops." + config['distributed_op_file']
    ...
    def _with_layout_infer(self, func: callable, *args, **kwargs) -> Tensor:
            ...
            if isinstance(output_layout, (tuple, list)):
                if len(py_output) == len(output_layout):
                    for i, output_item in enumerate(py_output):
                        output += (dist_parallel.DTensor.from_local(output_item, output_layout[i]),)
                else:
        ...
        return dist_parallel.DTensor.from_local(py_output, output_layout)
    ...
    def _with_layout_infer_with_tuple_expand(self, func: callable, *args, **kwargs) -> Tensor:
        ...
        for arg in args:
            if isinstance(arg, (tuple, list)):
                ...
                input_args.append(arg.to_local() if isinstance(arg, dist_parallel.DTensor) else arg)
    ...
    def _with_layout_infer_reshape(self, func: callable, *args, **kwargs) -> Tensor:
        ...
        return dist_parallel.DTensor.from_local(py_output, infer_output_tuple[0])
    ...
    def dispatch(self, op_call: callable, args: tuple[object, ...], kwargs: dict[str, object]):
        ...
        if op_name in self.whitelist:
            input_args = [arg.to_local() if isinstance(arg, dist_parallel.DTensor) else arg for arg in args]
