...
Ground every answer in the current code under `hyper_parallel/core/expert_parallel/` and `hyper_parallel/components/modules/moe.py`. When reasoning about API shape or semantics, you may align mentally with widely used MoE + expert-parallelism designs, but **do not name external frameworks** in user-facing explanations unless the user explicitly asks.

## Expertise Areas

### `components/modules/moe.py` — MoE building blocks

This is the canonical implementation location.
...
- **`GroupedExperts`** — weight storage `[num_experts, out_dim, in_dim]` for `w1`/`w3`, `[num_experts, in_dim, hidden_dim]` for `w2`. Three forward paths:
  - `_run_experts_for_loop` — plain for-loop over experts; always correct, the fallback.
...
- **`ExpertParallel`** (`BaseExpertParallel`) — standard all-to-all EP:
  ...
  - `_token_dispatch` (forward pre-hook): exchanges token counts via the eager `all_to_all_single`, then dispatches tokens via `differentiable_all_to_all_single`; applies `_permute` to convert rank-major → expert-major.
  - `_token_combine` (forward post-hook): `_unpermute` (expert-major → rank-major) then reverse `differentiable_all_to_all_single`.
...
### Differentiable dispatch — `differentiable_all_to_all_single`

- Lives in `hyper_parallel/core/dtensor/_utils.py` as an autograd function over `dist.all_to_all_single`.
...
- Exposed as `differentiable_all_to_all_single(input, input_splits, output_splits, group)`. The truly-async variant `differentiable_all_to_all_single_async` returns an `AsyncCollectiveTensor` whose `wait_tensor` is queued lazily in both directions.

### `core/expert_parallel/__init__.py` — public exports

Exports: `ExpertParallel`, `ExpertTensorParallel`, `TensorParallel`, `BaseExpertParallel`. The MoE building blocks live in `hyper_parallel/components/modules/moe.py`.
...
- **Differentiable collectives** — all dispatch/combine traffic goes through `differentiable_all_to_all_single`; never call `dist.all_to_all_single` directly from EP code.
...
- **Lazy NPU imports** — `torch_npu` stays imported inside methods (with `# pylint: disable=C0415`) so importing the library does not require the NPU stack.
