...
- Run commands from the repository root. `hyper_parallel/rl/` is the source root imported as `rl.*`.
  Follow the [runtime installation guide](../../hyper_parallel/rl/docs/hyper_rl_runtime_image.md) for dependencies
...
- RL and `hyper_parallel/models/qwen3/` use native Torch APIs. Do not introduce Platform dispatch or `get_platform()`.
  Qwen3 model loading and parallel adaptation belong in `models/qwen3/`; RL value heads and Critic behavior belong in
  `hyper_parallel/rl/rl/roles/policy/critic.py`.
- The built-in algorithms are GRPO and PPO. Model registration accepts Qwen3 dense; training supports FSDP sharding
  ...
  Check `rl/config.py` and `rl/roles/model.py` for the executable boundary; do not infer RL support from a main-project API.
