...
def _infer_skill_domain(files: List[str],
    ...
    feature_domain = f"新增 Skill: `{skill_name}`"
...
def _classify_file_bucket(filepath: str) -> str:
    ...
    ``.agent/skills/*/scripts/*.py`` are classified as production because
    they are executable scripts, not documentation.
    """
    if filepath.startswith(_TEST_PREFIXES):
        return "tests"
    if filepath.startswith(".agent/skills/") and filepath.endswith(".py"):
        return "production"
...
def _build_skill_section(skill_files: List[str]) -> List[str]:
    ...
    desc_parts.append("遵循 Skill 标准目录结构：")
    if has_skill_md:
...
def generate_pr_content(diff: str, commits: List[str],
    ...
    Autogit's contract with callers in an AI agent is: Python supplies
    mechanical facts, the AI supplies the narrative. This function
    therefore does NOT try to synthesize a
...
def prepare_pr_analysis(diff: str, commits: List[str]) -> str:
    ...
    the AI agent can generate a human-friendly PR description from it.
