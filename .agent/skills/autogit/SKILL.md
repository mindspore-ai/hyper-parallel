...
To also reject AI-assistant attribution in **every** `git commit` (manual or IDE commits, not just autogit), install the commit-msg hook once per clone:
...
- **non-tty (AI agent)** — one `AutoGitError` listing every undecided gate with the exact flag the AI must pass. AI asks the user one-at-a-time in chat, then re-invokes with all flags in a single call.
...
DO NOT add AI-assistant attribution trailers (e.g. "Co-Authored-By: ...") in commit messages.
If `.agent/rules/code-style.md` conflicts with a user request, explain the conflict and provide a compliant alternative instead of emitting non-compliant code.
