...
Subcommands:
    ...
    autofix <pr>        Drive the diagnose -> propose-plan workflow. The
                        ...
                        performed by the caller (Claude), since they require
...
def cmd_diagnose(args) -> int:
    ...
    if build and not build.get("building"):
        ...
        # lacks the pr-check-pass label. Always attach the comment-side
        # signals.
        ...
        has_both_labels = (
            "pr-check-pass" in (report.get("labels") or [])
            and "ci-pipeline-passed" in (report.get("labels") or [])
        )
    ...
    # Source of truth #2: only PR comments (no build registered yet, or build still running).
    comments = get_pr_comments(owner, repo, pr, _token())
    summary = parse_latest_failure(comments)
