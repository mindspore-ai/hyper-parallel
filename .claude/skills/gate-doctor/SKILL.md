...
repositories. The skill teaches Claude how to:
...
- A working `autogit` skill in the same `.claude/skills/` tree — `gate-doctor`
...
claude: Running gate-doctor `diagnose` …
...
```
user: 触发一下 #647 的门禁
claude: Running gate-doctor `retest` …
...
```
user: 修一下 #647 的流水线
claude: Running gate-doctor autofix …
...
- No `Co-Authored-By: Claude …` in commit messages. **Why:** the
...
- If `.claude/rules/code-style.md` exists in the active repo, load
