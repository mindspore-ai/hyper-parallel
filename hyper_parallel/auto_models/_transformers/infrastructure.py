...
from hyper_parallel.auto_models.components.compile import apply_compile
from hyper_parallel.auto_models.components.distributed.fsdp2 import FSDP2Manager, _instantiate_fsdp2
from hyper_parallel.auto_models.components.distributed.infrastructure import DistributedSetup, MeshContext
from hyper_parallel.auto_models.components.distributed.pipelining import AutoPipeline, _instantiate_pipeline
...
def _move_model_to_device(
    ...
    sharding_specs = {
        name: getattr(parameter, "_sharding_spec", None)
        for name, parameter in model.named_parameters(remove_duplicate=False)
    }
    model.to_empty(device=device)
    for name, parameter in model.named_parameters(remove_duplicate=False):
        sharding_spec = sharding_specs.get(name)
        if sharding_spec is not None:
            parameter._sharding_spec = sharding_spec  # pylint: disable=protected-access
    targets = _build_finalize_targets(model)
    for target_name, source_name in (
        getattr(model, "all_tied_weights_keys", {}) or {}
    ).items():
        target = targets.get(target_name)
        source = targets.get(source_name)
        if target is not None and source is not None:
            if target.is_parameter and source.is_parameter:
                target.module.register_parameter(target.tensor_name, source.tensor)
            else:
                target.tensor.data = source.tensor.data
    return model
...
def _apply_module_replacement_actions(
    ...
    from hyper_parallel.auto_models.components.model_transform import (  # pylint: disable=import-outside-toplevel
