...
from hyper_parallel.auto_models.components.loss.loss_parallel_ops_common import (
    _is_dtensor,
...
def distributed_cross_entropy(
    ...
    if _is_dtensor(input_tensor):
        ...
        mesh, mesh_dim = _get_mesh_and_dim(input_tensor)
    ...
    if mesh is None or mesh_dim is None:
        raise ValueError(
            "Unable to identify the mesh dimension that shards the class dimension. "
            f"Got placements: {input_dtensor.placements}"
        )
