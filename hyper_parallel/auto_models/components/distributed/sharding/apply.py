...
def _local_params_context(model: nn.Module):
    ...
    local_parameters = {}
    for name, param in list(model.named_parameters(remove_duplicate=False)):
        if isinstance(param, DTensor):
            source_shard_records[name] = param.placements
            local_parameter = local_parameters.get(id(param))
            if local_parameter is None:
                local_parameter = nn.Parameter(
                    param.to_local(),
                    requires_grad=param.requires_grad,
                )
                # FSDP consumes source_shard_info directly. Pure TP has no later
                # wrapper, so retain the same layout for checkpoint loading,
                # gradient clipping, and full-state reconstruction.
                local_parameter._sharding_spec = param.layout  # pylint: disable=protected-access
                local_parameters[id(param)] = local_parameter
            _set_param_by_path(model, name, local_parameter)
