...
def _broadcast_tied_param(model, tied_pair):
    """Make both ends of one tied-weight pair reference the same parameter.
    ...
    if param_a is None or param_b is None:
        return
    if tuple(param_a.shape) != tuple(param_b.shape):
        raise ValueError(
            f"Tied parameters must have matching shapes: {fqn_a}={tuple(param_a.shape)} "
            f"vs {fqn_b}={tuple(param_b.shape)}"
        )
    # FSDP discovers shared parameters by object identity. Sharing only the
    # current local storage lets the two parameters acquire independent FSDP
    # buffers after the first unshard and causes their optimizer updates to
    # diverge.
    _set_param_by_path(model, fqn_b, param_a)
