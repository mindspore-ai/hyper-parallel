...
def _kimi_k26_chunk_loss_forward(
    model: nn.Module,
    *,
    ...
    **kwargs: Any,
    ...
    # The LM head is not vocabulary sharded: ChunkedCausalLMLoss gates
    # tp_size != 1 out at bind time, so no tensor-parallel mesh is threaded
    # through here.
...
def bind_chunk_loss(model: nn.Module) -> None:
    ...
    if getattr(model, "_hp_kimi_k26_chunk_loss_bound", False):
        return
    model_type = getattr(getattr(model, "config", None), "model_type", None)
    if model_type not in KIMI_MODEL_TYPES:
        raise TypeError(
            "Kimi-K2.6 Chunk Loss adapter requires config.model_type in "
    ...
    def _chunk_loss_aware_forward(*args: Any, **kwargs: Any) -> Any:
        ...
        return _kimi_k26_chunk_loss_forward(model, **kwargs)

    _commit_forward_rewrite(
        _ForwardRewriteRequest(
            model,
            _chunk_loss_aware_forward,
            companion_attrs={"_hp_kimi_k26_chunk_loss_bound": True},
        )
    )
