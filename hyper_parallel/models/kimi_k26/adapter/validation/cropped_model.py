...
def _has_native_module(module_name: str) -> bool:
    """Return whether the installed transformers ships ``module_name``."""
    try:
...
def _require_native_kimi_support() -> None:
    """Fail fast when the installed transformers lacks the native Kimi-K2.6 model.

    The family builds on transformers' native ``kimi_k25`` (and, for the nested
    text tower, ``deepseek_v3``) instead of the checkpoint's remote code, so an
    environment without those modules cannot run this builder. Raising here keeps
    the failure specific rather than surfacing as an unrecognised-architecture
    error from ``AutoConfig``.

    Raises:
        ImportError: If transformers provides no native ``kimi_k25`` or
            ``deepseek_v3`` implementation.
    """
    from transformers import __version__ as transformers_version  # pylint: disable=import-outside-toplevel

    # Probing with find_spec keeps the check import-free; the model classes
    # themselves are resolved by transformers from the configuration class.
    missing = [
        name for name in (
            "transformers.models.kimi_k25.modeling_kimi_k25",
            "transformers.models.deepseek_v3.modeling_deepseek_v3",
        )
        if not _has_native_module(name)
    ]
    if missing:
        raise ImportError(
            "Kimi-K2.6 requires a transformers release that ships the native "
            "`kimi_k25` and `deepseek_v3` models (verified with 5.13.0); the "
            f"installed version is {transformers_version} and {missing} is missing. "
            "Upgrade transformers, or point model._target_ at an implementation "
            "that carries the model code itself."
        )
...
def build_cropped_kimi_vlm(
    ...
    # ``text_config.architectures`` (``DeepseekV3ForCausalLM``) is the checkpoint's own
    # declaration and is left untouched: the nested text class is resolved from the
    # configuration class, so this builder never rewrites the model structure.
