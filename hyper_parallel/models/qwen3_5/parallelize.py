...
def _apply_linear_attention_cp(
    ...
    """Apply the selected linear-attention CP executor."""
...
def parallelize_qwen3_5_cp(
    ...
    *,
    ...
    Linear-attention (:class:`Qwen3_5GatedDeltaNet`) layers use either a
    sequence-to-head Ulysses layout or the optimized recurrent-state P2P path.
    """
    ...
    logger.info_rank0(
        ...
        "linear-attn mode=%s hooks=%d",
        cp_mesh.size(), ulysses_degree, full_attached,
        linear_attention_cp_mode, linear_attached,
    )
    return model
...
def parallelize_qwen3_5(
    ...
    if tp_size > 1 and cp_size > 1 and _has_linear_attention_layers(model):
        raise NotImplementedError(
            "Qwen3.5 TP+CP for linear-attention layers is not supported yet. "
            "Set parallel.tp=1 when parallel.cp>1."
        )
    if cp_size > 1:
        try:
            cp_mesh = mesh["cp"]
