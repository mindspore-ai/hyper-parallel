...
# ============================================================================
"""Run native-module parity for V4.1 Engram, MoE, and CSA2.
...
def _run_engram(
    ...
    hp = DeepseekV41Engram(hp_config, 0, assets)
    ...
    native_input = source.to(dtype).detach().requires_grad_(True)
...
def _run_attention_topology(
    ...
    hp_layers = [
        DeepseekV41Attention(hp_config, layer)
    ...
    native_inputs = [source.to(dtype).detach().requires_grad_(True) for source in sources]
    hp_inputs = [
        source.to(device=target_device, dtype=dtype).detach().requires_grad_(True)
