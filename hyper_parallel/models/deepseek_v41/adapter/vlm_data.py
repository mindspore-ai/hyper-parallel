...
# ============================================================================
"""DeepSeek-V4.1 native image-text transformation and VLM batch policy."""
...
class DeepseekV41VLMBatchAdapter(DeepseekV41BatchAdapter):
    """Apply DeepSeek-V4.1 modality collation and CSA2 runtime semantics."""

    def batch_fields(self) -> tuple[str, ...]:
        """Declare V4.1 fields produced by transform/collation."""
        return (
            "attention_mask",
            "token_types",
            "pixel_values",
    ...
    def collate_items(
        ...
        """Merge fixed text fields and variable-size DeepSeek image fields.

        Args:
            items: Prepared VLM samples selected for one micro-batch.
            context: Generic batch construction facts.

        Returns:
            Collated text, image patch, grid, offset, and ownership fields.
        """
        del context
        self._validate_item_fields(items)
        fields = items[0].keys()
        batch = {}
        for field in fields:
            if field == "image_patch_offsets":
                batch[field] = self._merge_image_patch_offsets(items)
                continue
            values = [item[field] for item in items]
            if field in _CONCAT_MODAL_FIELDS:
                self._validate_tensor_values(field, values)
                batch[field] = torch.cat(values, dim=0)
            else:
                batch[field] = default_collate(values)
        batch["image_batch_indices"] = self._build_image_batch_indices(items)
        return batch

    @staticmethod
    def _validate_item_fields(items: Sequence[Mapping[str, Any]]) -> None:
        """Require every item to expose the same DeepSeek VLM fields."""
        expected_fields = set(items[0])
        for item_index, item in enumerate(items[1:], start=1):
            item_fields = set(item)
            if item_fields != expected_fields:
                raise ValueError(
                    "DeepSeek-V4.1 VLM items must expose identical fields; "
                    f"item 0 has {sorted(expected_fields)}, item {item_index} has {sorted(item_fields)}"
                )
        required_fields = {
            "input_ids",
            "labels",
        ...
        missing_fields = required_fields.difference(expected_fields)
        if missing_fields:
            raise ValueError(
                "DeepSeek-V4.1 VLM items are missing required fields: "
                f"{sorted(missing_fields)}"
            )

    @staticmethod
    def _validate_tensor_values(field: str, values: Sequence[Any]) -> None:
        """Reject non-tensor values for concatenated modality fields."""
        if any(not isinstance(value, torch.Tensor) for value in values):
            raise TypeError(f"DeepSeek-V4.1 VLM field {field!r} must contain tensors")

    @staticmethod
    def _merge_image_patch_offsets(items: Sequence[Mapping[str, Any]]) -> torch.Tensor:
        """Merge per-item patch offsets into one batch-global offset vector."""
        merged_offsets = []
        patch_total = 0
        for item in items:
            offsets = item["image_patch_offsets"]
            if not isinstance(offsets, torch.Tensor) or offsets.ndim != 1 or offsets.numel() == 0:
                raise ValueError(
                    "DeepSeek-V4.1 image_patch_offsets must be a non-empty one-dimensional tensor"
                )
            if int(offsets[0]) != 0 or torch.any(offsets[1:] < offsets[:-1]):
                raise ValueError(
                    "DeepSeek-V4.1 image_patch_offsets must start at zero and be non-decreasing"
                )
    ...
    @staticmethod
    def _build_image_batch_indices(items: Sequence[Mapping[str, Any]]) -> torch.Tensor:
        """Record which VLM item owns each concatenated image metadata row."""
        image_batch_indices = []
        for batch_index, item in enumerate(items):
            grids = item["image_vit_grid_hw"]
            if not isinstance(grids, torch.Tensor) or grids.ndim != 2 or grids.shape[1] != 2:
                raise ValueError(
                    "DeepSeek-V4.1 image_vit_grid_hw must have shape [num_images, 2]"
                )
            image_batch_indices.append(
                torch.full((grids.shape[0],), batch_index, dtype=torch.long, device=grids.device)
            )
