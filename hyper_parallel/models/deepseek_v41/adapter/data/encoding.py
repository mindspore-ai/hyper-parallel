...
def _tool_name_for_encoding(tool: Dict[str, Any]) -> str:
    ...
    qualified_name = name if namespace is None else f"{namespace}::{name}"
...
def decode_dsml_to_arguments(tool_name: str, tool_args: Dict[str, Tuple[str, str]]) -> Dict[str, str]:
    ...
    def _decode_value(key: str, value: str, string: str):
        if string == "true":
            value = to_json(value)
        encoded_value = f"{to_json(key)}: {value}"
...
def _is_image_block(block: Dict[str, Any]) -> bool:
    ...
    is_image = isinstance(block, dict) and block.get("type") in ("image", "image_url")
    return is_image
...
def _read_until_stop(index: int, text: str, stop: List[str]) -> Tuple[int, str, Optional[str]]:
    ...
    if matched_stop:
        ...
        next_index = min_pos + len(matched_stop)
        parse_result = (next_index, content, matched_stop)
    else:
        content = text[index:]
        parse_result = (len(text), content, None)
    return parse_result
...
def render_tools(tools: List[Dict[str, Union[str, Dict[str, Any]]]]) -> str:
    ...
    rendered_tools = TOOLS_TEMPLATE.format(
        tool_schemas="\n".join(tools_json),
...
def encode_messages(
    ...
    if return_multi_modal_data:
        ...
        encoded_messages = (prompt, media_data)
