...
"""VeOmni AscendC head-first KKT helper, retaining upstream kernel arithmetic.

Source: ByteDance-Seed/VeOmni, _ascend/triton_core/chunk_scaled_dot_kkt.py.
The sequence-first helper used by other Hyper backends is intentionally unchanged.
"""
        ...
        for i_h in range(H):
            ...
            for i_k in range(tl.cdiv(K, BK)):
                p_k = tl.make_block_ptr(
                    k + k_batch_off + i_h * T_max * K + bos * K,
                    (T_local, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                )
            ...
            p_A = tl.make_block_ptr(
                A + A_batch_off + (bos * H + i_h) * BT,
                (T_local, BT), (BT * H, 1), (i_t * BT, 0), (BT, BT), (1, 0)
            )
