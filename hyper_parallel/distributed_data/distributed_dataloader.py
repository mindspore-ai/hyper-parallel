...
# ============================================================================
"""Synchronous Source Loader, Planner, and Data Constructor orchestration."""
...
def _scaled_buffer_target(
    ...
    denominator = multiplier_denominator * source_count
...
class DistributedDataLoader(Iterator[Any]):
    ...
    One call to :func:`next` is a synchronous transaction: Source Loaders fill
    ...
    def __init__(
        ...
        self._source_loader_ranks = source_loader_ranks
    ...
    def __next__(self) -> Any:
        ...
        # Source buffers are committed only after construction and delivery
        # have both succeeded, leaving checkpoint boundaries unambiguous.
        if self._source_loader is not None:
            self._source_loader.commit(self._pending_local_keys)
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        state = {
            ...
            "source_loader": self._source_loader.state_dict() if self._source_loader is not None else None,
        }
        try:
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any]) -> None:
        ...
        source_state = state.get("source_loader")
        if (self._source_loader is None) != (source_state is None):
            raise ValueError("Distributed DataLoader checkpoint Source Loader ownership changed.")
        if self._source_loader is not None:
            self._source_loader.load_state_dict(source_state)
        self._step = step
    ...
    def set_epoch(self, epoch: int) -> None:
        ...
        if self._source_loader is not None:
            self._source_loader.set_epoch(epoch)
        self._step = 0
        self._stopped = False
    ...
    def _produce_on_data_plane(self) -> ConstructedBatch | None:
        ...
        local_selected_keys = {
            key for key in selected_keys if key.source_rank == self._topology.global_rank
        }
    ...
    def _next_plan_control(self) -> _PlanControl:
        attempt = 1
        while True:
            local_snapshot = self._fill_local_source(attempt)
            ...
            if self._topology.global_rank == self._data_plane.planner_rank:
                if snapshots is None:
                    planner_control = _PlanControl("error", error="Planner did not receive Source snapshots.")
                else:
    ...
    def _fill_local_source(self, attempt: int) -> _SourceSnapshot:
        is_source = self._topology.global_rank in self._source_loader_ranks
        if not is_source:
        ...
        if self._source_loader is None:
            return _SourceSnapshot(
                ...
                can_read_more=False,
                metadata=(),
                error=f"Source rank {self._topology.global_rank} did not provide a Dataset.",
            )

        source_count = len(self._source_loader_ranks)
        ...
        error = self._source_loader.fill(
        ...
        return _SourceSnapshot(
            ...
            stopped=self._stopped,
            is_source=True,
            exhausted=self._source_loader.exhausted,
            can_read_more=(
                not self._source_loader.exhausted
                and self._source_loader.buffer_size < self._max_buffered_samples
            ),
            metadata=self._source_loader.metadata(),
            error=error,
        )
    ...
    def _build_plan_control(self, snapshots: tuple[Any, ...]) -> _PlanControl:
        ...
        for snapshot in snapshots:
            if not isinstance(snapshot, _SourceSnapshot):
                return _PlanControl("error", error="A data-plane rank contributed an invalid Source snapshot.")
            expected_source = snapshot.rank in self._source_loader_ranks
            if snapshot.is_source != expected_source:
                return _PlanControl(
                    "error",
                    error=f"Rank {snapshot.rank} reported inconsistent Source Loader ownership.",
                )
        ...
        if len(contributed_ranks) != len(set(contributed_ranks)) or set(contributed_ranks) != set(
            ...
            return _PlanControl(
                "error",
                error=f"Data-plane Source snapshots have invalid rank coverage {contributed_ranks}.",
            )
        ...
        source_snapshots = [snapshot for snapshot in normalized if snapshot.is_source]
        candidates = tuple(metadata for snapshot in source_snapshots for metadata in snapshot.metadata)
        try:
        ...
        if all(snapshot.exhausted for snapshot in source_snapshots):
            return _PlanControl("stop")
        if any(snapshot.can_read_more for snapshot in source_snapshots):
            return _PlanControl("need_more")
        return _PlanControl(
            "error",
            error=(
                f"Source buffers reached max_buffered_samples={self._max_buffered_samples} before "
    ...
    def _prepare_outgoing(
        ...
        try:
            if local_selected_keys:
                if self._source_loader is None:
                    raise ValueError("A planned Source rank has no Source Loader.")
                payloads = self._source_loader.selected_payloads(local_selected_keys)
                target_by_key = {
