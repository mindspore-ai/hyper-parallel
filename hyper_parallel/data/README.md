...
`hyper_parallel.data` 负责把原始数据或 Indexed Dataset 转换为模型一次
...
dataloader:
  ...
  collate_fn:
    _target_: hyper_parallel.data.batching.build_indexed_collate_fn
  get_batch:
    _target_: hyper_parallel.data.batching.ParallelBatch
    source_type: indexed
```
...
```text
data/
├── __init__.py                         # data 包入口（不扁平导出 VLM）
├── README.md                           # 本文档
├── constants.py                        # IGNORE_INDEX 等共享常量
...
│   └── build_barrier.py                # Online 构建同步 barrier
├── text/
│   ├── build_tokenizer.py              # tokenizer adapter
│   ├── chat_template.py                # conversation template（双实现合并）
│   ├── build_data_transform.py         # plaintext/conversation/identity transform
...
│   ├── io.py                           # Indexed writer
│   └── csrc/indexed_helpers.cpp        # sample/blend index 实现
├── vlm/
│   ├── build_data_transform.py         # VLM chat transform
│   ├── build_processor.py              # VLM processor 构建
│   ├── collator.py                     # VLM collator
│   ├── dataset.py                      # VLM 源 Dataset
│   ├── get_batch.py                    # VLM batch 到模型输入
│   └── constants.py                    # 跨模态 token 索引常量
└── tools/
    ├── offline_preparation.py          # 本地 JSON/JSONL 到 Indexed Dataset
    ├── offline_config.py               # 离线预处理配置
    ├── huggingface_offline.py          # Hugging Face 下载后转换
    └── read_indexed_dataset.py         # 读取检查工具
```

离线预处理用户指南见 `docs/guide/data/offline_preparation_guide.md`。
