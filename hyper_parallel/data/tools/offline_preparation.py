...
def _add_offline_source_arguments(group: Any) -> None:
    ...
    arguments = (
        (("--dataset-name-or-path",), {
            "required": True,
            "help": "Input JSON/JSONL/JSON.GZ file, directory, or glob.",
        }),
...
def _resolve_input_files(dataset_name_or_path: str) -> list[str]:
    ...
    if not input_files:
        raise ValueError(
            "dataset_name_or_path must be a JSON/JSONL/JSON.GZ file, a directory containing "
            f"supported JSON files, or a matching glob; received {dataset_name_or_path!r}"
        )
    return input_files
