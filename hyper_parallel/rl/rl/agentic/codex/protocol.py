...
def _reasoning_text(item: dict[str, Any]) -> str:
    """Extract textual reasoning from response content or summary blocks."""
...
def _shell_arguments(action: Any) -> str:
    """Convert a validated local-shell action to backend tool arguments."""
...
def _shell_action(arguments: Any) -> dict[str, Any]:
    """Convert backend shell arguments to a validated local-shell action."""
    try:
...
class CodexResponsesProtocol:
    ...
    def _input_messages(
        ...
        """Translate response input items into backend chat messages."""
        messages: list[dict[str, Any]] = []
    ...
    def _chat_tool_call(
        ...
        """Convert a response tool call to its backend chat representation."""
        call_id = item.get("call_id", item.get("id"))
        if not isinstance(call_id, str) or not call_id:
    ...
    def _tools(
        ...
        """Build backend tool declarations and reversible namespace mappings."""
        if tools is None:
            return [], {}, {}
    ...
    def _tool_choice(
        ...
        """Translate a tool selection using the declared namespace aliases."""
