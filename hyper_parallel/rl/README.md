...
- **明确的运行起点**：提供[固定运行镜像](docker/README.md)、模型配置与启动脚本，通过 Qwen3-4B [单步检查](#快速开始)验证训练链路。
...
底层优化基础还包括 HyperParallel 的[单边通信与多核 MoE 通算重叠](../core/multicore/README.md)，其 HyperParallel-RL 接入与端到端收益待验证。
...
推荐使用**仓库源码 + 固定运行镜像**。当前运行环境为 Linux ARM64 与 Ascend NPU；以下基础示例使用四张空闲 NPU，Agentic 示例使用两张。宿主机需具备 Docker、兼容的 NPU driver，以及模型和数据存储空间。镜像所需空间与驱动要求见[运行镜像](docker/README.md#宿主要求)。
...
开发时直接修改本地源码，后续运行会挂载修改后的版本。镜像下载、digest 与依赖校验见[运行镜像文档](docker/README.md)。
...
**Qwen3-4B + GSM8K，四卡单步同步 GRPO。** 数据目录需包含 `train.parquet`、`test.parquet`，每行使用 `prompt` 和 `extra_info` 列；`extra_info` 提供答案，具体解析规则见[数据读取实现](rl/dataset/data_source.py)。
...
| 扩展范围 | 实现入口 |
| --- | --- |
| 奖励与任务环境 | [GSM8K 示例](examples/gsm8k/agent.py)展示环境与奖励实现，通过 `agentic.module_path` 和 `agentic.environment` 选择任务 |
| 多轮交互或自定义循环 | 使用 [Environment](rl/agentic/envs/environment.py)由框架驱动交互；[AgentProgram](rl/agentic/core/program_runner.py)承载程序化循环，内置路径及新增 harness 的接入边界见 [Agentic RL](docs/agentic_rl.md)；环境观察不参与 policy loss |
...
以下示例复用 GSM8K 多轮环境与计算器，仅替换终局评分。将代码保存到 `hyper_parallel/rl/examples/gsm8k/custom_reward.py`：
...
当前验证平台为**单节点 Ascend 910B3**，环境要求见[运行镜像](docker/README.md)。✅ 已支持 · ◐ 部分验证 · 🧪 仅组件可用 · ○ 规划；状态仅适用于所列组合，不代表长期学习收益已验证。
...
| [公共模块修改说明](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/docs/public_module_changes.md) | RL 目录外修改的必要性和接口影响 |
