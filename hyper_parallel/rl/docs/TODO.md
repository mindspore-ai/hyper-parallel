...
| **Qwen3-30B-A3B** | 数学推理 GRPO。T7 | MoE 真实学习、专家权重发布与 TP/EP 组合 | 当前暂缓；恢复适配后重新验证完整学习、恢复及性能结果。 |
...
接口合同与故障测试可使用可控测试替身，但不能替代真实模型验收。当前只维护 Qwen3-4B，Qwen3-30B-A3B 与 Moonlight 的 RL 适配已移除。下文 MoE 与更大模型任务保留为暂缓的长期规划，不能依赖已删除的代码或测试；重新接入时需要独立验收。
...
- [ ] **T7 一个 MoE 完整验证路径 · M2**
  - **依赖：** T1。
  - **交付：** 恢复 Qwen3-30B-A3B 适配后，选择 1 个主要后端和明确的 TP/EP 组合，完成数学推理 GRPO 的学习、评估、发布与恢复。
  - **验收：** 通过公共学习与效果标准；至少 1 次进程重启恢复并继续 2 个 step，并用设备侧集成测试验证所有目标参数完成加载；按公共性能口径提供时间、成本及吞吐结果；固定策略和 token 上报告训推概率差异、专家负载与路由一致性，无法直接获取路由时明确诊断缺口及替代实验。若超出预设容差，选择校正、路由控制或 replay 并做开关对照，不由版本提交推断概率一致。如重新接入 Moonlight，需独立验证各后端的非零学习，不能由其他后端结果代替。
...
- [ ] **T11 单一多模态训练闭环 · M4**
  ...
  - **验收：** 通过公共学习与效果标准；预处理对齐、媒体/token 位置、截断、混合长度 batch、序列化、checkpoint 恢复共 6 类检查全部通过；设备侧集成测试验证所有可训练模块完成发布，冻结模块保持不变。至少 10 条完整样本记录媒体、动作及奖励对应关系；既有文本 recipe 完成至少 2 个训练 step，新增模态专属 Trainer 数为 0。
...
- [ ] **T12 DeepSeek-V3 多节点异步 · M4**
  ...
  - **验收：** 通过公共学习、效果及性能标准，实测满足资源与吞吐预算；同一模型和硬件下比较同步与异步达到目标质量的时间/成本，另以相同生成预算核验质量容差，均达到预先冻结的门槛，并记录 lag、队列峰值及丢弃率。至少 1 次完整参数设备侧发布校验、1 次进程重启恢复及随后 2 个 step。注入单个 worker 退出和跨节点通信超时共 2 类故障，全部相关进程在约定超时内退出或恢复，无部分提交。新增拓扑各自给出结果；模型无法在小拓扑运行时不宣称线性加速。
