...
仓库中的 `examples/agents/gsm8k/agent.py` 是可直接对照的实现，同时提供单轮问答和组合 calculator 的多轮任务，
两条路径都通过同一个 `ENVIRONMENTS` registry 接入 Trainer。
...
- Agentic UT 入口为 `python tests/ut/rl/agentic/agentic_ut.py`；line 和 branch coverage 门槛均为 80%。
