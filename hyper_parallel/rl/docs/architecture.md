...
2. Actor 和按算法需求选择的冻结 Reference 调用 `models.qwen3.build_causal_lm`。
...
RL 退出通过 `rl/distributed.py::destroy_process_group` 销毁默认进程组，并清理引用该组的
mesh、layout、FSDP、P2P 和 redistribution 缓存。P2P 缓存使用 native-core 所在位置，
退出时使用 Torch 原生进程组销毁与 DeviceMesh 缓存清理入口；销毁失败时仍清理缓存并传播错误。
...
`rl/roles/model.py` 只调用模型及优化器构建入口、冻结 Reference，不实现 Qwen3 的 forward 或并行切分。
...
`models/qwen3/runtime.py` 只在本次 Qwen3 实例上提供别名完整的 planner 视图、FSDP 前的 tied 参数绑定、
...
Qwen3 专属构建实现统一位于 `models/qwen3/runtime.py`。一致性分支使用原有 HF attention recipe，
其 GRPO 验证结果不自动扩展到新增的 PPO 组合。

vLLM 的 `HyperQwen3ForCausalLM` 继续提供 paged attention/KV cache；Native-vLLM 选择保持不变。
模型专属推理适配位于 `rl/roles/rollout/models/<family>/`，公共服务与生成生命周期保留在
...
`examples/scripts/install_runtime.sh` 在容器临时目录构建安装，源码仍从只读挂载的 checkout 导入。
