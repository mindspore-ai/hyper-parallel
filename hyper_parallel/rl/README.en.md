...
- **A clear starting point:** A [pinned runtime image](docker/README.md), model configurations, and launch scripts support a Qwen3-4B [single-step check](#quick-start) of the training pipeline.
...
HyperParallel also provides [one-sided communication and multicore MoE communication–computation overlap](../core/multicore/README.md) as a foundation for further optimization. HyperParallel-RL integration and end-to-end benefits remain to be validated.
...
During development, edit the local source directly; subsequent runs mount the updated checkout. See the [runtime image documentation](docker/README.md) for image download instructions, the image digest, and dependency checks.
...
**Qwen3-4B + GSM8K, one synchronous GRPO step on four NPUs.** The data directory needs `train.parquet` and `test.parquet`, with `prompt` and `extra_info` columns in each row. `extra_info` supplies the answer; see the [dataset reader](rl/dataset/data_source.py) for parsing rules.
...
./hyper_parallel/rl/examples/gsm8k/scripts/run_qwen3_tp_docker.sh colocated
```
...
Both examples validate execution, not learning gains. For full training, use the [training entry point](train_rl.py) with the [GSM8K](examples/gsm8k/configs/qwen3_4b_gsm8k_vllm_production.yaml) or [Search-R1](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/examples/agents/search_R1/configs/multi_turn.yaml) configuration, adjusting the training budget, evaluation, and saving instead of using the launchers’ fixed-step checks.

Other entry points: [Programmatic agents](docs/agentic_rl.md) · [Deployment and sampling](docs/vllm_rollout.md) · [Bit-Exact checks](docs/qwen3_training_inference_consistency.md) · [MoE models](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/docs/moe_models.md).
...
This example reuses the GSM8K multi-turn environment and calculator, replacing only terminal scoring. Save it as `hyper_parallel/rl/examples/gsm8k/custom_reward.py`:

```python
from examples.gsm8k.agent import GSM8KMultiTurnEnvironment, build_gsm8k_environment
...
Currently validated on **single-node Ascend 910B3**; see the [runtime image](docker/README.md) for environment requirements. ✅ Supported · ◐ Partially validated · 🧪 Components only · ○ Planned. Status applies only to the listed combinations and does not establish long-term learning gains.
...
See [MoE Models](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/docs/moe_models.md) for detailed configurations and acceptance results, [TODO](docs/TODO.md) for planned dependencies and quantitative criteria, and [current limitations](#current-limitations) for usage boundaries.
...
```bash
# CPU self-checks: no model training or NPU jobs
python -m pytest -q hyper_parallel/rl/tests/st/test_st_runtime.py hyper_parallel/rl/tests/st/test_st_support.py

# Explicit real ST: run only after provisioning its resources
python -m pytest -vv -s hyper_parallel/rl/tests/st/test_rl_st.py
```

Real ST requires `RL_ST_MODEL`, `RL_ST_DATA`, `RL_ST_DEVICES`, and a locally available
[unified runtime image](docker/README.md). The six scenarios run serially and need at most four allocated NPUs.
The data directory must contain `train.parquet` and `test.parquet`; results default to `hyper_parallel/rl/output/`.
Standalone ST is excluded from runtime packages and is run from a complete source checkout.
...
| Document | Contents |
| --- | --- |
| [Runtime Image](docker/README.md) | Image download, verification, pinned dependencies, and host requirements |
...
| [MoE Models](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/docs/moe_models.md) | Models, TP/EP configurations, component ownership, and validation boundaries |
...
| [Public Module Changes](https://atomgit.com/mindspore/hyper-parallel/blob/f9f2696341c631327524ea37b6b2ca328076980a/hyper_parallel/rl/docs/public_module_changes.md) | Rationale and interface impact of changes outside the RL directory |
