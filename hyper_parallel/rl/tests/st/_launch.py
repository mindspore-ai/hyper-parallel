...
def main() -> None:
    ...
    # The result mount preserves rank evidence if SIGKILL prevents the final log drain.
    with tempfile.TemporaryDirectory(prefix=f"rl-st-{config.stem}-", dir=config.parent) as directory:
        ...
        try:
            ...
            for log in sorted(phase.rglob("*.log")):
                ...
                # Once drained, the phase log is the persistent record.
