...
cp "${rl_source_root}/pyproject.toml" "${rl_source_root}/hyper_parallel_vllm_plugin.py" "${rl_install_dir}/"
cp -r "${rl_source_root}/rl" "${rl_install_dir}/rl"
python -m pip install --no-deps --no-build-isolation "${rl_install_dir}"
python -c 'from importlib.metadata import entry_points; assert any(e.value == "hyper_parallel_vllm_plugin:register_hyper_models" for e in entry_points(group="vllm.general_plugins")), "Hyper-RL vLLM plugin is not installed"'
