...
# ============================================================================
"""Packing helpers for MindSpore fully_shard communication buffers."""
...
def supports_same_dim_strided_layout(hsdp_param: Any) -> bool:
    """Check whether the parameter's same-dimension StridedShard layout is supported for packing."""
...
def _resolve_unpacked_shape(
    ...
    if hsdp_param is not None and getattr(hsdp_param, "_orig_size", None) is not None:
        return _shape_tuple(getattr(hsdp_param, "_orig_size"))
    return _shape_tuple(local_tensor.shape)
...
def _get_packed_tensor_shape(
    ...
    return tuple(packed_tensor_shape)
    ...
    *,
    ...
    if world_size == 1:
        ...
        return ReduceScatterPlan(
            ...
            packed_shape=(1, math.prod(unpacked_shape)),
            packed_tensor_shape=unpacked_shape,
            unpacked_shape=unpacked_shape,
        )
    if len(local_tensor.shape) == 0:
    ...
    return ReduceScatterPlan(
        ...
        shard_dim=resolved_shard_dim,
        world_size=world_size,
        packed_shape=(world_size, total_numel // world_size),
...
def pack_for_reduce_scatter(
    ...
    if _shape_tuple(local_tensor.shape) != plan.unpacked_shape:
        raise AssertionError(
            "pack_for_reduce_scatter expects the unsharded local tensor shape to match "
            f"plan.unpacked_shape, but got {tuple(local_tensor.shape)} and "
            f"{tuple(plan.unpacked_shape)}"
        )
    if plan.pack_kind == "chunk_cat_non_dim0":
        chunks = ms.mint.chunk(local_tensor, plan.world_size, dim=plan.shard_dim)
        packed_tensor = ms.mint.cat(chunks, dim=0)
...
def unpack_from_all_gather(
    ...
    if plan.pack_kind == "chunk_cat_non_dim0":
        chunks = ms.mint.chunk(packed_tensor, plan.world_size, dim=0)
        return ms.mint.cat(chunks, dim=plan.shard_dim).contiguous()
