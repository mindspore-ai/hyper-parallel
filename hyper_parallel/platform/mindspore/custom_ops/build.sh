...
function show_help() {
    cat <<EOF
Usage:
  bash hyper_parallel/platform/mindspore/custom_ops/build.sh [OPTIONS]
...
for required_tool in cmake gcc g++ grep make ninja python3 sed; do
...
if [[ "${STAGE_ONLY}" == "off" ]]; then
    PAYLOAD_COMPONENT="${NATIVE_ROOT}/payload/hyper_parallel/platform/mindspore/custom_ops/lib"
    rm -rf "${PAYLOAD_COMPONENT}"
    mkdir -p "${PAYLOAD_COMPONENT}"
    cp -a "${OUTPUT_DIR}/." "${PAYLOAD_COMPONENT}/"
fi
