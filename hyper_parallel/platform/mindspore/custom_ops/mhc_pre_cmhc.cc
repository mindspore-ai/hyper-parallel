...
// gamma is optional: None from Python passes through as nullptr to aclnn
// (gammaOptional). The kernel applies (void)gamma (equivalent to gamma=1).
// value_or(ms::Tensor()) yields an empty tensor for TensorToDevice (no-op when
// gamma is absent).
std::vector<ms::Tensor> npu_mhc_pre_cmhc(const ms::Tensor &x, const ms::Tensor &phi,
                                         ...
                                         const std::optional<ms::Tensor> &gamma_opt,
    ...
    ms::TensorToDevice(x, phi, alpha, bias, gamma, perm_mats);
    ms::TensorAllocate({h_in, h_post, h_res, inv_rms, h_mix, h_pre, coeff});
    MS_DISPATCH_ACLNN(aclnnMhcPreCmhc, x, phi, alpha, bias, gamma_opt, perm_mats, norm_eps, hc_eps,
