...
class NpuMhcPreCmhcDFunction(DFunction):  # pylint: disable=W0221
    ...
    is optional and passes through as None (aclnn gammaOptional=nullptr).
    """
    ...
    def forward(ctx, x, phi, alpha, bias, perm_mats, gamma, hc_eps, norm_eps):
        ...
        Args:
            ...
            gamma: RMSNorm gamma [n, d]. dtype float32. Optional; kernel ignores (void)gamma.
            hc_eps: H_pre sigmoid eps parameter.
            norm_eps: RmsNorm eps parameter.
        ...
        ctx.save_for_backward(x, phi, alpha, perm_mats, inv_rms, h_mix, h_pre, h_post, coeff)
        ctx.hc_eps = hc_eps
        return result
    ...
    def backward(ctx, *grad_outputs):
        ...
        Returns:
            tuple: (grad_x, grad_phi, grad_alpha, grad_bias, None, None, None, None) --
                ...
                have no gradient).
        """
        x, phi, alpha, perm_mats, inv_rms, h_mix, h_pre, h_post, coeff = ctx.saved_tensors
        (grad_h_in, grad_h_post, grad_h_res,
         x, phi, alpha, perm_mats, inv_rms, h_mix, h_pre, h_post, coeff) = _ensure_contiguous(
            grad_outputs[0], grad_outputs[1], grad_outputs[2],
            x, phi, alpha, perm_mats, inv_rms, h_mix, h_pre, h_post, coeff)
