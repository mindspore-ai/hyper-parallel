...
def _canonical_input_tensor(tensor: Any) -> Any:
    """Return the local tensor carried by a DTensor-like wrapper."""
    to_local = getattr(tensor, "to_local", None)
    if hasattr(tensor, "_local_tensor") and callable(to_local):
        return to_local()
    return tensor
...
def _make_view_recipe(saved: _TensorMetadata, base: _TensorMetadata) -> Any:
    ...
    if not exact_input:
        # Exact non-contiguous inputs are safe, but deriving arbitrary views from
        # non-contiguous bases can cross holes in their logical footprint.
