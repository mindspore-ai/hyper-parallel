...
class _TorchContiguousGrad(torch.autograd.Function):
    """Keep the forward value unchanged and materialize its backward gradient."""

    @staticmethod
    def forward(ctx: Any, tensor: Tensor) -> Tensor:  # pylint: disable=arguments-differ
        """Return the collective output without a forward copy."""
        del ctx
        return tensor

    @staticmethod
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:  # pylint: disable=arguments-differ
        """Return a contiguous gradient to the upstream collective."""
        del ctx
