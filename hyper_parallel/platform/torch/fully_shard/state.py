...
class TorchHSDPStateV2(HSDPState):
    ...
    def set_separated_shard_comm(self, enable_separated: bool) -> None:
        """Select a dedicated shard communicator for this state's ReduceScatter.

        AllGather continues using ``mesh_info.shard_process_group``.  A single
        replacement group per original process group is shared across the
        configured module tree, without changing DeviceMesh's group cache.
        """
        import torch.distributed as dist  # pylint: disable=C0415

        if not enable_separated:
        ...
        for hsdp_param in self.hsdp_params:
            ...
            if shard_group is None or hsdp_param.shard_world_size <= 1:
                continue
            reduce_group = self.scheduler_ctx.separated_shard_groups.get(shard_group)
            if reduce_group is None:
                # Only members participate, so HSDP/EP subgroups need not call
                # new_group for other ranks' meshes. Call order must agree on
                # overlapping groups, as for the original FSDP collectives.
                reduce_group = dist.new_group(
                    ranks=dist.get_process_group_ranks(shard_group),
                    backend=dist.get_backend(shard_group),
                    use_local_synchronization=True,
                )
                self.scheduler_ctx.separated_shard_groups[shard_group] = reduce_group
            hsdp_param.mesh_info.reduce_scatter_process_group = reduce_group
