...
from .ascend_ub_manager import (
    ASCEND_MAX_GRID_DIM,
    compute_row_tile_block_size,
    max_grid_axis_chunks,
)
from .chunk_intra_token_parallel import (
    chunk_kda_fwd_intra_token_parallel_npu as chunk_kda_fwd_intra_token_parallel,
)
from .utils import ascend_compile_kwargs, exp2, input_guard, prepare_chunk_indices
from .wy_fast import recompute_w_u_fwd_kda_npu as _recompute_w_u_fwd_npu
...
# This fused inter kernel can livelock when Triton-Ascend applies automatic
# multibuffering or AutoBlockify. Only pass options exposed by the compiler.
...
def chunk_kda_bwd_kernel_intra_dq_db_npu(
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        i_ti = i_t * BT + i_i * BC
        if i_ti < T_cur:
            all_tokens = tl.cast(B, tl.int64) * T
            ...
            o_k = i_k * BK + tl.arange(0, BK)
            m_k = o_k < K
            db_ptr = db + (i_k * all_tokens + bos).to(tl.int64) * HV + i_hv
