...
# pylint: disable=redefined-outer-name,missing-public-type-hints,missing-public-docstring
...
def ascend_compile_kwargs(*, blacklist_auto_blockify: bool = False) -> dict:
    """Return stable Triton-Ascend launch options supported by this compiler."""
    ...
    if blacklist_auto_blockify:
        try:
            ...
            return kwargs
        fields = getattr(NPUOptions, "__dataclass_fields__", {})
        if "has_auto_blockify_blacklist_op" in fields:
            kwargs["has_auto_blockify_blacklist_op"] = True
    return kwargs


# torch_npu inductor uses ``extra.ascend`` while Triton-Ascend 3.2.1 exposes
# the same helpers as ``extra.cann``.
try:
    if not hasattr(tl.extra, "ascend") and hasattr(tl.extra, "cann"):
        tl.extra.ascend = tl.extra.cann
        sys.modules["triton.language.extra.ascend"] = tl.extra.cann
...
@triton.jit
def softplus(x):
    """Numerically stable softplus used by the KDA gate kernels."""
    return tl.where(x < 20.0, tl.math.log(1.0 + tl.math.exp(x)), x)
