...
def _require_triton_ascend() -> None:
    """Require a usable Triton runtime with the Ascend backend."""
    try:
        import_module("triton")
        ascend_backend = util.find_spec("triton.backends.ascend")
    except (ImportError, ModuleNotFoundError) as exc:
        raise RuntimeError(
            "The fused KDA backend requires a usable Triton-Ascend module."
        ) from exc
    if ascend_backend is None:
        raise RuntimeError(
            "The fused KDA backend requires the Triton Ascend backend."
        )
...
class _KDAStateP2PFunction(torch.autograd.Function):
    ...
    def forward(  # pylint: disable=arguments-differ,too-many-arguments,too-many-locals
        ...
        from .triton.beta_gate import (  # pylint: disable=import-outside-toplevel
            fused_beta_sigmoid_fwd_npu as fused_beta_sigmoid,
        )
        from .triton.chunk_delta_h import (  # pylint: disable=import-outside-toplevel
            chunk_gated_delta_rule_fwd_h_npu as chunk_gated_delta_rule_fwd_h,
        )
        from .triton.chunk_intra import (  # pylint: disable=import-outside-toplevel
            chunk_kda_fwd_intra_npu as chunk_kda_fwd_intra,
        )
        from .triton.gate import (  # pylint: disable=import-outside-toplevel
            kda_gate_chunk_cumsum_npu as kda_gate_chunk_cumsum,
        )
        from .triton.l2norm import (  # pylint: disable=import-outside-toplevel
            l2norm_fwd_npu as l2norm_fwd,
        )
        from .triton.output import (  # pylint: disable=import-outside-toplevel
            chunk_kda_fwd_output,
        )
        ...
        w, u, _, kg, attention_qk, attention_kk = chunk_kda_fwd_intra(
            q=query,
            k=key,
            v=value,
            gk=gate,
        ...
        states, value_new, _ = chunk_gated_delta_rule_fwd_h(
            k=kg,
            w=w,
            u=u,
            gk=gate,
        ...
        output = chunk_kda_fwd_output(
            q=query,
            v=value_new,
            g=gate,
            attention=attention_qk,
            h=states,
            scale=scale,
            chunk_size=chunk_size,
        )
    ...
    def backward(  # pylint: disable=arguments-differ,too-many-locals
        ...
        from .triton.beta_gate import (  # pylint: disable=import-outside-toplevel
            fused_beta_sigmoid_bwd_npu as fused_beta_sigmoid_bwd,
            fused_beta_sigmoid_fwd_npu as fused_beta_sigmoid,
        )
        from .triton.chunk_bwd import (  # pylint: disable=import-outside-toplevel
            chunk_kda_bwd_dAv_npu as chunk_kda_bwd_dAv,
            chunk_kda_bwd_wy_dqkg_fused_npu as chunk_kda_bwd_wy_dqkg_fused,
        )
        from .triton.chunk_delta_h import (  # pylint: disable=import-outside-toplevel
            chunk_gated_delta_rule_bwd_dhu_npu as chunk_gated_delta_rule_bwd_dhu,
            chunk_gated_delta_rule_fwd_h_npu as chunk_gated_delta_rule_fwd_h,
        )
        from .triton.chunk_intra import (  # pylint: disable=import-outside-toplevel
            chunk_kda_bwd_intra_npu as chunk_kda_bwd_intra,
        )
        from .triton.cumsum import (  # pylint: disable=import-outside-toplevel
            chunk_local_cumsum_npu as chunk_local_cumsum,
        )
        from .triton.gate import (  # pylint: disable=import-outside-toplevel
            kda_gate_bwd_npu as kda_gate_bwd,
            kda_gate_chunk_cumsum_npu as kda_gate_chunk_cumsum,
        )
        from .triton.l2norm import (  # pylint: disable=import-outside-toplevel
            l2norm_bwd_npu as l2norm_bwd,
        )
        from .triton.wy_fast import (  # pylint: disable=import-outside-toplevel
            recompute_w_u_fwd_kda_npu as recompute_w_u_fwd,
        )
        ...
        w, u, query_gated, key_gated = recompute_w_u_fwd(
            q=query,
            k=key,
            v=value,
            beta=beta,
        ...
        grad_attention_qk, grad_value_local = chunk_kda_bwd_dAv(
            q=query,
            k=key,
            v=value_new,
            do=grad_output,
        ...
        grad_states, _, grad_value = chunk_gated_delta_rule_bwd_dhu(
            q=query_gated,
            k=key_gated,
            w=w,
            gk=gate,
        ...
        grad_query, grad_key, grad_beta, grad_gate = chunk_kda_bwd_intra(
            q=query,
            k=key,
            g=gate,
            beta=beta,
        ...
        grad_gate = chunk_local_cumsum(
            grad_gate,
            chunk_size=ctx.chunk_size,
            reverse=True,
        )
        grad_gate, grad_a_log, grad_dt_bias = kda_gate_bwd(
            g=gate_raw,
            A_log=a_log,
            dt_bias=dt_bias,
            dyg=grad_gate,
        ...
        grad_beta = fused_beta_sigmoid_bwd(beta_raw, grad_beta)
        grad_query = l2norm_bwd(query, query_rstd, grad_query)
        grad_key = l2norm_bwd(key, key_rstd, grad_key)
        if send_work is not None:
            send_work.wait()
