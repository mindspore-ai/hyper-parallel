...
from torch.distributed.algorithms._checkpoint.checkpoint_wrapper import checkpoint_wrapper as ckpt_wrapper
...
class SelectiveCheckpointContext:
    ...
    Example:
        ...
        >>>
        >>> context_fn = functools.partial(create_checkpoint_contexts, policy_fn)
        >>>
...
class _CachingTorchDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if policy in (CheckpointPolicy.MUST_SAVE, CheckpointPolicy.PREFER_SAVE):
            self.storage.save_storage[func].append(tree_map(
                lambda x: _VersionWrapper(_maybe_detach(x, any_ret_has_alias_info)), out))
            logger.debug("op is %s, policy is %s, save to device", func, policy)
            ...
            self.storage.swap_storage[func].append(
                tree_map(lambda x: SwapTensor(_maybe_detach(x, any_ret_has_alias_info)), out))
            logger.debug("op is %s, policy is %s, offload to host mem", func, policy)
        else:
            logger.debug("op is %s, policy is %s, recompute activation", func, policy)
        return out
...
class _CachedTorchDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if policy in (CheckpointPolicy.MUST_SAVE, CheckpointPolicy.PREFER_SAVE):
            storage = self.storage.save_storage.get(func)
            if storage is None:
                raise RuntimeError(
            ...
            logger.debug("op is %s, policy is %s, direct return", func, policy)
            out = tree_map(lambda x: x.get_val(
                self.allow_cache_entry_mutation), storage.pop(0))
        elif policy == CheckpointPolicy.MUST_SWAP:
            ...
            logger.debug("op is %s, policy is %s, load from host mem", func, policy)
...
def create_checkpoint_contexts(policy_fn_or_list, allow_cache_entry_mutation=False):
    """
    ...
    Example:
        ...
        >>>
        >>> # or equivalently
        >>> context_fn = functools.partial(create_checkpoint_contexts, ops_to_save)
        >>>
        >>> def fn(x, y):
