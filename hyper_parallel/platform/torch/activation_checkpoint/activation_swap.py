...
from hyper_parallel.utils import logger
    ...
    def __init__(self, policy_fn=None) -> None:
        ...
        def pack_to_cpu(tensor: torch.Tensor):
            ...
            if not self.add_to_storage:
                ...
                logger.info(f"swap add to group is {group_name}")
            logger.info(f"enter pack, tensor shape is {tensor.shape}, tensor id {id(tensor)}")
        ...
        def unpack_from_cpu(idx) -> torch.Tensor:
            ...
            logger.info(f"enter unpack, tensor shape is {tensor.shape}, tensor id {id(tensor)}")
            self.unpack_count += 1
...
class ActivationWrapper(torch.nn.Module, ABC):
    ...
    def _post_state_dict_hook(
        ...
        *args: Any, # pylint: disable=W0613
        ...
        so that this module can be loaded into non-swaped modules.
    ...
    def _pre_load_state_dict_hook(
        ...
        *args: Any,
        ...
        prefix so that non-swaped modules can be loaded into
