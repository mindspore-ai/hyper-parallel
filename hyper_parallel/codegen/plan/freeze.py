...
def freeze_plan(plan: Any, model: Any) -> FrozenPlan:
    ...
    return FrozenPlan(
        ...
        frozen_sharded_params=expand_frozen_sharded_params(plan),
...
def freeze_param_plan(plan: Any) -> dict[str, dict[str, Any]]:
    ...
    for fqn, spec in modules.items():
        entry: dict[str, Any] = {}
        for spec_field in PARAM_PLAN_SPEC_FIELDS:
            frozen = _freeze_spec_field(spec_field, getattr(spec, spec_field.name, None))
            if frozen is not _MISSING:
                entry[spec_field.key] = frozen
        result[fqn] = entry
    return result
...
def _freeze_spec_field(spec_field: Any, value: Any) -> Any:
...
def freeze_injections(plan: Any) -> list[dict[str, Any]]:
    ...
    for fqn, spec in modules.items():
        entry: dict[str, Any] = {}
        for spec_field in INJECTION_SPEC_FIELDS:
            frozen = _freeze_spec_field(spec_field, getattr(spec, spec_field.name, None))
            if frozen is not _MISSING:
                entry[spec_field.key] = frozen
        if entry:
...
def expand_frozen_sharded_params(plan: Any) -> list[str]:
