...
- ``region``        — the boundary has a ``local_compute_fn`` injection (EP
  MoE local region): the forward delegates to ``self.__hp_compute__`` and
...
- ``generic`` (forced) — a boundary with an ``inner_wrapper`` injection is
  ...
  to the module at runtime by ``hp_apply_inner_wrapper``, so the class
...
- ``tp_collective`` — every declared transition is identity or TP-lowerable
  ...
  emitter may sink bare operator calls (``self._hp_tp.all_gather(...)``),
...
- ``generic``       — anything else (non-tp axis differences, unsupported
  ...
  the instance-bound compiled plan (``self._hp_boundary``) with a
...
def classify_boundary_form(
    ...
    """
    # A local-compute region runs its compute fn on local tensors between the
    # entry/exit redistributes; the sides are still classified below so the
    # emitter's region template can annotate each transition.  An
    # unlowerable transition does NOT force the generic form there — the
    # compiled plan's redistribute owns it either way.
    is_region = injection is not None and injection.get("local_compute_fn") is not None
    # An inner-wrapper boundary keeps the generic rewrite even when every
    # transition is identity: ``hp_apply_inner_wrapper`` mutates the target
    ...
    entry_out_names = declared_out_names(entry)
    if is_region:
        return BoundaryForm(
            form=FORM_REGION,
            in_ops=in_ops,
...
def _entry_has_ep_placement(entry: dict[str, Any]) -> bool:
    ...
    forward's ``_hp_boundary`` reference would never be bound.
    """
...
def declared_out_names(entry: dict[str, Any]) -> tuple[str, ...]:
    """The entry's declared output order (``out_names`` or ``out_src`` keys).

    Output names are not retained on the frozen entry, so a local-region
    forward wraps its local results per ``out_src`` before the boundary exit
    and reads the declared order off the same place the rewrap does.
    """
...
def is_boundary_entry(entry: Any) -> bool:
    """Whether a frozen param-plan entry is a boundary.

    Single definition for both the emitter (which sinks the entry's forward
    into its boundary form) and the installer (which wraps the module): an
    entry carrying any layout contract is a boundary, an explicit
    ``is_boundary`` wins over that inference.  Two copies of this predicate
    had drifted apart -- they disagreed on an entry carrying only
    ``out_names`` -- so both consumers read it from here.
    """
    if not isinstance(entry, dict):
        return False
    if entry.get("is_boundary") is not None:
        return bool(entry["is_boundary"])
    return any(entry.get(field) for field in _ENTRY_META_FIELDS)


#: Frozen-entry keys that make an entry a boundary: the in/out placement
#: contracts plus the declared output order a local-region forward wraps by.
