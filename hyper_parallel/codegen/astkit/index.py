...
class FunctionInfo:
    ...
    #: Name of the ``*args`` parameter, if present.
    vararg_name: Optional[str] = None
    #: Name of the ``**kwargs`` parameter, if present.
...
class SourceIndex:
    ...
    #: Byte offset where the import block ends and code/classes may begin.
...
def _build_function_info(node: ast.AST, text: str, offset_for) -> FunctionInfo:
    """Collect offsets, parameters and ``*args``/``**kwargs`` names for a def node."""
    ...
    if params is not None:
        ...
        vararg_name = params.vararg.arg if params.vararg is not None else None
        kwarg_name = params.kwarg.arg if params.kwarg is not None else None
...
def returns_any_value(source_text: str, func: FunctionInfo) -> bool:
    """Whether every ``return`` exits with a single value or a tuple value.

    Static tuple-output templates can apply output redistribution to
    ``outputs[0]`` and rebuild the original object, matching the compiled plan's
    index-0 behavior. A bare ``return`` has no value to transform; a list return
    would need list-specific rebuilding, so it fails closed for now.
    """
    if func.body_start is None or func.body_end is None:
        return False
    def_line_start = _def_keyword_offset(source_text, func)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Return):
            ...
            if node.value is None:
                return False
            if isinstance(node.value, (ast.List, ast.Starred)):
                return False
    return seen_return
...
def _line_offsets(text: str) -> list[int]:
    """Byte offset of each line's start (``offsets[line_no - 1]`` is 1-based line ``line_no``).

    Single implementation, shared with ``codegen.source.compat`` (which imports
    it from here — the existing layering direction is ``source`` -> ``astkit``).
    """
    offsets: list[int] = [0]
