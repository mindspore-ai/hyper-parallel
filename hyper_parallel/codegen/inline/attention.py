...
lowering + ``hyper_install_boundaries``) and the declared CP inner wrapper is
installed by ``hyper_apply_inner_wrapper`` — the same shared mechanisms native
uses at apply time.
...
#: ``hyper_install_boundaries``) and the CP inner wrapper is installed by
