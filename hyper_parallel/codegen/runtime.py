...
def hyper_install_boundaries(
    ...
    *,
    ...
    region_fqns = {
        rule.get("match")
        for rule in (injections or [])
        if rule.get("local_compute_fn") is not None and rule.get("match")
    }
    ...
    for fqn, entry in sorted(param_plan.items()):
        ...
        if _has_lowered_forward(module):
            ...
            if fqn in region_fqns and entry.get("out_src"):
                # The lowered forward calls ``self._hyper_rewrap(outputs)``
