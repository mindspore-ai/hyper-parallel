...
def _rewrap_execute(
    output: Any,
    rewrap_plan: list[tuple[int, str, tuple]],
    mesh: Any,
) -> Any:
    """Execute pre-parsed rewrap ops against one forward output."""
...
def _compile_boundary(entry: dict[str, Any], mesh: Any, mesh_dim_names):
    ...
    The generated path compiles per module at install time instead of sharing
    the ``_BOUNDARY_CACHE`` entry: ``_bind_input_indices`` mutates
    ``op.arg_index`` against one module's forward signature, and two classes
...
class InlineParallelState:
    """Thin runtime state read by clean inline generated forwards."""

    def __init__(self, *, tp: Any = None, cp_mesh: Any = None, ep_group: Any = None) -> None:
        self.tp = tp
    ...
    def cp_enabled(self) -> bool:
        return self.cp_mesh is not None and self.cp_mesh.size() > 1

    @property
    def ep_enabled(self) -> bool:
        return self.ep_group is not None and self.ep_group.size() > 1
...
def get_inline_parallel_state(module_name: str) -> InlineParallelState:
    """Return the state registered for one generated modeling module."""
    return _INLINE_PARALLEL_STATE_BY_MODULE.get(
        module_name, _EMPTY_INLINE_PARALLEL_STATE
    )


def _register_inline_parallel_state(
    generated_module: Any,
    mesh_context: Any,
    mesh_dim_names: Optional[tuple[str, ...]],
) -> None:
    """Install the one module-level state object clean inline forwards read."""
    ...
    tp_ops = _build_generated_tp_ops(generated_module, tp_mesh)
    _INLINE_PARALLEL_STATE_BY_MODULE[generated_module.__name__] = InlineParallelState(
        tp=tp_ops,
        cp_mesh=cp_mesh,
        ep_group=ep_group,
    )
...
def _build_generated_tp_ops(generated_module: Any, tp_mesh: Any) -> Any:
    """Construct generated-file TP ops with direct production collectives."""
    if tp_mesh is None or tp_mesh.size() <= 1:
        return None
    tp_ops_type = getattr(generated_module, "TPOperators", None)
    if tp_ops_type is None:
        return None
    group = _mesh_group(tp_mesh, "tp")
    if group is None:
        return None
    ...
    return tp_ops_type(group, tp_mesh.size(), tp_mesh.get_local_rank(), backend)
    ...
    *,
    ...
    for fqn, entry in sorted(param_plan.items()):
        ...
        if _has_lowered_forward(module, generated_module):
            injection = _injection_spec(injections or [], param_plan, fqn)
            installed = InstalledBoundary(
...
def _parallelize_inline_from_meta(model: Any, mesh_context: Any, meta: CodegenMeta, generated_module: Any) -> dict:
    ...
    mesh_dim_names = meta.mesh_dim_names
    injections = hyper_expand_injections(meta.injections or [])
    _register_inline_parallel_state(generated_module, mesh_context, mesh_dim_names)
    hyper_shard_params(model, param_plan, mesh_context, mesh_dim_names)
    hyper_apply_special_handlers(
        model, meta.special_handlers or {}, mesh_context, mesh_dim_names
    )
    hyper_replicate_tied(model, meta.tied_pairs or [])
    ...
    return hyper_build_tp_grad_info(
        model, param_plan, mesh_context, tied_pairs=meta.tied_pairs or []
    )
