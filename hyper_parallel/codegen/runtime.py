...
forward assembly (the codegen Phase C helpers ``hp_install_boundaries`` /
``hp_bind_compute`` / ``hp_apply_inner_wrapper``) stays literal, because
...
def publish_codegen_mesh_context(artifact_dir: str, mesh_context: Any) -> None:
    ...
    identity redistributions well-defined without calling ``hp_parallelize``.
    """
...
def hp_build_tp_grad_info(
    ...
    plain locals, so ``hp_build_tp_grad_info`` must be the LAST sharding
    helper the generated ``hp_parallelize`` runs (the applier's own ordering
    invariant).
    """
...
class _FrozenBoundarySpec:
    ...
    The injection helpers (``hp_bind_compute`` and
    ``hp_apply_inner_wrapper``) also hand this to the applier's resolution
...
def hp_to_local_if_dtensor(output):
...
class InstalledBoundary:
    ...
    ``hp_install_boundaries`` resolves everything once, at install time
    ...
    ``self._hp_boundary.redistribute_inputs/outputs`` with zero per-call
    ...
    def __init__(
        ...
        *,
        ...
        if dense_mesh is None:
            raise ValueError(
                "hp_install_boundaries requires a DeviceMesh / MeshContext "
...
class TPOperators:
    ...
    A ``tp_collective`` template calls ``self._hp_tp.<kind>(...)`` directly
    (``emit/parallel``'s static form); ``hp_install_boundaries`` binds this
...
def _install_inline_state_bindings(module: Any, expert_mesh: Any) -> None:
    ...
    the boundary templates read: ``self._hp_ep_group`` plus the
...
def _is_external_state_inline_module(module: Any, generated_module: Any = None) -> bool:
    ...
    ``hp_bind_inline_state`` binds, so the boundary installers must leave it
    alone.
    """
    if generated_module is None:
        return False
...
def _install_deferred_bias_pair(module: Any, spec: Any, *, lowered: bool) -> None:
    ...
    ``module._hp_deferred_bias``, which the emitted forward templates call
    ...
    ``self._hp_deferred_bias(outputs)`` reads it at the exit.  Child-owned
    ...
    module._hp_deferred_bias = functools.partial(
...
def _wrap_installed_boundary_forward(
    ...
    def codegen_boundary_forward(*args: Any, **kwargs: Any) -> Any:
        ...
        if deferred_bias:
            outputs = module._hp_deferred_bias(outputs)
        return outputs
...
def _wrap_static_fallback_forward(module: Any, installed: InstalledBoundary) -> None:
    ...
    The static template's body references ``self._hp_tp``, which this
    ...
    already bound on the module (``_hp_deferred_bias``, installed before
    ...
    deferred_bias = getattr(module, "_hp_deferred_bias", None)
...
def _install_static_tp_operators(
    ...
    bind ``module._hp_tp`` only when the live verdict reproduces the
    ...
    if (
        ...
        module._hp_tp = TPOperators(lowerer)
        return
    logger.warning(
        "hp_install_boundaries: static tp_collective forward on %s does not "
...
def _install_toggle_bindings(
    ...
    calls ``self._hp_tp.<kind>(...)`` inside ``if self.{axis}_enable:``
    ...
    - ``module.tp_enable`` / ``module.cp_enable`` / ``module.ep_enable`` are
      ...
      genuinely active.
    - ``module._hp_tp`` reuses the SAME ``TPOperators`` + native lowerer that
...
def hp_install_boundaries(
    ...
    *,
    ...
    ``InstalledBoundary`` is stored as ``module._hp_boundary`` — the
    ...
    its bare operators bound (``module._hp_tp``) only after the live-mesh
    ...
    ``module._hp_deferred_bias`` re-adds each bias exactly once after the
...
def hp_expand_injections(
    ...
    called once at the top of the generated ``hp_parallelize`` — restores
    the per-FQN shape, so ``hp_install_boundaries`` /
    ``hp_bind_compute`` / ``hp_apply_inner_wrapper`` keep their frozen
...
def hp_bind_compute(
    ...
    *,
    ...
    """Bind ``module.__hp_compute__`` for every local-region boundary.

    A generated local-compute forward calls
    ``self.__hp_compute__(*args, **kwargs)`` on the redistributed inputs.
    ...
    if dense_mesh is None:
        raise ValueError(
            "hp_bind_compute requires a DeviceMesh / MeshContext to bind compute against"
        )
    if not injections:
        return
    for rule in injections:
        ...
        if not fqn:
            raise ValueError(
                f"hp_bind_compute: injection rule {rule!r} has no 'match' FQN"
            )
        ...
        # inlined forward and must not receive __hp_compute__, but the local
        ...
        # bound as __hp_compute__.
        if compute_fn is None or compute_fn is module.forward:
            continue
        module.__hp_compute__ = compute_fn


def hp_apply_inner_wrapper(
    model,
    injections: list[dict[str, Any]],
    mesh_context,
    mesh_dim_names: Optional[tuple[str, ...]] = None,
    *,
    ...
    if dense_mesh is None:
        raise ValueError(
            "hp_apply_inner_wrapper requires a DeviceMesh / MeshContext to install against"
        )
    if not injections:
        return
    ...
    for rule in injections:
        ...
        if not fqn:
            raise ValueError(
                f"hp_apply_inner_wrapper: injection rule {rule!r} has no "
                "'match' FQN"
            )
        entry = (param_plan or {}).get(fqn, {})
        spec = _FrozenBoundarySpec(entry, rule)
...
def _install_vocab_parallel_embedding(module: Any, tp_mesh: Any) -> None:
    ...
    ``hp_install_boundaries`` is documented idempotent, while native Phase C
    runs exactly once.
    """
...
def parallelize_from_generated(
    ...
    *,
    ...
    """Run the generated module's ``hp_parallelize`` entry point.
...
def _parallelize_via_native_apply(
    ...
    runtime against the ``_hp_boundary`` / ``__hp_compute__`` bound
    ...
    injections = hp_expand_injections(meta.injections or [])
...
__all__ = [
    ...
    "hp_bind_compute",
    "hp_bind_inline_state",
    "hp_build_tp_grad_info",
    "hp_install_boundaries",
    "hp_to_local_if_dtensor",
