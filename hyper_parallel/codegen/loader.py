...
"run generation before building from the generated artifact"
        )
    candidates = sorted(
...
def _load_generated_checkpoint(
    ...
    if torch_dtype not in (None, "auto"):
        _cast_parameters_to(model, torch_dtype)
    return model


def _cast_parameters_to(model: Any, torch_dtype: Any) -> None:
    """Cast the model's parameters to ``torch_dtype`` the way native loading does.
    ...
    value and makes the generated model diverge numerically from the native one
    even though both consume the same checkpoint.  Modules listed in
    ``_keep_in_fp32_modules`` stay in fp32 on both paths, so they are skipped.

    Args:
        model: Model whose parameters are cast in place.
        torch_dtype: Configured dtype, either ``"bfloat16"`` or
            ``"torch.bfloat16"``.
    """
    import torch  # pylint: disable=import-outside-toplevel

    dtype = getattr(torch, str(torch_dtype).rsplit(".", 1)[-1])
...
def _source_model_shapes(hf_config: Any) -> dict[str, tuple[int, ...]]:
    ...
    from hyper_parallel import init_empty_weights  # pylint: disable=import-outside-toplevel
