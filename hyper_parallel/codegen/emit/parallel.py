...
def lower_forward_boundaries(
    ...
    *,
    ...
    / ``_hyper_tp`` instance attributes that ``hyper_install_boundaries`` binds
...
def lower_forward_boundaries_toggle(
    ...
    *,
    ...
    - ``self._hyper_tp`` — the bare TP operator object bound by
      ``hyper_install_boundaries``'s collectives step for the ``tp_enable`` body.
...
def _render_toggle_forward(
    ...
    *,
    ...
    ``self._hyper_tp``; the ``tp_enable`` / ``cp_enable`` / ``ep_enable``
    ...
    for axis in reversed(("ep", "cp", "tp")):
        ...
        for name, op in sorted(axis_ops.items()):
            if op.kind == "all_reduce":
                lines.append("            outputs = self._hyper_tp.all_reduce(outputs)")
            elif op.kind == "all_gather":
                lines.append(
                    f"            outputs = self._hyper_tp.all_gather(outputs, dim={op.tensor_dim})"
                )
            else:
                lines.append(
                    f"            outputs = self._hyper_tp.reduce_scatter(outputs, dim={op.tensor_dim})"
                )
        lines.append("")
...
def _build_forward_body(
    ...
    instance-bound compiled plan (``self._hyper_boundary``, installed by
    ``hyper_install_boundaries``); the static ``tp_collective`` form calls bare
...
def _render_output_tp_call(
    ...
    if tuple_output and (name not in out_names or out_names.index(name) == 0):
        return [
            f"        _hyper_output_0 = {_render_tp_call_expr('outputs[0]', op)}",
            "        outputs = (_hyper_output_0, *outputs[1:])",
        ]
...
def _render_local_compute(
    ...
    **kwargs)`` (``_wrap_local_region_forward``), so the generated forward
    ...
    return "\n".join([
        ...
        *_ordered_notes(form.in_notes),
        input_redist,
        "        args = tuple(hyper_to_local_if_dtensor(arg) for arg in args)",
        "        kwargs = {",
        "            key: hyper_to_local_if_dtensor(value)",
