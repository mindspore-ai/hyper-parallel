...
def _record_for_target(
    ...
    """Serialize one replacement target into its meta record."""
    spec = target.spec
    factory_path = paths_by_spec.get(id(spec))
    if factory_path is None:
        raise TypeError(
            f"codegen: replacement factory for {spec.match!r} has no serializable "
            "import path — the gen backend needs a YAML Target-backed "
            "@module_replacement factory"
        )
    record = {
