...
def inject_codegen_imports(text: str) -> str:
...
def inject_hyper_parallelize(text: str, frozen_plan: Any) -> str:
    ...
    replication, instance-level wrapping for boundaries whose class was not
    sunk into source (``hyper_wrap_module_boundaries``), forward binding
    binding (``hyper_bind_compute`` for the shape-2 EP local region,
    ``hyper_apply_inner_wrapper`` for the shape-3 CP inner wrapper), then build
    ``tp_grad_info`` LAST (that is the one-shot ``_local_params_context``
    unwrap; after it the model's DTensor params are permanently plain locals,
    so nothing may shard after it).
    """
    body = (
        "\n"
        ...
        "\n"
        "    ``mesh_context`` is published into the module ``__dict__`` (the\n"
        "    lowered boundary forwards reference it as a bare global), so a\n"
        "    direct ``layer.mlp(x)`` call after parallelization resolves it.\n"
        "    \"\"\"\n"
        "    globals()[\"mesh_context\"] = mesh_context\n"
        "    hyper_shard_params(\n"
...
def emit_modeling_file(meta: Any) -> str:
    ...
    text = inject_codegen_imports(text)
...
def _lower_forward_boundaries(text: str, meta: Any, module_name: str) -> str:
    ...
    Each boundary class's ``forward`` is rewritten to call
    ``hyper_redistribute`` on the way in/out and run the region's compute
...
def _banner(meta: Any) -> str:
    ...
    lines = [
        "# Generated by Hyper Codegen. DO NOT EDIT MANUALLY.",
        "#",
