...
class CodegenMeta:
    ...
    #: Generated class names whose ``forward`` an inline strategy body replaces
    #: (the EP MoE shell), so it orchestrates through instance attributes bound
    #: at install time (``hyper_bind_inline_state``) instead of a compiled
    #: boundary.  Derived from structure at generation time and copied here so
    #: the runtime reads the artifact's own record rather than a second copy
    #: that has to be kept in sync by hand.  A *rendered* component class keeps
    #: its own forward plus the emitted boundary form, so it is not listed.
