...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    setattr(args, "_config_overrides", config_overrides)
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    config = _load_config(args.config, _cli_overrides(args))
...
def cmd_check_all(args: argparse.Namespace) -> int:
    ...
    if _cli_overrides(args):
        raise ValueError("check --all does not accept per-config CLI overrides")
    failed = 0
...
def _load_config(yaml_path: str, overrides: Optional[list[str]] = None):
    """Load a ``TrainerConfig`` from a YAML path."""
    from hyper_parallel.trainer.config.parser import load_training_config

    return load_training_config(yaml_path, overrides or [])
...
def _model_name(config: object) -> Optional[str]:
    from hyper_parallel.codegen.manager import (  # pylint: disable=import-outside-toplevel
        _model_name as _manager_model_name,
        _resolve_hf_config_for_layout,
    )

    return _manager_model_name(config, _resolve_hf_config_for_layout(config))


def _model_name_for(yaml_path: str, overrides: Optional[list[str]] = None) -> Optional[str]:
    """Model name from a YAML (used by ``clean`` before config load)."""
    try:
        return _model_name(_load_config(yaml_path, overrides))
    except Exception:  # noqa: BLE001 - clean falls back to default naming
        return None


def _cli_overrides(args: argparse.Namespace) -> list[str]:
    """Typed config overrides captured after argparse consumes codegen flags."""
    return list(getattr(args, "_config_overrides", None) or [])
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the argparse parser for the codegen CLI."""
    parser = argparse.ArgumentParser(
        prog="hyper_parallel.codegen",
        description="Codegen: generate/check/clean codegen artifacts.",
    )
...
def _cmd_check_dispatch(args: argparse.Namespace) -> int:
    ...
    modes = [bool(args.all), bool(args.transformers_pin)]
    if sum(modes) > 1:
        raise ValueError("check accepts only one of --all / --transformers-pin")
    if args.all:
        return cmd_check_all(args)
    if args.transformers_pin:
        return cmd_check_pin(args)
    if not args.config:
        raise ValueError("check needs --config, --all, or --transformers-pin")
