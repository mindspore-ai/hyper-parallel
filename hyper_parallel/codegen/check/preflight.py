...
def verify_boundary_forms(meta: Any, layout: Any) -> None:
    ...
    - a fully inlined component body (no ``_forward_impl``, no marker): its
      forward must expose the instance-bound channel (``self._hyper_tp`` /
      ``self._hyper_cp_mesh`` / ``self._hyper_ep_group`` /
    ...
    for class_name, _form, emitted, func, _injection, _deferred_bias in iter_emitted_forms(
        ...
        has_inline_marker = "_hyper_inline_boundary = True" in class_body
        body = source_text[func.body_start:func.body_end]
        # Every generated communication path — inlined or lowered — reads the
        # instance-attribute channel (``self._hyper_tp`` / ``self._hyper_cp_mesh``
        # / ``self._hyper_ep_group`` / ``self.<axis>_enable``) the runtime binds
