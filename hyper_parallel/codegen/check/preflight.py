...
def verify_boundary_forms(meta: Any, layout: Any) -> None:
    ...
    for class_name, _form, emitted, func, _injection in iter_emitted_forms(
        ...
        has_clean_inline = "get_parallel_state()" in body and any(
            token in body
            for token in (
                "ps.tp.",
                "ps.cp_mesh",
        ...
        if has_clean_inline:
            stray = re.search(
                r"self\._hyper_(?:inline_boundary|tp|cp_mesh|ep_group)", body
            )
            if stray is not None:
                raise RuntimeError(
                    f"codegen preflight: clean inline boundary class "
                    f"{class_name} still references {stray.group(0)}; "
        ...
        if has_inline_marker:
            ...
            if has_marker:
                raise RuntimeError(
                    f"codegen preflight: inline boundary class {class_name} "
                    f"also carries the {TP_FORM_MARKER} marker; regenerate "
                    "the artifact"
                )
            continue
