...
def verify_boundary_forms(meta: Any, layout: Any) -> None:
    ...
    - ``tp_collective``: the class carries a form marker (``TP_FORM_MARKER``
      ...
      (``self._hp_tp`` / ``self._forward_impl``) — plus the D-22 exit call
      ``self._hp_deferred_bias`` when the class's frozen entry declares
    ...
    - a fully inlined component body (no ``_forward_impl``, no marker): its
      forward must expose the instance-bound channel (``self._hp_tp`` /
      ``self._hp_cp_mesh`` / ``self._hp_ep_group`` /
      ``self.<axis>_enable``) and must not read the removed module-level
      parallel-state accessor.  A class whose forward an inline *strategy*
      body replaced (the MoE EP shell) is stripped from ``boundary_classes``,
      so it is not visited here; this shape covers an artifact that still
      marks its inline orchestration explicitly.
    ...
    for class_name, _, emitted, func, _, deferred_bias in iter_emitted_forms(
        source_text,
        meta,
        ...
        # The opt-in toggle template marks its classes with TOGGLE_FORM_MARKER:
        # a boundary the classifier calls tp_collective carries either marker
        # depending on which generation mode emitted it.
        has_toggle_marker = (
            f'{TP_FORM_ATTRIBUTE} = "{TOGGLE_FORM_MARKER}"' in class_body
        )
        has_inline_marker = "_hp_inline_boundary = True" in class_body
        body = source_text[func.body_start:func.body_end]
        # Every generated communication path — inlined or lowered — reads the
        # instance-attribute channel (``self._hp_tp`` / ``self._hp_cp_mesh``
        # / ``self._hp_ep_group`` / ``self.<axis>_enable``) the runtime binds
        # at install time.  The retired module-level accessor must never come
        # back: a body still reading it was emitted by the accessor era, and the
        # class would raise NameError at train time.
        stray_accessor = re.search(r"get_parallel_state|get_inline_parallel_state", body)
        if stray_accessor is not None:
            raise RuntimeError(
                f"codegen preflight: class {class_name} still reads the removed "
                f"module-level accessor {stray_accessor.group(0)!r}; the parallel "
                "channel is bound on the instance at install time — regenerate "
                "the artifact"
            )
        has_inline_channel = any(
            token in body
            for token in (
                "self._hp_tp",
                "self._hp_cp_mesh",
        ...
        if has_inline_marker or (
            has_inline_channel and not (has_impl or has_marker or has_toggle_marker)
        ):
            # A fully inlined component body (the inline-marker shape): it
            # keeps no extracted ``_forward_impl`` and no form marker, and
            # must expose the generated communication path through the channel.
            if not has_inline_channel:
                raise RuntimeError(
                    f"codegen preflight: inline boundary class {class_name} "
                    "does not expose a generated communication path; "
                    "regenerate the artifact"
                )
            if has_marker:
                raise RuntimeError(
                    f"codegen preflight: clean inline boundary class "
                    f"{class_name} also carries the {TP_FORM_MARKER} marker; "
                    "regenerate the artifact"
                )
            continue
        ...
        if emitted == FORM_TP_COLLECTIVE:
            if not (has_marker or has_toggle_marker):
                raise RuntimeError(
