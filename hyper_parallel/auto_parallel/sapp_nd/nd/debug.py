...
def gen_colors(categories):
    ...
    try:
        ...
        raise ValueError(f"Unknown performance category: {exc.args[0]}") from exc
...
def get_comm_classified_data(csv_f, plot_idle=False):
    ...
    with open(csv_f, newline="", encoding="utf-8") as csv_file:
        ...
        for row in rows:
            ...
            try:
                comm_wait_time_classified["BUBBLE"] = comm_wait_time_classified[str(RealParts.PP_WAIT)]
            except KeyError as exc:
                raise ValueError("Classified communication data must include 'pp_wait'") from exc
            if plot_idle:
