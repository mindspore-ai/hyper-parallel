...
class Layer:
    ...
    def __init__(
        ...
        memory_activation_rec: Optional[Dict[Recompute.TYPE, float]] = None,
    ) -> None:
        """Build a :class:`Layer` record.

        Args:
            model_name (str): Name of the owning model (``"misc"`` for ad-hoc entries).
            name (str): Layer name.
            ...
            backward_coef_rec (Optional[Dict[Recompute.TYPE, float]], optional): Per-recomputation-type overhead
                coefficients (``None`` -> zeros). Default: ``None``.
        ...
        if "layers_description" in layer_data_json:
            for layer_data in layer_data_json["layers_description"]:
                new_layer = Layer(
                    ...
                    backward_time_rec={
                        r: layer_data.get(Recompute.JSON_TIME_NAME[r])
                        for r in Recompute.TYPE
                    },
                    backward_coef_rec={
                        r: layer_data.get(Recompute.JSON_COEF_NAME[r])
                        for r in Recompute.TYPE
                    },
                    memory_activation_rec={
                        r: layer_data.get(Recompute.JSON_MEMORY_NAME[r])
                        for r in Recompute.TYPE
                    },
