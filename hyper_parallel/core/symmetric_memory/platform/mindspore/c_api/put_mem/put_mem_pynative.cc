...
class PutMemRunner : public ms::pynative::PyboostRunner {
 public:
  using ms::pynative::PyboostRunner::PyboostRunner;

  virtual ~PutMemRunner() = default;
 ...
 protected :
  void LaunchKernel() override {
    ...
    ShmemKernel::aclshmem_put_mem(block_dim_, this->stream(), element_size_, target_ptr, target_offset_ptr, src_ptr,
...
void npu_put_mem(const ms::Tensor &target, const ms::Tensor &target_offset, const ms::Tensor &src,
                 const ms::Tensor &src_offset, const ms::Tensor &size, const int64_t target_pe) {
  auto op_name = "PutMem";
  auto runner = std::make_shared<ms_custom_ops::PutMemRunner>(op_name);
...
auto pyboost_put_mem(const ms::Tensor &target, const ms::Tensor &target_offset, const ms::Tensor &src,
                     const ms::Tensor &src_offset, const ms::Tensor &size, const int64_t target_pe) {
  return ms::pynative::PyboostRunner::Call<0>(ms_custom_ops::npu_put_mem, target, target_offset, src, src_offset, size,
                                              target_pe);  // 0 represent output num = 0
}

MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("put_mem", &pyboost_put_mem, "Put Mem", pybind11::arg("target"), pybind11::arg("target_offset"),
