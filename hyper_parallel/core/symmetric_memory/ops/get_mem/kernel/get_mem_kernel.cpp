...
class GetmemKernel {
 public:
  __aicore__ inline GetmemKernel() {}
  ...
  __aicore__ inline void Process() {
    ...
    auto ptr = aclshmem_ptr(src_ptr, target_pe_);
    ...
    for (uint64_t i = 0; i < repeat_times; i++) {
      ...
      aclshmemi_copy_ub2gm(target_ptr + i * repeat_elem, buf, block_size);
    ...
    if (remain > 0) {
      ...
      AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID);
      AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID);
      aclshmemi_copy_ub2gm(target_ptr + repeat_times * repeat_elem, buf, remain);
      AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID);
    }
    ...
    if (non_blocking_) {
      // shmem_getmem_nbi(target_ptr, src_ptr, size_per_core * sizeof(T), target_pe_);
      aclshmem_fence();
    } else {
      // shmem_getmem_nbi(target_ptr, src_ptr, size_per_core * sizeof(T), target_pe_);
      aclshmem_fence();
...
template <typename T>
__global__ __aicore__ void get_mem_kernel(GM_ADDR target, GM_ADDR target_offset, GM_ADDR src, GM_ADDR src_offset,
                                          GM_ADDR size, int64_t target_pe, bool non_blocking, GM_ADDR Syncmem) {
  GetmemKernel<T> op;
  op.Init(target, target_offset, src, src_offset, size, target_pe, non_blocking, Syncmem);
  op.Process();
}

void get_mem(uint32_t block_dim, void *stream, uint64_t elementSize, uint8_t *target, uint8_t *target_offset,
             uint8_t *src, uint8_t *src_offset, uint8_t *size, int64_t target_pe, bool non_blocking, uint8_t *Syncmem) {
  if (elementSize == 1) {
    get_mem_kernel<int8_t>
      <<<block_dim, nullptr, stream>>>(target, target_offset, src, src_offset, size, target_pe, non_blocking, Syncmem);
  } else if (elementSize == 2) {
    get_mem_kernel<int16_t>
      <<<block_dim, nullptr, stream>>>(target, target_offset, src, src_offset, size, target_pe, non_blocking, Syncmem);
  } else if (elementSize == 4) {
    get_mem_kernel<int32_t>
      <<<block_dim, nullptr, stream>>>(target, target_offset, src, src_offset, size, target_pe, non_blocking, Syncmem);
  } else if (elementSize == 8) {
    get_mem_kernel<int64_t>
      <<<block_dim, nullptr, stream>>>(target, target_offset, src, src_offset, size, target_pe, non_blocking, Syncmem);
  } else {
