...
from hyper_parallel.core.optimizer.sharding_category import build_optimizer_hsdp_assignment
...
class BaseDistributedOptimizer(torch.optim.Optimizer):
    ...
    def __init__(self, params: Any, defaults: Dict[str, Any], is_muon: bool) -> None:
        ...
        self.replicate_param_assignment: Dict[int, Any] = {}
        self._group_hsdp_by_mesh()

    def _group_hsdp_by_mesh(self) -> None:
        """Group parameters by HSDP communication domain and layout."""
        for group_key, group in enumerate(self.param_groups):
            self.replicate_param_assignment[group_key] = build_optimizer_hsdp_assignment(group["params"])
    ...
    def _collect_broadcast_tensors(
        ...
        # 1. Get valid_records
        for group_key, _ in enumerate(self.param_groups):
            group_assignment = self.replicate_param_assignment.get(group_key)
            if not group_assignment:
                continue

            for hsdp_info in group_assignment.hsdp:
                if not hsdp_info or not hsdp_info.is_replicated or not hsdp_info.replicate_pgs:
                    continue

                for record in hsdp_info.all_records:
                    src_coord = hsdp_info.owner_rank_coord(record)
                    if not src_coord or any(c < 0 for c in src_coord):
                        continue

                    valid_records.append((record.param, src_coord, hsdp_info.replicate_pgs))

        # 2. Process records.
        for p, src_coord, replicate_pgs in valid_records:
            if target == "param":
    ...
    def cleanup_synced_state(self) -> None:
        ...
        """
        params_to_remove = []
        for assignment in self.replicate_param_assignment.values():
            for hsdp_info in assignment.hsdp:
