...
# ============================================================================
"""torch dtensor base (independent module for Tier C rebinding)

``DTensorBase`` is subclassed by ``core.dtensor.dtensor.DTensor``, so its
``__mro__`` is frozen at the ``class DTensor(DTensorBase)`` definition line.
The MindSpore patch must therefore replace the ``DTensorBase`` name in this
module *before* ``core.dtensor.dtensor`` is imported — that is why this class
lives in its own module rather than behind a runtime ``platform`` binding.
"""
