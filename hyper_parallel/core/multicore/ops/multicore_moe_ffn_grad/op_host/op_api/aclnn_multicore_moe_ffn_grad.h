 ...
 */
#ifndef OP_API_INC_MULTICORE_MOE_FFN_GRAD_
#define OP_API_INC_MULTICORE_MOE_FFN_GRAD_
...
template <typename T>
struct MulticoreMoeFfnGradParamsBase {
    const T *hidden = nullptr;
    const T *w1 = nullptr;
    const T *permute_out = nullptr;
    const T *weight = nullptr; // w2
};
using MulticoreMoeFfnGradParams = MulticoreMoeFfnGradParamsBase<aclTensor>;
...
/**
 */
ACLNN_API aclnnStatus aclnnMulticoreMoeFfnGrad(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                         aclrtStream stream);
...
#endif  // OP_API_INC_MULTICORE_MOE_FFN_GRAD_
