...
int main() {
  ...
  std::vector<int64_t> xShape_1 = {8192, 4096};
  std::vector<int64_t> inputYShape_1 = {8192, 2048};
  std::vector<int64_t> outShape_1 = {8192, 2048};
  std::vector<int64_t> outputZShape_1 = {8192, 2048};


  void* xDeviceAddr_1 = nullptr;
  void *inputYDeviceAddr_1 = nullptr;
  void* outDeviceAddr_1 = nullptr;
  void *outputZDeviceAddr_1 = nullptr;

  aclTensor* x_1 = nullptr;
  aclTensor *inputY_1 = nullptr;
  aclTensor* out_1 = nullptr;
  aclTensor *outputZ_1 = nullptr;

  size_t inputYShapeSize_1 = inputYShape_1[0] * inputYShape_1[1];
  size_t outputZShapeSize_1 = outputZShape_1[0] * outputZShape_1[1];
  std::vector<aclFloat16> xHostData_1(xShape_1[0]*xShape_1[1], 1);
  std::vector<aclFloat16> inputYHostData_1(inputYShape[0] * inputYShape[1], 1);
  std::vector<aclFloat16> outHostData_1(outShape_1[0]*outShape_1[1], 1);
  std::vector<aclFloat16> outputZHostData_1(outputZShape_1[0] * outputZShape_1[1]);
  size_t dtypeSize_1 = sizeof(aclFloat16);

  int64_t first_shape_1 = 2048;
  int64_t second_shape_1 = 4096;
  int64_t split_value_1 = 128;
  int task_num_1 = 64;

  aclTensor *t1_1 = nullptr;
  void* t1DeviceAddr_1 = nullptr;
  TilingData tilingData_1;
  get_tiling_data_add(tilingData_1, split_value_1);
  uint8_t* t1_ptr_1 = reinterpret_cast<uint8_t*>(&tilingData_1);
  size_t t1_size_1 = sizeof(TilingData);
  std::cout<<"multicore_moe_ffn t1_size_1:" << t1_size_1<<std::endl;
  ret = CreateAclTensorToTilingData(t1_ptr_1, t1_size_1, &t1DeviceAddr_1, aclDataType::ACL_UINT8, &t1_1);

  aclTensor *t2_1 = nullptr;
  void* t2DeviceAddr_1 = nullptr;
  SwiGluTilingData swiGluTilingData_1;
  get_tiling_data_swiglu(swiGluTilingData_1, split_value_1);
  uint8_t* t2_ptr_1 = reinterpret_cast<uint8_t*>(&swiGluTilingData_1);
  size_t t2_size_1 = sizeof(SwiGluTilingData);
  std::cout<<"multicore_moe_ffn t2_size_1:" << t2_size_1<<std::endl;
  ret = CreateAclTensorToTilingData(t2_ptr_1, t2_size_1, &t2DeviceAddr_1, aclDataType::ACL_UINT8, &t2_1);

  //   std::vector<TaskDesc> all_tasks;
  RuntimeConfig runtimeConfig;
  init_runtime_config(runtimeConfig, NUM_WORKERS);

  std::vector<TaskDesc> all_tasks_0_1;
  for (int i = 0; i < task_num; i++) {
    ...
    //   runtimeConfig.all_tasks[(chu*2) * NUM_WORKERS +remain] = taskDesc;
    // }
    all_tasks_0_1.push_back(taskDesc);
  }

  std::vector<TaskDesc> all_tasks_0_2;
  for (int i = 0; i < task_num; i++) {
    TensorDesc inputs[MAX_INPUTS_PER_TASK];
    TensorDesc outputs[MAX_OUTPUTS_PER_TASK];
    ...
    taskDesc.trigger_event = task_num + task_num_1 + 1; //-1;
    ...
    // }
    all_tasks_0_2.push_back(taskDesc);
  }

  std::vector<TaskDesc> all_tasks_1_1;
  for (int i = 0; i < task_num_1; i++) {
    TensorDesc inputs[MAX_INPUTS_PER_TASK];
    TensorDesc outputs[MAX_OUTPUTS_PER_TASK];
    uint32_t input_len = 1;
    uint32_t output_len = 1;
    for (uint32_t j = 0; j < input_len; j++) {
        TensorDesc tensorDesc;
        tensorDesc.data_type = static_cast<uint32_t>(dtypeSize);
        tensorDesc.input_position = 6; //x
        tensorDesc.base_ptr_offset = i * second_shape_1 * split_value_1 * 1;
        inputs[j] = tensorDesc;
    }
    for (uint32_t j = 0; j < output_len; j++) {
        TensorDesc tensorDesc;
        tensorDesc.data_type = static_cast<uint32_t>(dtypeSize);
        tensorDesc.input_position = 8; // out
        tensorDesc.base_ptr_offset = i * first_shape_1 * split_value_1 * 1;
        outputs[j] = tensorDesc;
    }
    TaskDesc taskDesc = register_swi_glu_task(inputs, input_len, outputs, output_len);
    taskDesc.trigger_event = task_num + i + 1;
    taskDesc.dependent_event = -1 + 1;

    all_tasks_1_1.push_back(taskDesc);
  }

  std::vector<TaskDesc> all_tasks_1_2;
  for (int i = 0; i < task_num_1; i++) {
    TensorDesc inputs[MAX_INPUTS_PER_TASK];
    TensorDesc outputs[MAX_OUTPUTS_PER_TASK];
    uint32_t input_len = 2;
    ...
    for (uint32_t j = 0; j < input_len; j++) {
        ...
        tensorDesc.data_type = static_cast<uint32_t>(dtypeSize);
        if (j == 0) {
          tensorDesc.input_position = 8; //x
        } else {
          tensorDesc.input_position = 7; //x
        }
        tensorDesc.base_ptr_offset = i * first_shape_1 * split_value_1 * 1;
        inputs[j] = tensorDesc;
    }
    for (uint32_t j = 0; j < output_len; j++) {
        TensorDesc tensorDesc;
        tensorDesc.data_type = static_cast<uint32_t>(dtypeSize);
        tensorDesc.input_position = 11; // out
        tensorDesc.base_ptr_offset = i * first_shape_1 * split_value_1 * 1;
        outputs[j] = tensorDesc;
    }
    TaskDesc taskDesc = register_add_custom_task(inputs, input_len, outputs, output_len);
    taskDesc.trigger_event = task_num + task_num_1 + 1; //-1;
    taskDesc.dependent_event = task_num + i + 1;
    all_tasks_1_2.push_back(taskDesc);
  }

  int l0 = 0, l1 = 0, l2 = 0, l3 = 0;
  // for (int i = 0; i < task_num * 2 + task_num_1 * 2; i++) {
  //   int remain = i % 96;
  //   if (remain < 32) {
  //     runtimeConfig.all_tasks[i] = all_tasks_0_1[l0++];
  //   } else if (remain>=32 && remain<48) {
  //     runtimeConfig.all_tasks[i] = all_tasks_1_1[l2++];
  //   } else if (remain>=48 && remain<80) {
  //     runtimeConfig.all_tasks[i] = all_tasks_0_2[l1++];
  //   } else if (remain>=80 && remain<96) {
  //     runtimeConfig.all_tasks[i] = all_tasks_1_2[l3++];
  //   }
  // }
  ...
  runtimeConfig.all_tasks[task_num * 2 + task_num_1 * 2] = taskDesc;
...
//   std::vector<EventDesc> all_events;
  ...
  event_desc_0.num_triggers = task_num + task_num_1;
  event_desc_0.first_task_id = 0;
  event_desc_0.last_task_id = 0 + task_num + task_num_1;
  ...
  runtimeConfig.all_event_num_triggers[0] = 0; // task_num + task_num_1;

  for (int i = 1; i < task_num + task_num_1 + 1; i++) {
  ...
  event_desc_1.first_task_id = (task_num + task_num_1) * 2;
  event_desc_1.last_task_id = (task_num + task_num_1) * 2 + 1;
  runtimeConfig.all_events[(task_num + task_num_1)+1] = event_desc_1;
  runtimeConfig.all_event_num_triggers[(task_num + task_num_1) + 1] = (task_num + task_num_1);

  runtimeConfig.sched_queue[0] = 0;
  runtimeConfig.task_num = (task_num + task_num_1) * 2;
  ...
  int dimOptional_1 = -1;
  void ** inputX_1 = (void **)(&xHostData_1);
  void ** input2_1 = (void **)(&inputYHostData_1);

  INFO_LOG("all start 0");
  size_t fileSize_1;
  size_t xShapeSize_1 = GetShapeSize(xShape_1);
  ReadFile("/home/z00797459/workspace/cann-ops-adv/cann-ops/src/activation/mega_kernel/examples/AclNNInvocationNaive_multicore_swiglu_add_task/input/input_x_1.bin", fileSize_1, *inputX_1, xShapeSize_1 * dtypeSize_1);
  ReadFile("/home/z00797459/workspace/cann-ops-adv/cann-ops/src/activation/mega_kernel/examples/AclNNInvocationNaive_multicore_swiglu_add_task/input/input_y_1.bin", fileSize_1, *input2_1, inputYShapeSize_1 * dtypeSize_1);

  ret = CreateAclTensor(xHostData_1, xShape_1, &xDeviceAddr_1, aclDataType::ACL_FLOAT16, &x_1);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  INFO_LOG("swiglu start 0");
  ret = CreateAclTensor(inputYHostData_1, inputYShape_1, &inputYDeviceAddr_1, aclDataType::ACL_FLOAT16, &inputY_1);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  INFO_LOG("swiglu start 1");
  ret = CreateAclTensor(outHostData_1, outShape_1, &outDeviceAddr_1, aclDataType::ACL_FLOAT16, &out_1);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  INFO_LOG("swiglu start 2");

  std::cout<<"multicore_moe_ffn xDeviceAddr_1:" << xDeviceAddr_1<<std::endl;
  std::cout<<"multicore_moe_ffn inputYDeviceAddr_1:" << inputYDeviceAddr_1<<std::endl;
  std::cout<<"multicore_moe_ffn outDeviceAddr_1:" << outDeviceAddr_1<<std::endl;

  std::cout<<"multicore_moe_ffn xDeviceAddr_1 1:" << &xDeviceAddr_1<<std::endl;
  std::cout<<"multicore_moe_ffn inputYDeviceAddr_1 1:" << &inputYDeviceAddr_1<<std::endl;
  std::cout<<"multicore_moe_ffn outDeviceAddr_1 1:" << &outDeviceAddr_1<<std::endl;

  ret = CreateAclTensor(outputZHostData_1, outputZShape_1, &outputZDeviceAddr_1, aclDataType::ACL_FLOAT16, &outputZ_1);
  CHECK_RET(ret == ACL_SUCCESS, return FAILED);
  INFO_LOG("add start 0");
  ...
  ret = aclnnMulticoreMoeFfnGetWorkspaceSize(x, inputY, out, t1, t2, runtimeConfig_tensor,  x_1, inputY_1, out_1, t1_1, t2_1, outputZ, outputZ_1, &workspaceSize_1, &executor_1);
...
//   }
  ...
  auto size_1 = GetShapeSize(outputZShape_1);
  std::vector<aclFloat16> resultData_1(size_1, 0);
  ret = aclrtMemcpy(resultData_1.data(), resultData_1.size() * sizeof(resultData_1[0]), outputZDeviceAddr_1,
                      size_1 * sizeof(aclFloat16), ACL_MEMCPY_DEVICE_TO_HOST);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return FAILED);
  void ** output1_1=(void **)(&resultData_1);

  size_t dataType_1 = sizeof(uint16_t);
  WriteFile("../output/output_swiglu_addcustom_onetest_1.bin", *output1_1, outputZShapeSize_1 * dataType_1);
  INFO_LOG("Write output success");
