...
class GMMCompute {
 ...
 protected:
    ...
    GlobalTensor<DTYPE_WEIGHT> scaleGm;
#endif
    uint32_t ubBaseN;
    uint32_t ubBaseK;
    uint32_t ubCalSize;
...
__aicore__ inline void GMMCompute<mmType, sync>::SetGlobalBufferBias(uint32_t groupIdx,
    ...
    if (hasBias) {
        ...
        constexpr bool isBiasEpilogue_ =
            AscendC::IsSameType<DTYPE_WEIGHT, bfloat16_t>::value &&
            (AscendC::IsSameType<DTYPE_WEIGHT, bfloat16_t>::value || AscendC::IsSameType<DTYPE_WEIGHT, float>::value);
...
__aicore__ inline void GMMCompute<mmType, sync>::MMCompute(uint32_t groupIdx, MNConfig& mnConfig, uint32_t coreIdx) {
    ...
    // init global buffer
    if (singleX == 0) {
        xGm.SetGlobalBuffer((__gm__ AT*)xTensorPtr + groupIdx);
    } else {
...
#if defined(GMM_QUANT_INT8)
    if (singleWeight == 0) {
        scaleGm.SetGlobalBuffer(GetTensorAddr<DTYPE_WEIGHT>(groupIdx, scaleTensorPtr));
    } else {
        scaleGm.SetGlobalBuffer(GetTensorAddr<DTYPE_WEIGHT>(0, scaleTensorPtr) + mnConfig.nAxisBaseOffset);
    }
    mm.SetQuantVector(scaleGm[tailN]);
#endif
    SetGlobalBufferBias(groupIdx, tailN, mnConfig);
    if (singleY == 0) {
        yGm.SetGlobalBuffer((__gm__ CT*)yTensorPtr + groupIdx);
    } else {
