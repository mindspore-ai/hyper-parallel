...
class KernelWorker {
 public:
  ...
  __aicore__ inline void Init(uint32_t worker_id, __gm__ uint8_t *runtimeConfigPtr, GM_ADDR *input_list) {
    ...
    all_event_counters.SetGlobalBuffer((__gm__ int32_t *)(input_list[24]), MAX_EVENT_NUM);
    ...
    this->core_num = getExtraValueFromTiling(input_list[23], 5);
 ...
 private:
  ...
  __aicore__ inline void ExecuteSwiglu(const TaskDesc &task_desc) {
    if (task_desc.inputs[0].dynamic_shape == 1 || task_desc.outputs[0].dynamic_shape == 1) {
      ...
      if (dynamic_data.dynamic_type == DynamicType::DYNAMIC_DSV3_MOE_FFN) {
        ...
        int64_t ep = getExtraValueFromTiling(input_list[23], 1);
        int64_t expert_num = getExtraValueFromTiling(input_list[23], 2);
    ...
    if (task_desc.inputs[0].dynamic_shape == 1 || task_desc.inputs[1].dynamic_shape == 1) {
      ...
      if (dynamic_data.dynamic_type == DynamicType::DYNAMIC_DSV3_MOE_FFN) {
        ...
        int64_t output_0_offset = task_desc.outputs[0].dynamic_shape == 1
                                    ...
                                    : task_desc.outputs[0].base_ptr_offset * task_desc.outputs[0].data_type;
        grouped_matmul(input_list[task_desc.inputs[0].input_position] + input_0_offset,  // x
                       input_list[task_desc.inputs[1].input_position],                   // weight,
                       nullptr, nullptr, nullptr, nullptr, nullptr, grouped_list_real,
                       // grouped_list,
                       nullptr,
                       input_list[task_desc.outputs[0].input_position] + output_0_offset,  // x
                       input_list[11],                                                     // workspace,
                       tiling_data_addr);
      }
      return;
    }
  ...
  __aicore__ inline void ExecuteShmemPutMem(TaskDesc task_desc) {
    GM_ADDR signal = input_list[24];

    int64_t ep = getExtraValueFromTiling(input_list[23], 1);
    int64_t expert_num = getExtraValueFromTiling(input_list[23], 2);
    int64_t expert_num_single_rank = expert_num / ep;
    int64_t hidden_size = getExtraValueFromTiling(input_list[23], 3);

    int64_t rank_id = (*(__gm__ int64_t *)(input_list[23])) / ep * ep;
    ...
    send_data_size_ = static_cast<int64_t>(send_data_size_)
      ...
      put_mem_signal_kernel(input_list[task_desc.outputs[0].input_position] +
                            ...
                            input_list[task_desc.inputs[1].input_position] +
                              task_desc.inputs[1].base_ptr_offset * task_desc.inputs[1].data_type,  // x
                            src_offset_,
