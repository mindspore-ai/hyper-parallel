...
namespace l0op {
OP_TYPE_REGISTER(MegaMoeGrad);

const std::array<const aclTensor *, 7> MegaMoeGrad(
  ...
  L0_DFX(MegaMoeGrad, dispatch_target, dispatch_target_off, dy, dispatch_src_off, dispatch_size, hidden,
  ...
  if (ret != ACL_SUCCESS) {
    OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "MegaMoeGrad ADD_TO_LAUNCHER_LIST_AICORE failed.");
