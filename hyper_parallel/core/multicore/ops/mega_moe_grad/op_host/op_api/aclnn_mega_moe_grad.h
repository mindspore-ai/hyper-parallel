 ...
 */
#ifndef OP_API_INC_MEGA_MOE_GRAD_
#define OP_API_INC_MEGA_MOE_GRAD_
...
template <typename T>
struct MegaMoeGradParamsBase {
  const T *hidden = nullptr;
  const T *w1 = nullptr;
  const T *permute_out = nullptr;
  const T *weight = nullptr;  // w2
};
using MegaMoeGradParams = MegaMoeGradParamsBase<aclTensor>;
...
/**
 */
ACLNN_API aclnnStatus aclnnMegaMoeGrad(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                               aclrtStream stream);
...
#endif  // OP_API_INC_MEGA_MOE_GRAD_
