...
void Failure(const FailureContext &context, const Status &error) noexcept {
  ...
  if (error.cann_error_code.has_value()) {
    Line(Level::Error, context.root_rank, context.file, context.line, "op=%s phase=%s err=%s cann=%d msg=\"%.*s\"",
         OperationName(context.operation), PhaseName(context.phase), ErrorName(error.error_code),
         *error.cann_error_code, message_length, error.message.data());
    return;
  }
  Line(Level::Error, context.root_rank, context.file, context.line,
       "op=%s phase=%s err=%s cann=None msg=\"%.*s\"", OperationName(context.operation), PhaseName(context.phase),
