...
class PutMemKernel {
 public:
  __aicore__ inline PutMemKernel() {}
  ...
  __aicore__ inline void Process() {
    ...
    if (non_blocking_) {
      // shmem_putmem_nbi(target_ptr, src_ptr, size_per_core * sizeof(T), target_pe_);
      aclshmem_fence();
    } else {
      // shmem_putmem_nbi(target_ptr, src_ptr, size_per_core * sizeof(T), target_pe_);
      aclshmem_fence();
...
template <typename T>
__global__ __aicore__ void put_mem_kernel(GM_ADDR target, GM_ADDR target_offset, GM_ADDR src, GM_ADDR src_offset,
