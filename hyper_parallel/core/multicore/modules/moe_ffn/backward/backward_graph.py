...
_TILING_POS_W2_GRAD     = 22
_TILING_POS_W1_GRAD     = 23
...
def _build_bwd_tensor_specs(tsv, hidden_size, intermediate_size, dtype_size):
    ...
    w2_grad_x1 = TensorSpec(
        "w2_grad_x1", [tsv.per_rank_seq, hidden_size],
        param_position=17, dtype_size=dtype_size,
        tensor_type=1, is_dynamic=True, transpose=True)
    w2_grad_y = TensorSpec(
        "w2_grad_y", [sre, hidden_size, intermediate_size * 2],
    ...
    return (target, target_offset, src, src_offset, size_d,
            w1_grad_x1, w1_grad_y, act_grad_weight, act_grad_y,
            swiglu_dy, swiglu_out, gate_grad_weight, gate_grad_y,
            combine_out, target_offset_c, src_offset_c, size_c,
            w2_grad_x1, w2_grad_y, glist)


def _build_bwd_ops_first(tsv, specs, *, dispatch_sv, act_grad_sv, w1_grad_sv,
                         swiglu_sv, num_cube_cores):
    """Create dispatch, act_grad, w1_grad, swiglu_grad operator nodes."""
    ...
    w1_grad = OperatorNode(
        ...
        inputs=[w1_grad_x1, target, glist], outputs=[w1_grad_y],
        param_positions=[5, 0, 19, 6], split_value=w1_grad_sv,
        split_spec=SplitSpec(
    ...
    return dispatch, act_grad, w1_grad, swiglu_grad


def _build_bwd_ops_second(specs, *, gate_grad_sv, w2_grad_sv, combine_sv, num_cube_cores):
    """Create gate_grad, combine, w2_grad operator nodes."""
    (*_, swiglu_out, gate_grad_weight, gate_grad_y,
     combine_out, target_offset_c, src_offset_c, size_c,
     w2_grad_x1, w2_grad_y, glist) = specs
    ...
    w2_grad = OperatorNode(
        ...
        inputs=[w2_grad_x1, swiglu_out, glist], outputs=[w2_grad_y],
        param_positions=[17, 10, 19, 18], split_value=w2_grad_sv,
        split_spec=SplitSpec(
            split_inputs=[(1, 0)], split_output_dims=[0],
            task_num_fn=lambda tsv, _ncc=num_cube_cores: _ncc * tsv.single_rank_expert_num,
        ),
    ...
    return gate_grad, combine, w2_grad
...
def build_backward_graph(tsv, *,
    ...
    Execution order:
      dispatch -> {act_grad, w1_grad} -> swiglu_grad -> gate_grad -> {combine, w2_grad}
    act_grad and w1_grad run in parallel (both depend on dispatch).
    combine and w2_grad run in parallel (both depend on gate_grad).
    ...
    Param positions (C++ memory slots):
      ...
      w1_grad:      [x1=5, x2=0, glist=19 | y=6]
      ...
      w2_grad:      [x1=17, x2=10, glist=19 | y=18]

    Args:
        tsv: TaskSplitValue carrying TP/EP/seq partition metadata.
        dispatch_sv: tile size for dispatch AllToAll.
        act_grad_sv: tile size for activation gradient GMM.
        w1_grad_sv: tile size for w1 weight gradient GMM.
        swiglu_sv: tile size for SwiGLU-grad.
        gate_grad_sv: tile size for gate gradient GMM.
        w2_grad_sv: tile size for w2 weight gradient GMM.
        combine_sv: tile size for combine AllToAll.
    ...
    dispatch, act_grad, w1_grad, swiglu_grad = _build_bwd_ops_first(
        tsv, specs,
        dispatch_sv=dispatch_sv, act_grad_sv=act_grad_sv,
        w1_grad_sv=w1_grad_sv, swiglu_sv=swiglu_sv, num_cube_cores=num_cube_cores,
    )
    gate_grad, combine, w2_grad = _build_bwd_ops_second(
        specs,
        gate_grad_sv=gate_grad_sv, w2_grad_sv=w2_grad_sv,
        combine_sv=combine_sv, num_cube_cores=num_cube_cores,
    )
    graph = ComputeGraph()
    (graph.add_op(dispatch).add_op(act_grad).add_op(w1_grad)
          .add_op(swiglu_grad).add_op(gate_grad).add_op(combine).add_op(w2_grad)
          .add_edge(dispatch,    act_grad)
          .add_edge(dispatch,    w1_grad)
          .add_edge(act_grad,    swiglu_grad)
          .add_edge(swiglu_grad, gate_grad)
          .add_edge(gate_grad,   combine)
          .add_edge(gate_grad,   w2_grad))
    return graph
