...
// moe_ffn_fwd has 5 in-place outputs:
//   dispatch_target, up_proj_y, swiglu_out, down_proj_y, combine_target
static const mindspore::ShapeArray kFwdFakeOutShapes{
    mindspore::ShapeVector{1}, mindspore::ShapeVector{1},
    mindspore::ShapeVector{1}, mindspore::ShapeVector{1},
    mindspore::ShapeVector{1}};
static const std::vector<mindspore::TypeId> kFwdFakeOutTypes{
...
class OPS_API CustomMoeFfnFwdOpFuncImpl : public OpFuncImpl {
 public:
  ShapeArray InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    return kFwdFakeOutShapes;
  }

  std::vector<TypeId> InferType(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    return kFwdFakeOutTypes;
  }

  bool GeneralInferRegistered() const override { return true; }
};

class CustomMoeFfnFwd : public KernelMod {
 public:
  CustomMoeFfnFwd() = default;
  ~CustomMoeFfnFwd() = default;
  using KernelMod::Init;

  bool Init(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) override {
    MS_LOG(WARNING) << "CustomMoeFfnFwd graph-mode stub: input=" << inputs.size()
                    << " output=" << outputs.size();
    return true;
  }
  ...
  bool Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
    ...
    MS_LOG(WARNING) << "CustomMoeFfnFwd graph-mode stub launch: input=" << inputs.size()
                    << " output=" << outputs.size();
    return true;
  }
...
REG_GRAPH_MODE_OP(moe_ffn_fwd, ms_multicore::CustomMoeFfnFwdOpFuncImpl, ms_multicore::CustomMoeFfnFwd);
