...
// moe_bwd has 7 declared returns (following mega_moe_grad_op.yaml):
...
class OPS_API CustomMoeBwdOpFuncImpl : public OpFuncImpl {
 public:
...
class CustomMoeBwd : public KernelMod {
 public:
  CustomMoeBwd() = default;
  ~CustomMoeBwd() = default;
  using KernelMod::Init;

  bool Init(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) override {
    MS_LOG(WARNING) << "CustomMoeBwd graph-mode stub: input=" << inputs.size()
                    << " output=" << outputs.size();
    return true;
  }
  ...
  bool Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
    ...
    MS_LOG(WARNING) << "CustomMoeBwd graph-mode stub launch: input=" << inputs.size()
                    << " output=" << outputs.size();
    return true;
  }
...
REG_GRAPH_MODE_OP(mega_moe_grad, ms_multicore::CustomMoeBwdOpFuncImpl, ms_multicore::CustomMoeBwd);
