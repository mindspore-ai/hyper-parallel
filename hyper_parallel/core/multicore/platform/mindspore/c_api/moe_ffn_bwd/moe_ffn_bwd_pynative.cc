...
// =============================================================================
// PYBOOST MODE IMPLEMENTATION — moe_ffn_bwd
//
...
// framework.  The function name "aclnnMulticoreMoeFfnGrad" is resolved at
...
// npu_moe_ffn_bwd — returns the 7 inplace output tensors declared in YAML
std::vector<ms::Tensor> npu_moe_ffn_bwd(
  ...
  auto runner = std::make_shared<ms::pynative::AclnnOpRunner>("MoeFfnBwd");
  MS_EXCEPTION_IF_NULL(runner);
  runner->SetLaunchFunc(LAUNCH_ACLNN_FUNC(aclnnMulticoreMoeFfnGrad,
  ...
  // outputs = the 7 inplace tensors declared as returns in moe_ffn_bwd_op.yaml
  runner->Run(inputs, {});
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("moe_ffn_bwd", PYBOOST_CALLER(7, ms_multicore::npu_moe_ffn_bwd));
}
