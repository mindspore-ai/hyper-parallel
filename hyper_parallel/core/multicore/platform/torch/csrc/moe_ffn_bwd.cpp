 ...
 *
 * PyTorch TORCH_LIBRARY_IMPL for hyper_parallel::moe_ffn_bwd.
 *
 * NPU implementation calls aclnnMulticoreMoeFfnGrad via EXEC_NPU_CMD.
...
BwdReturn moe_ffn_bwd_npu(
    ...
    int64_t hidden_size,
    int64_t seq_size) {
    EXEC_NPU_CMD_EXT(aclnnMulticoreMoeFfnGrad,
...
TORCH_LIBRARY_IMPL(hyper_parallel, PrivateUse1, m) {
    m.impl("moe_ffn_bwd", &moe_ffn_bwd_npu);
}

// Meta device
TORCH_LIBRARY_IMPL(hyper_parallel, Meta, m) {
    m.impl("moe_ffn_bwd", &moe_ffn_bwd_meta);
}
