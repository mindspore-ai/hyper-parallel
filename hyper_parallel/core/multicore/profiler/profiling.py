...
@dataclass(frozen=True)
class _ProfileCapacities:
    """Optional Host-computed AIC/AIV record capacities."""
...
class _PreparedMegaKernelRuntime:
    ...
    def parse(self, buffer: Any, *, detailed_task_names: bool) -> dict[str, Any]:
        ...
        return _parse_cycle_buffer(
            ...
            _ProfileCapacities(
                aic=self._layout.aic_record_capacity,
                aiv=self._layout.aiv_record_capacity,
            ),
        )
...
def _parse_cycle_buffer(
    buffer: Any,
    config: _CycleTraceConfig | None = None,
    capacities: _ProfileCapacities | None = None,
    **legacy_options: Any,
) -> dict[str, Any]:
    """Decode a device cycle buffer with already resolved hardware and display metadata."""
    config, capacities = _resolve_cycle_parse_options(config, capacities, legacy_options)
    ...
    capacities = capacities or _ProfileCapacities()
...
def _resolve_cycle_parse_options(
    config: _CycleTraceConfig | None,
    capacities: _ProfileCapacities | None,
    legacy_options: dict[str, Any],
) -> tuple[_CycleTraceConfig, _ProfileCapacities]:
    """Support the legacy keyword-only decoder call used by internal test utilities."""
    if config is not None:
        if legacy_options:
            raise TypeError("legacy cycle-buffer options cannot be combined with config")
        return config, capacities or _ProfileCapacities()
    required = {
        "rank",
        "device_id",
        "cycle_frequency_mhz",
    ...
    missing = sorted(required.difference(legacy_options))
    if missing:
        raise TypeError(f"missing required cycle-buffer options: {', '.join(missing)}")
    legacy_options = dict(legacy_options)
    resolved_capacities = _ProfileCapacities(
        aic=legacy_options.pop("aic_record_capacity", None),
        aiv=legacy_options.pop("aiv_record_capacity", None),
    )
    resolved_config = _CycleTraceConfig(
        rank=legacy_options.pop("rank"),
        device_id=legacy_options.pop("device_id"),
        cycle_frequency_mhz=legacy_options.pop("cycle_frequency_mhz"),
        detailed_task_names=legacy_options.pop("detailed_task_names"),
        kernel_name=legacy_options.pop("kernel_name"),
        owner_label=legacy_options.pop("owner_label"),
        stage_names=legacy_options.pop("stage_names"),
        soc_name=legacy_options.pop("soc_name"),
        task_stage_names=legacy_options.pop("task_stage_names", None),
    )
    if legacy_options:
        raise TypeError(f"unexpected cycle-buffer options: {', '.join(sorted(legacy_options))}")
    return resolved_config, resolved_capacities
