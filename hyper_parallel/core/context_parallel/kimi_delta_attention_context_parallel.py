...
# This Torch implementation lives in core to match the existing context-
# parallel public API while all accelerator kernels remain platform-local.
...
class KimiDeltaAttentionContextParallel(ParallelStyle):
    """Apply a Kimi K3 context-parallel executor to one attention layer."""
    ...
    def __init__(
        ...
        *,
        ...
        """Configure the KDA context-parallel mode."""
        if mode not in ("ulysses", "p2p"):
            raise ValueError(
                "KimiDeltaAttentionContextParallel mode must be 'ulysses' "
                f"or 'p2p', got {mode!r}."
            )
        if chunk_size <= 0:
            raise ValueError(f"chunk_size must be positive, got {chunk_size}.")
    ...
    def apply(self, module: nn.Module, device_mesh: DeviceMesh) -> nn.Module:
        """Patch ``module.forward`` while retaining its parameters and state dict."""
        existing_executor = getattr(module, "_hp_kda_cp_executor", None)
        if existing_executor is not None:
            raise RuntimeError(
                "KimiDeltaAttentionContextParallel has already been applied to "
                f"this module with {type(existing_executor).__name__}; applying "
                "a CP wrapper twice is not supported."
            )
        executor_class = {
            "ulysses": KimiDeltaAttentionLayerUlyssesCP,
            "p2p": KimiDeltaAttentionLayerP2PCP,
        }[self.mode]
        ...
        object.__setattr__(module, "_hp_kda_cp_executor", executor)
        object.__setattr__(module, "_hp_kda_original_forward", module.forward)

        def _forward(*args: Any, **kwargs: Any):
