...
# ============================================================================
"""HSDP cell state"""
from hyper_parallel.core.hsdp_refactor.hsdp_param_buffer import HSDPParamBuffer
from hyper_parallel.core.hsdp_refactor.hsdp_grad_buffer import HSDPGradBuffer
...
class HSDPState:
    ...
    def _init_param_buffers(self):
        ...
        for hsdp_param in self.sharded_hsdp_params:
            param_buffer_key = hsdp_param.sharded_group_info.group_name + str(hsdp_param.param.dtype)
            if param_buffer_key not in group_to_buffer:
                buffer = HSDPParamBuffer(self.config, hsdp_param, self.platform)
    ...
    def _init_grad_buffers(self):
        ...
        def get_bucket_key(buffer_key, hsdp_param):
            if self.config.bucket_size < 0:
                return buffer_key
            param_size = hsdp_param.param.numel() * self.platform.get_param_type_size(hsdp_param.param)
        ...
        for hsdp_param in self.hsdp_params:
            if not hsdp_param.param.requires_grad:
                continue
            buffer_key = hsdp_param.sharded_group_info.group_name + hsdp_param.unsharded_group_info.group_name \
                         + str(hsdp_param.param.dtype)
            bucket_key = get_bucket_key(buffer_key, hsdp_param)
            if bucket_key not in group_to_buffer:
                buffer = HSDPGradBuffer(self.config, hsdp_param, self.platform)
