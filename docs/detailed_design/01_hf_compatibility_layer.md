...
| `components/distributed/config.py` | `DistributedSetup` — 分布式拓扑容器 → 06_distributed_infrastructure.md §3 |
| `components/distributed/mesh.py` | `MeshContext` / `ParallelismSizes` — DeviceMesh 构建 → 06_distributed_infrastructure.md §3 |
| `components/distributed/fsdp2.py` | `FSDP2Manager` — DP 维度包裹 → 06_distributed_infrastructure.md §4 |
| `components/distributed/dtensor_utils.py` | `_local_params_context` — 生产模式零拷贝 → 06_distributed_infrastructure.md §5 |
| `components/distributed/sharding_planner.py` | **ShardingPlanner** — hyper_parallel 核心差异化能力（AutoModel 无此层） |
| `components/distributed/sharding_applier.py` | ShardingApplier — 将 ShardingPlan 应用到模型 |
| `components/distributed/sharding_config.py` | ShardingConfig / ShardingPlan / ModuleShardingSpec 数据模型 |
| `components/checkpoint/conversion_mapping.py` | HF checkpoint key ↔ 模型 FQN 双向映射 |
| `components/models/common/state_dict_adapter.py` | `StateDictAdapter` 基类 |
| `components/models/common/hf_checkpointing_mixin.py` | `HFCheckpointingMixin` |
...
**设计原则**：`components/distributed/` 下的所有模块**零依赖**于 `recipes/`、`_transformers/`、`components/models/`。它们只依赖 `torch` + `DTensor` + 自身的数据结构（`ShardingPlan`、`ModuleShardingSpec`）。这是与 AutoModel 的关键架构差异——AutoModel 的 `parallelize_fn` 深度嵌入每个模型实现，Hyper-Parallel 的并行组件是**纯外部注入**，完全取代了模型内嵌的 `parallelize_fn`。

### 1.2 迁移路径：`components/` 与现有 `core/` 代码的关系

01 引入的 `components/` 目录（`components/distributed/`、`components/checkpoint/`、`components/models/` 等）与现有 `core/dtensor`、`core/shard`、`dmodule` 并存并逐步收敛，关系如下（详见总计划附录 A.2）：

| 现有代码 | 01 引入的 `components/` 对应物 | 关系 |
|---------|------------------------------|------|
| `core/dtensor/` — DTensor 工具与 placement 推导 | `components/distributed/dtensor_utils.py`（`_local_params_context` 等） | **封装复用**：`components/` 调用 `core/dtensor` 的底层 DTensor API，对外提供面向 ShardingPlan 的语义入口；`core/dtensor` 保持作为底层库不动。 |
| `core/shard/` — 分片应用器与 param 管线 | `components/distributed/sharding_applier.py` + `sharding_planner.py` + `sharding_config.py` | **复用**：底层算子保留为 DTensor dispatch 后端（被 `DTensor.redistribute()` / `distribute_tensor()` 等内部复用，`core/shard/` 不下线）；旧的高层 shard plan 编排 API 被 `ShardingPlanner`/`ShardingApplier` 替代，用户侧只能通过 `components/distributed/` 入口使用（与 06 §1 一致） |
| `dmodule/` — 模型动态包装与 spec 注册 | `_transformers/registry.py` + `components/models/common/` | **替代**：`MODEL_ARCH_MAPPING` + `HyperAutoModel` 取代 `dmodule` 的 `ModelSpec`/`register_spec` 机制（见 §1 "涉及删除的旧代码"）。迁移期 `dmodule` 旧模型按 §12 流程逐个迁入 `components/models/`，迁完即下线 `dmodule` 对应条目。 |

**迁移原则**：
- `core/dtensor` 作为底层 DTensor 能力保留，`components/distributed/` 在其上构建 ShardingPlan 驱动的声明式分片层。
- `core/shard` 属于**复用层**（底层算子保留为 DTensor dispatch 后端，不下线）；`dmodule` 属于**被替代层**。迁移期允许并存，但**新代码必须只走 `components/` 入口**，禁止新增对 `core/shard` 高层编排 API 与 `dmodule` 的直接调用。
...
② _resolve_target("hyper_parallel.HyperAutoModelForCausalLM.from_pretrained")
...
#      a) _wrap("_target_", "hyper_parallel.HyperAutoModelForCausalLM.from_pretrained")
#         k == "_target_" → _resolve_target("hyper_parallel.HyperAutoModelForCausalLM.from_pretrained")
#           parts = ["hyper_parallel", "HyperAutoModelForCausalLM", "from_pretrained"]
#           i=1: importlib.import_module("hyper_parallel") ✓
#                getattr(mod, "HyperAutoModelForCausalLM") → <class>
#                getattr(class, "from_pretrained") → <bound method>
#         保存: self._original_strings["_target_"] = "hyper_parallel.HyperAutoModelForCausalLM.from_pretrained"
...
dataloader:
  ...
  batch_size: 1
  collate_fn: hyper_parallel.components.datasets.utils.default_collater
```

解析过程：

```python
# _wrap("collate_fn", "hyper_parallel.components.datasets.utils.default_collater")
# k == "collate_fn" → k.endswith("_fn") is True!
# → _resolve_target("hyper_parallel.components.datasets.utils.default_collater")
#   → importlib.import_module("hyper_parallel.components.datasets.utils")
#   → getattr(module, "default_collater")
#   → <function default_collater>
# self._original_strings["collate_fn"] = "hyper_parallel.components.datasets.utils.default_collater"
...
training:
  _target_: hyper_parallel.TrainingLoop
  optimizer:
...
# components/models/common/param_utils.py
...
MODEL_ARCH_MAPPING = OrderedDict([
    # ── Dense 模型 ──
    ("LlamaForCausalLM", (
        "hyper_parallel.components.models.llama.model",
        "LlamaForCausalLM",
    )),
    ("Qwen3_5ForCausalLM", (
        "hyper_parallel.components.models.qwen3_5.model",
        "Qwen3_5ForCausalLM",
    )),
    # ── MoE 模型 ──
    ("Qwen3_5MoeForConditionalGeneration", (
        "hyper_parallel.components.models.qwen3_5_moe.model",
        "Qwen3_5MoeForConditionalGeneration",
    )),
    # ── VLM 模型 ──
    ...
    ("Qwen3VLMoeForConditionalGeneration", (
        "hyper_parallel.components.models.qwen3_vl_moe.model",
        "Qwen3VLMoeForConditionalGeneration",
    )),
])
...
# → importlib.import_module("hyper_parallel.components.models.qwen3_5.model")
# → 返回 Qwen3_5ForCausalLM 类

# from_pretrained("meta-llama/Llama-3.2-1B")
# → MODEL_ARCH_MAPPING["LlamaForCausalLM"]
# → importlib.import_module("hyper_parallel.components.models.llama.model")
...
from hyper_parallel.components.distributed.init_utils import get_world_size_safe
from hyper_parallel.components.utils.model_utils import init_empty_weights
from hyper_parallel.shared.utils import dtype_from_str
...
def from_pretrained(
    ...
    **kwargs,
    ...
    # ② 实例化基础设施
    from hyper_parallel._transformers.infrastructure import instantiate_infrastructure
    ...
    from hyper_parallel._transformers.registry import get_hf_config, get_is_hf_model
...
def _build_model(
    ...
    **kwargs,
    ...
    from hyper_parallel._transformers.model_init import _init_model
...
```python
# components/training/quantization.py
...
def _init_model(
    ...
    **kwargs,
    ...
    if is_hf_model:
        ...
        from hyper_parallel.components.models.common.hf_checkpointing_mixin import HFCheckpointingMixin
    ...
    from hyper_parallel._transformers.registry import _resolve_custom_model_cls
...
def instantiate_infrastructure(
    *,
    ...
    from hyper_parallel.components.distributed.fsdp2 import FSDP2Manager
    # 辅助工厂定义在 components/distributed/fsdp2.py 与 pipelining.py（§2.14）
    from hyper_parallel.components.distributed.fsdp2 import _instantiate_fsdp2
    from hyper_parallel.components.distributed.pipelining import _instantiate_pipeline
...
def apply_model_infrastructure(
    ...
    **kwargs,
    ...
    """
    # 本函数位于 hyper_parallel._transformers.infrastructure；下列 _apply_* 跨模块，
    # 需 local-import（第六轮 P1 修复：补全调用点 import，避免 NameError）。
    from hyper_parallel.components._peft.lora import _apply_peft
    ...
    if not is_hf_model and plan is not None:
        from hyper_parallel.components.distributed.sharding_applier import apply_sharding_plan
    ...
    if is_meta_device and load_base_model:
        ...
        from hyper_parallel.components.checkpoint.loading import load_base_model
        from hyper_parallel.components.checkpoint.checkpointing import _get_state_dict_adapter
    ...
    if peft_config is not None:
        from hyper_parallel.components._peft.lora import _freeze_non_lora_params
...
```python
# components/models/common/state_dict_adapter.py
...
```python
# components/models/qwen3_5/state_dict_adapter.py
...
`load_base_model` 的 canonical 实现位于 **04_checkpoint.md §5.3**（`components/checkpoint/loading.py`），
...
# （components/checkpoint/loading.py），01 不再重复定义，统一 import 引用：
#   from hyper_parallel.components.checkpoint.loading import _load_hf_checkpoint_preserving_dtype
# 签名（见 04 §5.3）：
...
# components/distributed/sharding_planner.py
...
# components/models/qwen3_5/model.py
