...
| 文件 | 职责 |
|------|------|
| `components/checkpoint/checkpointing.py` | `Checkpointer` 核心类 (new file, to be created) |
| `components/checkpoint/config.py` | `CheckpointingConfig` (new file, to be created) |
| `components/checkpoint/stateful_wrappers.py` | `ModelState` / `OptimizerState` wrapper (new file, to be created) |
| `components/checkpoint/conversion_mapping.py` | HF key ↔ 模型 FQN 映射 + `WeightConverter` (new file, to be created) |
| `components/checkpoint/_backports/hf_storage.py` | `_HuggingFaceStorageWriter` / `_HuggingFaceStorageReader` (new file, to be created) |
| `components/checkpoint/addons.py` | `ConsolidatedHFAddon` / `PeftAddon` (new file, to be created) |
...
| `hyper_parallel/trainer/callbacks/base.py` — `CheckpointCallback` | 移到 `components/checkpoint/checkpointing.py` |
| `hyper_parallel/trainer/callbacks/base.py` — `SafetensorsExportCallback` | 移到 `Checkpointer.save_model()` 内 |
| `hyper_parallel/core/distributed_checkpoint/offline_transform.py` | 移到 `components/checkpoint/_backports/consolidate_hf_safetensors.py` |
...
```python
# components/checkpoint/config.py
...
```python
# components/checkpoint/loading.py
...
```python
# components/checkpoint/stateful_wrappers.py
...
def _write_consolidate_script(weights_path: str) -> None:
    ...
    with open(script, "w") as f:
        ...
        f.write(f"python -m hyper_parallel.components.checkpoint.consolidate \\\n")
...
def _convert_checkpoint_with_transformers(model_path: str, config) -> dict:
    ...
    """
    from hyper_parallel.components.checkpoint.conversion_mapping import WeightConverter
