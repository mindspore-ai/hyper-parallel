# Claude Code Workflow In HyperParallel

This document summarizes the end-to-end development capabilities that
Claude Code provides in the HyperParallel repository, based on
`CLAUDE.md` and the `.claude/` configuration directory.

## Overview

Claude Code in this repository is not only a code generator. It is
...
For a normal repository developer, Claude Code capabilities can be
...
At the development stage, Claude Code can help in three ways.

### 1.1 Load Project Context Automatically

Before implementation, Claude Code can use:
...
When Claude Code writes or edits files, repository hooks are triggered
...
This means Claude Code can reduce missed follow-up work during coding,
not only after coding.

## 2. Code Checking

At the checking stage, Claude Code mainly helps with validation before
commit.
...
At the submission stage, Claude Code can help developers complete a
...
Claude Code submission workflow also includes:
...
At the review stage, Claude Code can be used as a repository-aware code
review assistant.
...
This repository does not treat review as style-only checking. Claude
...
For most team members, the most practical Claude Code entry points are:
...
1. Use Claude Code to implement or modify code with repository rules
   loaded.
...
flowchart LR
    ...
    A1["Project context<br/>CLAUDE.md + rules"] --> A
...
In this repository, Claude Code acts as a practical repository workflow
