...
>
> **维护规则:** 凡是新增/修改配置键、入口模块、指标或测试的改动,必须在**同一个
> diff** 内更新此处对应行,否则评审不通过。本文件由 `AGENTS.md` 与
> `docs/rl-architecture.md` 链接;**不在别处复制**。
>
...
| 优势估计:GRPO | `algorithm.name=grpo` → advantage | `rl/algorithm/advantage.py` | `rl/algorithm/advantage.py`(`@register_advantage_estimator("grpo")`) | `train/advantage` 分布 | `rl_tests/test_algorithm_registry.py` |
| 优势估计:GAE | `algorithm.*` → `@register_advantage_estimator("gae")` | `rl/algorithm/advantage.py` | `rl/algorithm/advantage.py` | `train/advantage` 分布 | `rl_tests/test_algorithm_registry.py` |
| 奖励:GSM8K | `agentic.environment=gsm8k` | `rl/algorithm/reward.py` | `rl/algorithm/reward.py`(`@register_reward("gsm8k")`) | `reward/mean`, `reward/accuracy`, `reward/min`, `reward/max` | `rl_tests/test_algorithm_registry.py` |
...
| rollout 拓扑 | `rollout.vllm.deployment` | `rl/config.py`(`_validate_vllm_deployment`) | `rl/roles/rollout/topology.py` | — | `rl_tests/test_rollout_topology.py`, `test_disjoint_topology.py` |
...
| 传输 IPC/HCCL | `deployment`(colocated→IPC,disjoint→HCCL) | `rl/roles/weight_sync/transfer.py` | `rl/roles/weight_sync/hccl.py`, `transfer.py` | — | `rl_tests/test_hccl_weight_sync.py` |
...
| Agentic 环境 | `agentic.environment=gsm8k`, `agentic.max_turns` | `rl/agentic/registry.py` | `rl/agentic/gsm8k.py`, `rl/agentic/session.py` | 每轮 reward | `rl_tests/test_agentic_runner.py` |
...
3. 如果你**加了指标**,补进 `指标` 列。
4. 如果你**加了测试**,补进 `测试` 列(测试文件必须真实存在)。
5. 改动是否涉及**已按设计移除**的行?复查 `config.py` 是否仍然拒绝它,并在 diff 中说明。
6. 新子系统一行写不下?加一行**并**在 `docs/rl-architecture.md` 补一条说明,否则本图会悄悄过期。
