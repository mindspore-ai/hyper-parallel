...
不证明训练已执行。`hyper_parallel/rl/tests/st/test_rl_st.py::test_rl_system` 才是实际 NPU 入口，
`hyper_parallel/rl/tests/st/st_runtime.py` 负责运行和恢复阶段，`hyper_parallel/rl/tests/st/st_evidence.py` 检查结果。
