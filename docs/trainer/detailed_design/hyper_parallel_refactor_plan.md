...
一个关键设计决策：**`components/distributed/` 是独立可用的，不依赖 Hyper-Parallel 的训练流程**。
...
│    可选——用户可以用任意训练框架替换                         │
├─────────────────────────────────────────────────────────┤
│  Layer 2: 模型层 (components/models/)                    │
│    LlamaForCausalLM、Qwen3_5ForCausalLM                 │
│    零 DTensor 代码——分片策略外部注入                       │
├─────────────────────────────────────────────────────────┤
│  Layer 1: 并行组件 (components/distributed/)  ★ 独立可用  │
...
| M_H.4 | `FinetuneRecipe.setup()` | 18 步组件构建（typed `.build()` + untyped `.instantiate()`） | 1 | 03 §5 |
| M_H.5 | `run_train_validation_loop()` | epoch 迭代 + 训练步 + validation + checkpoint + GC | 0.5 | 03 §6 |
...
重构目标目录 `components/distributed/` 全新，现有 `hyper_parallel/core/` 已有一套自研体系。
迁移策略为**封装复用**而非重写：
