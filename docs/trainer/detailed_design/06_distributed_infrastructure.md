...
| `components/distributed/dtensor_utils.py`（待创建） | `_local_params_context` / `_set_param_by_path` 的 **re-export 入口**——canonical 定义在 `sharding/apply.py`（其 docstring 明确"06 的 dtensor_utils.py re-export 本模块定义，勿另起副本"），本文件**只 re-export `sharding/apply.py` 定义、不另起实现**（见 05 §4.4.1） |
| `components/distributed/utils.py` | 通用工具：`FirstRankPerNode`（每节点首 rank 判定，02 数据管道引用）等 |
...
# components/distributed/sharding_config.py:41-52（该文件已自声明为 canonical），
# 此处直接 import：
