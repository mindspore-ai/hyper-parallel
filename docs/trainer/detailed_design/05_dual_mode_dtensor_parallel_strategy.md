...
| `components/distributed/sharding_applier.py` | `apply_sharding_plan`：双模式应用（validate / production） |
| `components/distributed/precompiled_boundary.py` | `PrecompiledBoundary` / `RedistOp`：编译期通信规划 |
| `components/distributed/param_role.py` | `ParamRole` 枚举 + `ParameterClassifier` |
| `components/distributed/sharding/apply.py` | `_local_params_context` / 路径工具 / `_stack_moe_experts`（D-09b 堆叠） |
| `components/distributed/cp_utils.py` | `flex_cp_allgather`（autograd 版 CP K/V all-gather）+ `shard_batch_for_cp` |
| `components/distributed/ep_utils.py` | `_hf_native_ep_compute` + `MOE_ROUTER_ADAPTERS`（D-09/D-10 EP 直通） |
| `components/distributed/local_region.py` | `local_region`：DTensor↔local 区域缝合（validate / 独立使用） |
| `components/distributed/tp_grad.py` | `build_tp_grad_info`：TP 梯度同步信息（§6.7.1） |
| `components/distributed/testing/grad_equiv.py` | 双模式梯度等价工具（§5.5） |
...
`components/distributed/` 下的所有模块**零依赖**于 `recipes/` 和 `_transformers/`。任何 HF 模型都可以直接使用 ShardingPlanner + apply_sharding_plan（以下示例只使用当前已实现并导出的符号，见 `components/distributed/__init__.py`）：
...
```python
# components/distributed/sharding_config.py
...
def _wrap_flex_attn_for_cp(inner_attn, cp_mesh):
    ...
    flex_cp_allgather 见本节末定义（`components/distributed/cp_utils.py`，
...
def flex_cp_allgather(k, v, cp_dim: int, cp_mesh):
    ...
    （canonical 实现见 components/distributed/cp_utils.py）。
...
def my_cp_wrapper(target_module, cp_mesh) -> None:
    ...
    def cp_forward(q, k, v, **kwargs):
        from hyper_parallel.components.distributed.cp_utils import flex_cp_allgather
...
```python
# components/distributed/testing/grad_equiv.py（canonical）
...
# ⚠️ 规划未实现：components/distributed 当前无 init_ep_token_dispatchers /
...
经 `components/distributed/__init__.py` 导出；与 06 的拓扑校验分工——这里
只看模型 config）：
...
| **tp_grad_info**：planner/applier 侧从 ShardingPlan 读出的 `{param_fqn: (tp_placement, tp_mesh)}` 查表 | `components/distributed/tp_grad.py` | **规划项**——`apply_sharding_plan` 已产出并返回，但 `fully_shard` 不接受 `tp_grad_info` 入参，全仓库无消费端（仅测试工具 `testing/grad_equiv.py::simulate_tp_replicate_grad_sync` 模拟该旁路）；fully_shard 消费端接线待落地（§6.7.1） |

#### 6.7.1 build_tp_grad_info：从 ShardingPlan 读取梯度同步信息（规划项：已产出，fully_shard 消费端待接线）

```python
# components/distributed/tp_grad.py
