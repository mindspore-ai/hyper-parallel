...
按 hyper_parallel/components/distributed/ 与 core/dtensor 实现实况标注
...
ROWS = [
    ...
    # ── S1 启动与配置 ────────────────────────────────────────────────────
    ("S1 启动与配置", "配置解析", "YAML 解析与递归包装",
     "读取训练 YAML，将内容递归包装为可点号访问的 ConfigNode",
     ALL, 1, "P0", "01 §2.3", TODO),
    ("S1 启动与配置", "配置解析", "组件引用即时解析",
     "_target_ 字符串 → 实际 Python 类/函数（点分隔导入 + file.py:attr 两种格式）",
     ALL, 0.5, "P0", "01 §2.4", TODO),
    ("S1 启动与配置", "配置解析", "值类型自动转换",
     "YAML 字符串 → Python 原生类型（ast.literal_eval + 特殊符号映射）",
     ALL, 0.5, "P0", "01 §2.5", TODO),
    ("S1 启动与配置", "配置解析", "嵌套组件递归实例化",
     "按需将嵌套 _target_ 配置递归实例化为 Python 对象",
     ALL, 1, "P0", "01 §2.9-2.10", TODO),
    ("S1 启动与配置", "配置解析", "辅助访问与序列化",
     "get / to_dict / to_yaml_dict / instantiate_path，含敏感字段脱敏",
     ALL, 0.5, "P1", "01 §2.7/2.11", TODO),
    ("S1 启动与配置", "配置解析", "导入安全控制",
     "白名单前缀限制 + 用户模块开关，防止 YAML 执行未授权导入",
     ALL, 0.5, "P1", "01 §2.12", TODO),
    ("S1 启动与配置", "配置解析", "类型化配置桥接",
     "RecipeConfig（canonical 定义在 01 §3.3）：原始配置按语义分类（优化器/调度器/损失/检查点），"
     "每类提供类型安全访问",
     ALL, 1, "P0", "01 §3.3", TODO),
