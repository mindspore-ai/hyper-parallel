...
> 范围：`hyper_parallel/components/distributed/` 全部 12 个源文件（约 3300 行）。
...
| 文件 | 职责 | 阶段 |
|---|---|---|
| [sharding_config.py](../../../hyper_parallel/components/distributed/sharding_config.py) | 数据模型：mesh 轴名、ModuleShardingSpec、ShardingPlan、ShardingTemplate、TEMPLATES | 数据模型 |
| [param_role.py](../../../hyper_parallel/components/distributed/param_role.py) | ParamRole 枚举 + ParameterClassifier（参数命名规则分类） | build-Phase 1 |
| [sharding_planner.py](../../../hyper_parallel/components/distributed/sharding_planner.py) | ShardingPlanner 6-phase 推导管线：模型 → ShardingPlan | build |
| [precompiled_boundary.py](../../../hyper_parallel/components/distributed/precompiled_boundary.py) | RedistOp / PrecompiledBoundary：placement 差异 → 预编译通信序列 | build + runtime |
| [sharding/apply.py](../../../hyper_parallel/components/distributed/sharding/apply.py) | 路径工具、`_local_params_context`（永久解包）、MoE expert 堆叠 | build |
| [sharding_applier.py](../../../hyper_parallel/components/distributed/sharding_applier.py) | `apply_sharding_plan`：Phase 0/A/B/C/D 应用计划 + 五路 forward 包装 | build + runtime |
| [tp_grad.py](../../../hyper_parallel/components/distributed/tp_grad.py) | `build_tp_grad_info`：从 plan 生成 FSDP 梯度旁路信息 | build |
| [head_count.py](../../../hyper_parallel/components/distributed/head_count.py) | D-17 TP 本地头数改写（兼容显式 `num_heads` reshape 写法） | build |
| [ep_utils.py](../../../hyper_parallel/components/distributed/ep_utils.py) | EP a2a（后端分派）、router adapter、TP-extend-EP 前向 | runtime |
| [cp_utils.py](../../../hyper_parallel/components/distributed/cp_utils.py) | CP K/V all-gather、CP 数据切分、offset causal mask | runtime + 数据管道 |
| [local_region.py](../../../hyper_parallel/components/distributed/local_region.py) | DTensor→local→DTensor 局部区域包装（独立工具，validate 用） | runtime（独立） |
| [testing/grad_equiv.py](../../../hyper_parallel/components/distributed/testing/grad_equiv.py) | 双模式梯度等价验证工具 | 测试 |
| [__init__.py](../../../hyper_parallel/components/distributed/__init__.py) | 公共 API 导出 | — |
...
[sharding_config.py:L41-52](../../../hyper_parallel/components/distributed/sharding_config.py#L41-L52)

str 枚举：`TP/CP/EP/PP/DP/DP_REPLICATE/DP_SHARD/DP_SHARD_CP/DP_CP/EP_SHARD`。因为是 str 枚举，`MeshAxisName.TP == "tp"`，可直接做 dict key 与字符串比较。[L57-59](../../../hyper_parallel/components/distributed/sharding_config.py#L57-L59) 定义了 `TP/CP/EP` 简写别名。

### 1.2 `NamedPlacement` — 命名 placement

[sharding_config.py:L63-63](../../../hyper_parallel/components/distributed/sharding_config.py#L63-L63)
...
[sharding_config.py:L83-267](../../../hyper_parallel/components/distributed/sharding_config.py#L83-L267)
...
[sharding_config.py:L271-288](../../../hyper_parallel/components/distributed/sharding_config.py#L271-L288)
...
[sharding_config.py:L375-487](../../../hyper_parallel/components/distributed/sharding_config.py#L375-L487)、[L273-386](../../../hyper_parallel/components/distributed/sharding_config.py#L273-L386)
...
- **`_multi_dim(tp=, cp=, ep=)`** [L183-192](../../../hyper_parallel/components/distributed/sharding_config.py#L183-L192)：构造 NamedPlacement，自动剔除 None 维（size=1 轴不写 key）。
- **`resolve_placements(named, mesh_dim_names)`** [L195-200](../../../hyper_parallel/components/distributed/sharding_config.py#L195-L200)：把 NamedPlacement 按 mesh_dim_names 顺序排成 `List[Placement]`，缺失轴补 `Replicate()`。**这是 "named → positional" 的唯一转换点**，与 DeviceMesh 对齐。
- **`_normalize_out_fields(spec)`** [L203-213](../../../hyper_parallel/components/distributed/sharding_config.py#L203-L213)：标量简写 `{TP: ...}` 归一化为 `{"output": {TP: ...}}`（启发式：dict 的 value 不全是 dict → 判定为简写）。幂等。
- **`_hid(tp_p, cp_p, ep_p)` / `_out(...)`** [L216-223](../../../hyper_parallel/components/distributed/sharding_config.py#L216-L223)：hidden_states 单输入 / 单输出契约简写。
- **`PlacementMismatchError`** [L66-79](../../../hyper_parallel/components/distributed/sharding_config.py#L66-L79)：placement 声明与实际不一致的统一异常（validate 校验、参数分片两处抛出；Phase 5 链式比较已于 2026-07-22 降级为 warning，见 §2.7）。
...
入口：[sharding_planner.py:L177-246](../../../hyper_parallel/components/distributed/sharding_planner.py#L177-L246)
...
[sharding_planner.py:L250-272](../../../hyper_parallel/components/distributed/sharding_planner.py#L250-L272)
...
[sharding_planner.py:L274-285](../../../hyper_parallel/components/distributed/sharding_planner.py#L274-L285)
...
#### TP-extend-EP 语义常量
[L192-196](../../../hyper_parallel/components/distributed/sharding_planner.py#L192-L196)：`ep_extend = ep_size if ep_size > 1 else 0`——**ep_size 即扩展 EP 组大小**（a2a 通信域，由 TP 组向相邻 dp/cp rank 扩展；D-10 TP-extend-EP，无单独 etp 配置）。
...
#### `ParamRole` 枚举
[param_role.py:L33-51](../../../hyper_parallel/components/distributed/param_role.py#L33-L51)
...
[param_role.py:L101-107](../../../hyper_parallel/components/distributed/param_role.py#L101-L107)

遍历 `named_parameters()` 逐参数分类。

#### `classify_param(name, overrides)` — 单参数三级匹配
[param_role.py:L109-122](../../../hyper_parallel/components/distributed/param_role.py#L109-L122)

1. **架构覆盖** `ARCH_OVERRIDES[arch]`（[planner L79-91](../../../hyper_parallel/components/distributed/sharding_planner.py#L79-L91)：llama/qwen2/qwen3/mixtral 为空表占位；已内置 **DeepSeek MLA** 条目（deepseekv2/v3 两种拼写），q_a/kv_a → `REPLICATED`、q_b/kv_b → `COLWISE`，见 §2.1 枚举注记与 test_s1_mla_deepseek.py）；
2. **默认命名规则**（[param_role.py:L59-83](../../../hyper_parallel/components/distributed/param_role.py#L59-L83)），按序首匹配——排序原则"更具体在前"：`shared_expert` 先于 `experts`；`.bias` 先于 colwise（否则 `q_proj.bias` 被 q_proj 截获）；
3. 未命中 → `SKIP`。
...
[sharding_planner.py:L295-340](../../../hyper_parallel/components/distributed/sharding_planner.py#L295-L340)
...
[sharding_planner.py:L344-402](../../../hyper_parallel/components/distributed/sharding_planner.py#L344-L402)
...
2. **叶子段守卫**（[L110-115](../../../hyper_parallel/components/distributed/sharding_planner.py#L110-L115)）：`q_proj/gate_proj/experts/fc1/...` 等叶段名 → unknown（自身不是边界容器）；
3. **数字段守卫** [L376-377](../../../hyper_parallel/components/distributed/sharding_planner.py#L376-L377)：段名是纯数字（HF per-expert 容器 `experts.0..N`）→ unknown，参数须向上聚合到 moe 容器（D-09）；
...
[sharding_planner.py:L406-468](../../../hyper_parallel/components/distributed/sharding_planner.py#L406-L468)
...
3. **运行时覆盖**：
   - lm_head 的 out_dst 按 `loss_parallel` 实时决定（Shard(-1) vs Replicate）[L444-448](../../../hyper_parallel/components/distributed/sharding_planner.py#L444-L448)；
   - embed 在 CP+SP 下 in/out 的 CP 维改 Shard(1)（D-05：数据管道已切好 input_ids，不能二次切分）[L455-460](../../../hyper_parallel/components/distributed/sharding_planner.py#L455-L460)；
...
[sharding_planner.py:L471-506](../../../hyper_parallel/components/distributed/sharding_planner.py#L471-L506)
...
**D-08 ndim 感知**（[`_moe_expert_tp_placement` L148-167](../../../hyper_parallel/components/distributed/sharding_planner.py#L148-L167)）：stacked expert 权重是 3D `[E, H_out, H_in]`，dim 0 是 expert 维（归 EP Shard(0)），所以 TP 的 colwise/rowwise 要 +1 维：colwise（切 H_out）→ Shard(1)，rowwise（切 H_in）→ Shard(2)。
...
[sharding_planner.py:L556-653](../../../hyper_parallel/components/distributed/sharding_planner.py#L556-L653)

**功能**：识别 HF 原生 MoE 的两种 expert 布局（D-09a 堆叠 / D-11 batched），把 spec 改写为（堆叠元数据 +）TP-extend-EP 形式。
**命中条件**：`ep_extend>0`（即 ep_size>1）且组内全部 MOE_EXPERT 参数属于同一布局（`_PER_EXPERT_RE` / `_BATCHED_EXPERT_RE`（[L547-554](../../../hyper_parallel/components/distributed/sharding_planner.py#L547-L554)），混合不标记并 warning）：
...
原命中条件（[L547-554](../../../hyper_parallel/components/distributed/sharding_planner.py#L547-L554)，`^experts\.(\d+)\.([^.]+)\.weight$`）。任一不满足 → 原样返回（pre-stacked 布局走模块自身 dispatcher）。

**命中后做三件事**：

1. **TP-extend-EP 校验** `_validate_ep_extend`（[L534-566](../../../hyper_parallel/components/distributed/sharding_planner.py#L534-L566)）：
...
2. **参数表改写** [L631-642](../../../hyper_parallel/components/distributed/sharding_planner.py#L631-L642)：per-expert 布局按投影名聚合并删除 per-expert 条目、插入 stacked 条目；batched 布局原地改写——两者最终契约相同：
   ```python
...
3. **边界契约改 SP-in identity** [L644-653](../../../hyper_parallel/components/distributed/sharding_planner.py#L644-L653)：`in_dst = in_src`，`out_src = out_dst = in_src 的布局`——Megatron MoE 本就不 gather 序列，通信全部内聚到 region 内部（`_hf_native_ep_compute`）。同时记录 `_moe_router`（按 arch 选 adapter，未注册落 `"default"`）和 `_ep_size`。
...
[sharding_planner.py:L655-712](../../../hyper_parallel/components/distributed/sharding_planner.py#L655-L712)
...
- **结构标记**：`use_local_map` 为公开字段——用户显式置 True（自研数据相关模块）合并后保留；推断模板为 True 时强制置位 [L703-709](../../../hyper_parallel/components/distributed/sharding_planner.py#L703-L709)——MoE all-to-all 与 CP K/V gather 缺失会导致数值错误，不允许借覆盖关闭；
...
[sharding_planner.py:L716-773](../../../hyper_parallel/components/distributed/sharding_planner.py#L716-L773)

**功能**：按 forward 顺序逐对相邻边界模块，填充缺省 in_src + 相邻契约不一致告警 + 标记 `_is_terminal`。

**步骤**：
1. `_topological_sort_by_forward_order`（[L823-841](../../../hyper_parallel/components/distributed/sharding_planner.py#L823-L841)）：按 `named_modules` 注册顺序排序边界 FQN；
2. 逐对 `(curr, next)`：若 `curr.out_dst` 非空，用 `_pair_contracts` 配对 key（[L809-821](../../../hyper_parallel/components/distributed/sharding_planner.py#L809-L821)：**双方都恰好 1 个 entry 时按"唯一 arg"配对（名字无关）**——解决 attention 输出名 "output" vs moe_mlp 输入名 "x_BLD" 不同名问题；否则按 key 名配对）；
3. 每对 `(out_key, in_key)`：
...
- **`_collect_special_handlers(param_roles)`** [L814-829](../../../hyper_parallel/components/distributed/sharding_planner.py#L814-L829)：SPECIAL 角色参数 → handler 名（`_SPECIAL_HANDLER_PATTERNS` [L115-119](../../../hyper_parallel/components/distributed/sharding_planner.py#L115-L119)：`gated_delta/a_log/dt_bias` → `gated_delta_tp_shard`，未注册归 `"default"`）。
- **`_detect_tied_pairs(model)`** [L834-854](../../../hyper_parallel/components/distributed/sharding_planner.py#L834-L854)：`config.tie_word_embeddings=True` 时用 `named_parameters(remove_duplicate=False)` 找 `embed_tokens.weight` + `lm_head.weight` 两端 FQN（tied 参数默认去重只出现一次，必须关去重）。**PP 场景注意**：tied 对分居不同 stage 时单 part 内无法成对检测，需用户显式声明 `plan.tied_pairs`。
...
[sharding_planner.py:L856-899](../../../hyper_parallel/components/distributed/sharding_planner.py#L856-L899)
...
入口：[sharding_applier.py:L74-151](../../../hyper_parallel/components/distributed/sharding_applier.py#L77-L162)
...
- **`_get_active_mesh(mesh, plan.mesh_dim_names)`** [L169-L176](../../../hyper_parallel/components/distributed/sharding_applier.py#L169-L176)：取与 plan 对齐的活跃子 mesh。plan 剔除了 size=1 轴，但传入 mesh 可能仍含——placement 数必须与 mesh 维数对齐，否则 `distribute_tensor` 静默错轴。**注意 `full_mesh` 在 [L91](../../../hyper_parallel/components/distributed/sharding_applier.py#L91) 先行保存**——D-10 派生 expert mesh 需要全 dense 区域（含 dp/cp 轴）。
- **TP-extend-EP expert mesh 构建** [L99-L102](../../../hyper_parallel/components/distributed/sharding_applier.py#L99-L102)：任一 spec 的 `_ep_size>0` → `_build_expert_mesh(full_mesh, ...)`。

#### `_expert_mesh_layout(mesh, mesh_dim_names, ep_size)` — 纯映射（可单测）
[sharding_applier.py:L186-213](../../../hyper_parallel/components/distributed/sharding_applier.py#L197-L224)
...
#### `_build_expert_mesh(...)`
[L231-L231](../../../hyper_parallel/components/distributed/sharding_applier.py#L231-L231)：用 layout 的 rank_list 调 `init_device_mesh` 建真实 DeviceMesh（含进程组）。
...
[L104-L106](../../../hyper_parallel/components/distributed/sharding_applier.py#L104-L106)：对所有 spec 再跑一遍 `_normalize_out_fields`（幂等，覆盖用户经 plan_overrides 注入的路径）。

### 3.2 Phase A：参数分片

[L108-L136](../../../hyper_parallel/components/distributed/sharding_applier.py#L108-L136)：逐模块：
...
3. **production → D-17 头数改写**：`maybe_update_head_counts`（[head_count.py](../../../hyper_parallel/components/distributed/head_count.py)）——前向跑在永久解包的 local tensor 上，把模块缓存的头数属性改写为本地值（见下）；validate 此处不改。

#### production 头数改写（D-17，`maybe_update_head_counts`）
[sharding_applier.py:L129-L136](../../../hyper_parallel/components/distributed/sharding_applier.py#L129-L136) + [head_count.py](../../../hyper_parallel/components/distributed/head_count.py)
...
**双模式规则**（关键）：只有"前向看到 local tensor"的模块才改写——production 全量改写；validate 下普通 boundary 模块跑 DTensor dispatch（**全局逻辑形状**，显式全局头数天然正确，改写反而错）不改写，local-region 模块（区域内两模式都是 local tensor）在 Phase C 分支内改写（[sharding_applier.py:L310-L316](../../../hyper_parallel/components/distributed/sharding_applier.py#L310-L316)）。
...
[sharding/apply.py:L102-140](../../../hyper_parallel/components/distributed/sharding/apply.py#L102-L140)
...
- 创建 `_StackedExperts` holder（[L96-99](../../../hyper_parallel/components/distributed/sharding/apply.py#L96-L99)，空 nn.Module 容器），注册 stacked Parameter；
...
[sharding_applier.py:L233-258](../../../hyper_parallel/components/distributed/sharding_applier.py#L244-L269)
...
- [`_get_attr_by_path` L32-37](../../../hyper_parallel/components/distributed/sharding/apply.py#L32-L37)：点分 FQN 取属性，数字段走 ModuleList 索引；
- [`_set_param_by_path` L40-54](../../../hyper_parallel/components/distributed/sharding/apply.py#L40-L54)：定位父模块后 `register_parameter` 替换叶参数（直接 `setattr(model, dotted, ...)` 只会设怪属性）；
- [`_resolve_module` L57-68](../../../hyper_parallel/components/distributed/sharding/apply.py#L57-L68)：按模块 FQN 取模块（不剥末段）。
...
[L138-L146](../../../hyper_parallel/components/distributed/sharding_applier.py#L138-L146)：按 `plan.special_handlers` 调 `SPECIAL_HANDLERS[name](module, param_name, mesh)`。当前唯一实现 `_shard_gated_delta`（[planner L91-106](../../../hyper_parallel/components/distributed/sharding_planner.py#L91-L106)，SSM/Mamba 类骨架：直接 Shard(0) 回退，head 对齐切分留待具体模型接入）。

### 3.4 Phase C 入口：production 永久解包 + tp_grad_info

[L148-L155](../../../hyper_parallel/components/distributed/sharding_applier.py#L148-L155)

#### `_local_params_context(model)` → `{fqn: placements}`
[sharding/apply.py:L71-86](../../../hyper_parallel/components/distributed/sharding/apply.py#L71-L86)
...
[tp_grad.py:L26-57](../../../hyper_parallel/components/distributed/tp_grad.py#L26-L57)

**为什么从 plan 读而非 DTensor**：production 下参数已被解包，只有 plan 保留完整 placement 信息。
- 普通参数：`named_placement.get("tp", Replicate())`；
- **D-10 TP-extend-EP expert 参数** [L38-44](../../../hyper_parallel/components/distributed/tp_grad.py#L38-L44)：强制 `Shard(1)`——expert 权重在派生 expert mesh (edp, ep) 上仅按 expert 维分片，梯度是各 rank 不同的 local shard（不同 expert + 扩展 EP 组聚合的 token），**不做 TP 组同步**；缺省 Replicate 会让 FSDP 对分片梯度错误 all-reduce；
- tied 对归一化 [L47-56](../../../hyper_parallel/components/distributed/tp_grad.py#L47-L56)：两端 placement 不一致时取较细分片（Shard 优先），保证两端 TP 归约语义一致。
...
[sharding_applier.py:L277-L326](../../../hyper_parallel/components/distributed/sharding_applier.py#L277-L326)
...
[sharding_applier.py:L322-L349](../../../hyper_parallel/components/distributed/sharding_applier.py#L322-L349)
...
**位置回退** [L345-L349](../../../hyper_parallel/components/distributed/sharding_applier.py#L345-L349)：in_plan 仅 1 个 op 且签名未命中 → 绑定到首个 positional 参数。覆盖"模板 key `hidden_states` vs 叶模块签名 `nn.Linear.forward(input)` 不同名"。

### 3.6 Phase D：tied weights

[L161-L165](../../../hyper_parallel/components/distributed/sharding_applier.py#L161-L165) → [`_replicate_tied_weights` L1113-1117](../../../hyper_parallel/components/distributed/sharding_applier.py#L1131-L1135) → [`_broadcast_tied_param` L1086-1110](../../../hyper_parallel/components/distributed/sharding_applier.py#L1104-L1128)。
...
[precompiled_boundary.py](../../../hyper_parallel/components/distributed/precompiled_boundary.py)
...
[precompiled_boundary.py:L42-66](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L42-L66)
...
[precompiled_boundary.py:L92-130](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L92-L130)

字段：`arg_name / arg_index / mesh / src_placements / dst_placements / collective_type`。

**`execute(tensor, as_dtensor=False)`** [L105-129](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L105-L129)：
...
[precompiled_boundary.py:L133-238](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L133-L238)

**`__init__`** [L136-141](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L136-L141)：编译 `in_plan` + `out_plan` 两个 RedistOp 列表。

**`_compile_input_plan`** [L145-163](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L145-L163)：对 `in_src ∪ in_dst` 的每个输入名生成一个 RedistOp（identity 也保留——输入需要 from_local 包装/解包）。

**`_compile_output_plan`** [L165-196](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L165-L196)：`out_src/out_dst` 任一为 None → 空计划；identity 跳过（**输出侧不编译 identity**——出口张量在 wrapper 层统一处理）；`arg_index` 取 `spec.out_names` 顺序（缺省按 out_src key 顺序）。
...
**`redistribute_inputs(args, kwargs, as_dtensor=False)`** [L200-212](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L200-L212)：逐 op 用 `_get_arg`（先 kwargs 名、后 arg_index）取参，执行后 `_set_arg` 写回。arg 找不到（None）跳过——如 embed 的 in_src key "input" 与实际 kwargs 名 "input_ids" 不同名且 identity。

**`redistribute_outputs(outputs, as_dtensor_input=False)`** [L215-237](../../../hyper_parallel/components/distributed/precompiled_boundary.py#L215-L237)：单输出/多输出 tuple 保序处理，按 `arg_index` 定位；`as_dtensor_input=True`（validate）保持 DTensor 供 out_dst 校验，否则返回 local。
...
[sharding_applier.py:L334-350](../../../hyper_parallel/components/distributed/sharding_applier.py#L352-L368)
...
[sharding_applier.py:L352-391](../../../hyper_parallel/components/distributed/sharding_applier.py#L370-L409)
...
[sharding_applier.py:L417-446](../../../hyper_parallel/components/distributed/sharding_applier.py#L435-L464)

out_src/out_dst 共用的校验实现：多输出按 `spec.out_names`（缺省声明 key 顺序）映射 tuple 位置；非 DTensor 输出跳过；比较前 `_normalize_placements_ndim`（[L427-L435](../../../hyper_parallel/components/distributed/sharding_applier.py#L427-L435)）把 `Shard(-1)` 按实际 ndim 归一；不一致抛 `PlacementMismatchError`。
...
[sharding_applier.py:L521-589](../../../hyper_parallel/components/distributed/sharding_applier.py#L539-L607)
...
**`_resolve_local_compute_fn`** [L499-L536](../../../hyper_parallel/components/distributed/sharding_applier.py#L499-L536)（**单一解析链**，返回 None = 不走骨架）：
...
**`_wrap_local_region_forward`** [L539-L607](../../../hyper_parallel/components/distributed/sharding_applier.py#L539-L607)（通用骨架，原 `_wrap_moe_forward`）：boundary 入口 → local region → 按声明 out_src 重包装 → boundary 出口。**两模式共用同一份代码**（local_region 容错透传语义）。

**`local_region_forward` 四步** [L573-L605](../../../hyper_parallel/components/distributed/sharding_applier.py#L573-L605)：
1. boundary 入口（validate 时 as_dtensor=True；TP-extend-EP 契约是 SP-in identity，通常直通）；
2. local region：validate 下 `to_local` 输入 + `_temp_local_params`（[L477-L496](../../../hyper_parallel/components/distributed/sharding_applier.py#L477-L496)，临时解包 DTensor 参数、退出恢复，保持 DTensor 传播链不断）；production 直接跑；
...
**链 1：`_resolve_inner_target`** [L614-L647](../../../hyper_parallel/components/distributed/sharding_applier.py#L614-L647)（纯位置）：
...
**链 2：`_resolve_inner_wrapper`** [L718-L779](../../../hyper_parallel/components/distributed/sharding_applier.py#L718-L779)（纯行为，返回 `(name, target, apply_fn)` 或 None）：
1. `inner_wrapper` 是 **Callable** → 全自定义（`_apply_custom_inner_wrapper` [L689-L715](../../../hyper_parallel/components/distributed/sharding_applier.py#L689-L715)：应用 + validate 收 DTensor 时一次性 WARNING 提示双模式容错或声明 `use_local_map`）；
2. `inner_wrapper` 是 **str** → `CP_WRAPPER_REGISTRY` 查找（未知名 fail-fast 并列出可用名；target 缺失 fail-fast 提示 `inner_target`）；
3. `inner_target`/`_needs_cp_attn` 声明 → **启发式 2×2 分派**（`_dispatch_builtin_cp_wrapper` [L682-L686](../../../hyper_parallel/components/distributed/sharding_applier.py#L682-L686)：`_is_hf_style_attention` [L668-L679](../../../hyper_parallel/components/distributed/sharding_applier.py#L668-L679) 首参名 `hidden_states` → HF 原语拦截路；`_is_sdpa/_is_flex_attention` [L658-L666](../../../hyper_parallel/components/distributed/sharding_applier.py#L658-L666) 按 `config._attn_implementation`/类名选 SDPA/Flex）；target 缺失 → fail-fast（缺失 K/V all-gather 是静默数值错误）；
4. 皆无 → `None`。

**协调器：`_wrap_cp_inner_attention`** [L782-L820](../../../hyper_parallel/components/distributed/sharding_applier.py#L782-L820)：解析 → 应用 → **回写 `spec._resolved_inner_wrapper`** + INFO 日志（target/wrapper/来源；启发式分派会提示可用 str 固定）。`direct=True`（默认）供测试/手动接入（直接调用即显式意图）；`_apply_phase_c` 传 `direct=False`。

**内置注册表 `CP_WRAPPER_REGISTRY`** [L1031-L1036](../../../hyper_parallel/components/distributed/sharding_applier.py#L1031-L1036)（契约 `fn(target, cp_mesh, *, spec, mesh, mesh_dim_names)`，用户可注册命名方案）：

| 注册名 | 函数 | 机制 |
|---|---|---|
| `"sdpa_hf"` | [`_wrap_hf_sdpa_for_cp` L887-945](../../../hyper_parallel/components/distributed/sharding_applier.py#L905-L963) | 临时全局替换 `F.scaled_dot_product_attention`（try/finally 还原；非线程安全但单进程 SPMD 训练下安全，与 TorchTitan CP 实现一致），复用 HF 投影/RoPE；**发火检测**：未拦到调用即 RuntimeError（启发式误猜不再静默） |
| `"flex_hf"` | [`_wrap_hf_flex_for_cp` L948-1004](../../../hyper_parallel/components/distributed/sharding_applier.py#L966-L1022) | 同上，拦截 `flex_attention`（`block_mask` 须按**全局 kv 长度**构建；发火检测同） |
| `"sdpa_qkv"` | [`_wrap_sdpa_for_cp` L833-858](../../../hyper_parallel/components/distributed/sharding_applier.py#L851-L876) | 包装 `forward(q,k,v,...)`：显式 all-gather K/V + D-04 mask 后调原 forward |
| `"flex_qkv"` | [`_wrap_flex_attn_for_cp` L861-884](../../../hyper_parallel/components/distributed/sharding_applier.py#L879-L902) | 同上（block_mask 约束同 flex_hf） |
...
[sharding_applier.py:L805-826](../../../hyper_parallel/components/distributed/sharding_applier.py#L823-L844)

1. `flex_cp_allgather(k, v, cp_dim=2, cp_mesh)` —— K/V 沿序列维 all-gather；
2. **D-04** [L828-L842](../../../hyper_parallel/components/distributed/sharding_applier.py#L828-L842)：`is_causal` 且 **CP 激活**（`cp_mesh.size() > 1`，2026-07-21 由 `q_len ≠ kv_len` 形状比较修订为语义判断——GQA 差异在 head 维不影响序列维，但 cross-attention/KV-cache 的 q_len≠kv_len 与 CP 无关，形状推断会把 `lo` 偏移语义用错）时，torch 的 is_causal 按左上角对齐（等价于假设 Q chunk 位于序列开头）、对 rank>0 的 Q chunk 掩码错误（G4）→ 替换为 `_cp_offset_causal_mask(q_len, kv_len, lo=cp_rank*q_len)`（[cp_utils.py:L214-224](../../../hyper_parallel/components/distributed/cp_utils.py#L214-L224)：允许 attend `j <= lo + i`；rank0 lo=0 退化为标准 causal）。性能注记：显式 `attn_mask` 使 SDPA flash backend 不可选（回退 mem_efficient/math），正确性优先。

#### CP 原语（cp_utils.py）

- **`_AllGatherAlongDim`** [cp_utils.py:L34-55](../../../hyper_parallel/components/distributed/cp_utils.py#L34-L55)：autograd.Function——forward 沿 cp_dim all-gather + 按 rank 序 cat；backward all-reduce 后取本 rank chunk（reduce-scatter 语义。plain `dist.all_gather` 无 autograd 核，必须显式实现）。
- **`flex_cp_allgather(k, v, cp_dim, cp_mesh)`** [L58-78](../../../hyper_parallel/components/distributed/cp_utils.py#L58-L78)：CP≤1 直通；通信组取 `cp_mesh.get_group()`——**DeviceMesh 构建时已缓存，此处禁调 `dist.new_group`**（否则每次 forward 泄露一个进程组）。
- **`shard_batch_for_cp(batch, cp_mesh)`** [L83-144](../../../hyper_parallel/components/distributed/cp_utils.py#L83-L144)：数据管道 CP 切分。pad 到 2·cp 倍数（position_ids 递增 pad、labels pad -100）→ 按 `[cp_rank·chunk, (cp_rank+1)·chunk)` 切片；`seq_lens/seq_lens_padded` 走 `_shard_seq_lens_for_cp`（[L150-211](../../../hyper_parallel/components/distributed/cp_utils.py#L150-L211)：逐 pack 求与本地区间的交集，跨界截断、哨兵 -1000 保留、输出平移到本地坐标系）。
...
- [`_is_vocab_parallel_embed` L1025-1032](../../../hyper_parallel/components/distributed/sharding_applier.py#L1043-L1050)：nn.Embedding + weight 在 TP 上 Shard(0) + TP>1；
- [`_wrap_vocab_parallel_embedding` L1035-1057](../../../hyper_parallel/components/distributed/sharding_applier.py#L1053-L1075)：Megatron 风格 masked embedding。
...
[ep_utils.py:L51-145](../../../hyper_parallel/components/distributed/ep_utils.py#L51-L145)

- **`_ep_all_to_all(x, send_counts, recv_counts, group)`** [L137-145](../../../hyper_parallel/components/distributed/ep_utils.py#L137-L145)：统一入口。counts 是 `list[int]`（长度 = 组大小，各 dest/src rank 的行数）。NCCL/HCCL → uneven；gloo 等 → padded。
- **`_EPAllToAllUneven`** [L55-77](../../../hyper_parallel/components/distributed/ep_utils.py#L55-L77)：`split(x, send_counts) → dist.all_to_all(list) → cat`，零填充；backward 交换 send/recv counts 再做一次（a2a 自逆）。
> 后端能力实证（2026-07-20 实测）：gloo 支持等长 `all_to_all_single`（含 int64）但不支持 list-based 不等长 `all_to_all`；NCCL/HCCL 两者皆支持。这是下述两条路径的分派依据。

- **`_EPAllToAllPadded`** [L80-134](../../../hyper_parallel/components/distributed/ep_utils.py#L80-L134)：各 dest chunk pad 到**全局** max（`all_reduce MAX` 求得——a2a_single 要求各 rank chunk 等长且全局一致）→ `all_to_all_single` → 按 recv_counts unpad；backward 按 recv pad → a2a_single → 按 send unpad。两路径数值语义一致（pad 行不参与计算）。
...
- **`_softmax_topk_router`** [L152-176](../../../hyper_parallel/components/distributed/ep_utils.py#L152-L176)：default adapter（Linear gate 的 softmax topk + `norm_topk_prob` 归一化）。返回 `(topk_idx [T,K] int64, topk_w [T,K] float)`。
- **`_topk_router_module`** [L179-193](../../../hyper_parallel/components/distributed/ep_utils.py#L179-L193)：**qwen3moe/mixtral** adapter——gate 为 TopKRouter 模块（HF 2025 重构后），forward 直返 `(logits, scores, indices)`，取后两个。
- **`_sigmoid_group_router`**（参数来源优先级：模块自身属性 > `module.config`——n_group/topk_group/top_k/norm_topk_prob/routed_scaling_factor 均按此顺序取） [L196-241](../../../hyper_parallel/components/distributed/ep_utils.py#L196-L241)：**deepseekv3/glm4moe** adapter——sigmoid + `e_score_correction_bias` + group-limited topk（n_group/topk_group，缺省跳过）+ 可选归一化 + `routed_scaling_factor`，与 HF `route_tokens_to_experts` 逐步一致。
- **`MOE_ROUTER_ADAPTERS`** [L256-265](../../../hyper_parallel/components/distributed/ep_utils.py#L256-L265)：planner 按 arch 名选（`_get_architecture` 去后缀形式如 `qwen3moe`，下划线别名覆盖 model_type 路径）；未注册落 default。
...
[ep_utils.py:L272-299](../../../hyper_parallel/components/distributed/ep_utils.py#L272-L299)
...
[ep_utils.py:L310-396](../../../hyper_parallel/components/distributed/ep_utils.py#L310-L396)
...
| 步 | 代码 | 做什么 | 张量形状变化（本例） |
|---|---|---|---|
| 0 | [L327-337](../../../hyper_parallel/components/distributed/ep_utils.py#L327-L337) | 解析权重；`e_global = e_local × ep_size`；`expert_offset = ep_rank × e_local`；x 展平 [T,H] | T = B·S/2 |
| 1 router | [L338-344](../../../hyper_parallel/components/distributed/ep_utils.py#L338-L344) | 本地 chunk 上路由（**无通信**）；`dest = flat_idx // e_local`（目标 EP rank） | topk_idx [T,2] → flat [2T] |
| 2 排序+counts | [L346-354](../../../hyper_parallel/components/distributed/ep_utils.py#L346-L354) | 按 `(dest, expert)` 排序得 `perm`；`send_counts = bincount(dest)`；counts 经 `all_to_all_single` 交换得 `recv_counts` | send_x [Σsend, H] |
| 3 a2a dispatch | [L356-359](../../../hyper_parallel/components/distributed/ep_utils.py#L356-L359) | `_ep_all_to_all` 过扩展 EP 组：token（带完整 H）发往 expert 持有 rank（可跨 TP/dp 坐标）；expert id 同步交换 | recv_x [Σrecv, H] |
| 4 本地 SwiGLU | [L361-376](../../../hyper_parallel/components/distributed/ep_utils.py#L361-L376) | 按 expert id 排序 → 逐本地 expert SwiGLU（**完整 expert 权重**，输出 complete，**无 Partial**）；fused 布局（`w_up is None`）走 `linear(x, gate_up[i]).chunk(2)` 分支 → 逆排序还原 | y [Σrecv, H] |
| 5 a2a combine | [L381-384](../../../hyper_parallel/components/distributed/ep_utils.py#L381-L384) | 反向 a2a 回扩展 EP 组 → 逆 `perm` → 按 `topk_w` 加权、按 K 聚合 → reshape | out [B, S/2, H] |
| 6 shared_experts | [L387-395](../../../hyper_parallel/components/distributed/ep_utils.py#L387-L395) | 若存在：本地 chunk × TP 分片权重 → Partial → **TP 组 all_reduce** 后相加 | — |
...
[local_region.py:L110-230](../../../hyper_parallel/components/distributed/local_region.py#L110-L230)
...
**容错透传语义**（三点增强，[L169-223](../../../hyper_parallel/components/distributed/local_region.py#L169-L223)）：
...
辅助：[`_bind_arg_names` L63-79](../../../hyper_parallel/components/distributed/local_region.py#L63-L79)（签名内省，positional→名映射；C 扩展式签名不可内省时回退为空 dict——此时仅 kwargs 传参能命中 `in_placements`，positional 参数全部直通）、[`_normalize_out_placements` L82-107](../../../hyper_parallel/components/distributed/local_region.py#L82-L107)（扁平/逐输出两种写法归一）。
...
[testing/grad_equiv.py](../../../hyper_parallel/components/distributed/testing/grad_equiv.py)

两模式 backward **同为 local 路径**（无 "DTensor backward" 对照组），梯度等价直接逐参数比较：
- [`run_one_step` L36-46](../../../hyper_parallel/components/distributed/testing/grad_equiv.py#L36-L46)：单步 forward+backward，返回 `{param_fqn: grad}`；
- [`assert_grad_equivalence` L49-57](../../../hyper_parallel/components/distributed/testing/grad_equiv.py#L49-L57)：双模式梯度逐参数 assert_close；
- [`simulate_tp_replicate_grad_sync` L60-69](../../../hyper_parallel/components/distributed/testing/grad_equiv.py#L60-L69)：模拟 tp_grad_info 旁路（TP-Replicate 参数梯度的 TP all-reduce）——真实路径由 FSDP2 fork 完成，此处用于独立验证。
...
from hyper_parallel.components.distributed.sharding_config import TP, CP
