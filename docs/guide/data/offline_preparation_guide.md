...
`huggingface_offline` 会通过 Hugging Face Datasets 读取这些本地格式，先归一化为
JSONL，再执行现有 tokenizer 和 `.bin/.idx` 生成流程。一个转换调用中的文件必须属于同一种格式；
不同格式请分别执行转换。目录会递归发现受支持的文件。
...
```bash
python -m hyper_parallel.data.tools.offline_preparation [参数]
```
...
```bash
python -m hyper_parallel.data.tools.huggingface_offline [参数]
```
...
from hyper_parallel.data.tools.offline_preparation import prepare_offline_dataset
...
```bash
python -m hyper_parallel.data.tools.offline_preparation --help
python -m hyper_parallel.data.tools.huggingface_offline --help
```
