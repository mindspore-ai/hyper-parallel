...
codegen 把并行规划与高性能模块替换显示的写入模型文件。codegen读取训练 YAML配置，离线推导 TP/CP/EP 等并行契约，将参数布局、边界重分发、前向边界改写和模块替换冻结进 artifact（`codegen_meta.json` + 生成版 modeling 文件）。训练时各 rank 加载同一份 artifact，并执行其中的 `hyper_parallelize()`。
...
python -m hyper_parallel.auto_models.codegen --verbose generate \
  --config examples/training_demo/train_codegen_qwen3_moe.yaml

# 预飞完整性校验：signature、文件哈希、meta、import 与冻结计划结构
python -m hyper_parallel.auto_models.codegen --verbose check \
  --config examples/training_demo/train_codegen_qwen3_moe.yaml
...
# 删除某一 YAML 的产物
python -m hyper_parallel.auto_models.codegen --verbose clean \
  --config examples/training_demo/train_codegen_qwen3_moe.yaml
```
...
对于 `replace_module`，原生路径是在 trainer 的动态 applier 中匹配模块并替换；codegen 会把替换记录写成 generated modeling 文件里的常量，并在模型 `__init__` 末尾调用：
...
_HYPER_MODULE_OVERRIDES = [
    {
        ...
        "factory": "hyper_parallel.auto_models.components.models.qwen3_moe_fusions.replace_qwen3_moe_flash_attention",
        "exact_type": False,
    }
]
