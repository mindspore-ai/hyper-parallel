# components/distributed 双模式 DTensor 使用教程

> 适用组件：`hyper_parallel/components/distributed/`
...
`components/distributed` 是独立可用的 DTensor 分片组件，零依赖训练流程
...
```python
from hyper_parallel.components.distributed.sharding_planner import ARCH_OVERRIDES
from hyper_parallel.components.distributed.param_role import ParamRole
...
```python
from hyper_parallel.components.distributed import ModuleShardingSpec
...
```python
from hyper_parallel.components.distributed.sharding_applier import CP_WRAPPER_REGISTRY
