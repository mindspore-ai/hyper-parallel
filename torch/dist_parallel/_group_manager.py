...
def generate_groups_from_template(
    ...
    for i in range(1, template_len - 1):
        if normalized_template[i + 1] - normalized_template[i] != intra_step:
            raise ValueError(
                f"Template must have consistent intra-group step. "
                f"Found {normalized_template[i+1]} - {normalized_template[i]} = {normalized_template[i+1]-normalized_template[i]}, "
    ...
    # 3. 计算所有合法的块起始位置
    group_starts = []
    for start in range(0, world_size, inter_step):
        # 确保块能完整容纳（块内所有子组的rank都不越界）
        if start + block_size <= world_size:
    ...
    for start_block in group_starts:
        # 块内最大偏移量：确保子组不超出当前块范围
        max_offset = block_size - template_span
        # 遍历块内所有合法偏移（步长=1，覆盖所有可能的子组）
        for offset in range(0, max_offset):
            # 基于归一化模板生成当前偏移对应的子组
            group = [start_block + offset + normalized_template[i] for i in range(template_len)]
            # 最终校验：所有rank都在world_size范围内
    ...
    if len(all_ranks) != len(unique_ranks):
        raise ValueError(f"Duplicate ranks found! Some ranks appear in multiple groups.")
