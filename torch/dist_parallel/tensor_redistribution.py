...
import torch
import torch.distributed.nn.functional as dnn_f
import dist_parallel
from .redistribute_infer import RedistributionOperatorInfer
...
class TensorRedistribution:
    ...
    def _construct_all_concat_new(self, x, *args):
        ...
        out = dnn_f.all_gather(x, group=group)
        return torch.cat(out, dim=concat_dim)
    ...
    def _construct_all_to_all(self, x, *args):
        ...
        output_tensor = torch.empty(reshaped_shape, device=x.device, dtype=x.dtype)
        output_tensor = dnn_f.all_to_all_single(
            output_tensor,
            x_reshaped,
            group=group
        )
    ...
    def redistribution(self, input_x, to_layout):
        ...
        if self._apply_eazy_redistribute(from_layout, to_layout):
            ...
            raise ValueError("Cannot infer redistribute between from_layout: {from_layout}, and to_layout: {to_layout}.")
        return dist_parallel.DTensor.from_local(x, to_layout)
    ...
    def _allreduce_along_dev_dim(self, x, op, layout, dev_dim):
        ...
        if op == 'avg':
            ...
            x  = dnn_f.all_reduce(x, group=group)
            x = x / dev_num
        else:
            x = dnn_f.all_reduce(x, group=group)
        if zero_dim:
    ...
    def _reduce_scatter_along_dev_dim_with_axis(self, x, axis, op, layout, dev_dim):
        ...
        dev_num = layout.device_matrix[layout.alias_name.index(dev_dim)]
        group = layout.get_comm_group_by_axis(dev_dim, self.rank_id)
        x_tuple = torch.chunk(x, dev_num, dim=axis)
        output_tensor = torch.empty(x_tuple[0].shape, device=x.device, dtype=x.dtype)
        if op == 'avg':
            output_tensor = dnn_f.reduce_scatter(output_tensor, x_tuple, group=group)
