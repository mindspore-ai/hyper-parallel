...
CASES = (
    ...
    Case("dense-tp2-consistency-direct", tp=2, strategy="direct_reshard", exact=True),
    Case("dense-disjoint-consistency-full", tp=2, disjoint=True, exact=True),
    Case("dense-disjoint-consistency-direct", tp=2, strategy="direct_reshard", disjoint=True, exact=True),
    ...
    Case("ppo-tp2-direct", tp=2, strategy="direct_reshard", algorithm="ppo"),
    Case("ppo-checkpoint-resume", algorithm="ppo", resume=True),
)
...
def prepare_config(case: Case, phase: int, ports: tuple[int, int], devices: list[int]) -> dict:
    ...
    if case.runner != "internal":
        ...
        recipe = EXAMPLES / "configs/qwen3_4b_gsm8k_ppo.yaml"
    else:
    ...
    train.update(max_steps=1 if case.resume and phase == 1 else 2,
                 prompt_batch_size=2, micro_batch_size=2,
