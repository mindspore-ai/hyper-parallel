...
# ============================================================================
"""Torch STs for DeviceMesh.concatenate."""
...
from tests.st.utils import _DEVICE_TYPE, init_backend
...
def test_device_mesh_concatenate_supports_root_and_flattened_dims():
    """
    Feature: Torch DeviceMesh.concatenate with root and flattened dims.
    ...
    assert torch.equal(root_mesh.mesh.cpu(), concat_flat_mesh.mesh.cpu()), (
        f"Flattened concatenate should preserve root mesh tensor on rank {rank}"
    )


def test_device_mesh_concatenate_rejects_out_of_root_order():
    """
    Feature: Torch DeviceMesh.concatenate validates original root order.
