...
5. **TP 反向归一化**：TP plan 让 logits 是 Replicate（`use_local_output=True`），每个 TP rank 都计算了一份相同的 scalar loss；若不归一化，反向时 grad 会在 TP 维上被求和 `tp_size` 次。我们把 `1.0 / tp_size` 作为 `loss.backward(...)` 的 gradient seed，正好抵消（与 `tests/st/fully_shard/_test_tp_fully_shard_e2e.py` 中的做法一致）。
...
进入本目录后再执行（`tests/st/utils.py:torchrun_case` 拼出的命令对 worker 文件名是相对路径，需要 pytest 的 `cwd` 与 launcher 同目录，与仓库其他多卡 launcher 用法一致）：

```bash
cd tests/st/accuracy
