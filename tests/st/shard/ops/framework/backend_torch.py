...
from tests.st.shard.framework.backend import ShardBackend
from tests.st.shard.framework.case_spec import CompareSpec, InputSpec
from tests.st.shard.framework.utils import build_numpy
from tests.st.shard.utils import local_to_global as _local_to_global
...
class TorchHcclBackend(_TorchBackendBase):
    ...
    def maybe_init_dist(self) -> None:
        ...
        from tests.st.utils import init_dist
        init_dist()
...
class TorchGlooBackend(_TorchBackendBase):
    ...
    def maybe_init_dist(self) -> None:
        ...
        if self._dist_inited:
            return
        # pylint: disable=C0415
        from tests.st.utils import init_dist_gloo
        init_dist_gloo()
        self._dist_inited = True
