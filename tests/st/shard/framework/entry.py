...
from tests.st.shard.framework.backend import resolve_backend
...
from tests.st.shard.framework.reporter import Reporter
from tests.st.shard.framework.utils import (
    ENV_CASE_NAMES,
    ENV_CASES_PKG,
    ENV_DEVICE_TYPE,
    ENV_FAIL_FAST,
...
_PLATFORM_FRAMEWORK_PKG = {
    "torch": "tests.st.shard.ops.framework",
}
...
def _require_env(key: str) -> str:
    ...
    if not val:
        raise RuntimeError(
            ...
            f"tests.st.shard.framework.runner.RUNNER?"
        )
    return val
