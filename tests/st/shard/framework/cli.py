...
Pytest-style (file path, with optional ::case_name)::
    ...
    python -m tests.st.shard.framework tests/st/.../case_sort.py
    python -m tests.st.shard.framework tests/st/.../case_sort.py::sort_2d_dp_last_dim

Flag-style::

    python -m tests.st.shard.framework --case sort_2d_dp_last_dim
    python -m tests.st.shard.framework --case-glob "sort_*" --tag sort
...
from tests.st.shard.framework.case_spec import OpShardCase
from tests.st.shard.framework.registry import load_cases_from_package
from tests.st.shard.framework.runner import RUNNER
from tests.st.shard.framework.utils import match_filter

_FW_CFG: Dict[str, dict] = {
    "torch": {
        "cases_pkg": "tests.st.shard.ops.cases",
        "platform_pkg": ["tests.st.shard.ops.framework"],
        "device_type": "cpu",
    },
}
...
def _module_from_path(file_path: str) -> str:
    """Convert ``tests/st/shard/ops/cases/case_sort.py`` → module name."""
...
def main(argv: List[str] = None) -> int:
    ...
    p = argparse.ArgumentParser(
        prog="python -m tests.st.shard.framework",
