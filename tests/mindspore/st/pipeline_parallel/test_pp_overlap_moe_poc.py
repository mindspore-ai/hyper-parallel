...
def test_pp_overlap_moe_accuracy_edge_p2p() -> None:
    """Per-edge PP groups — numerical equivalence and deadlock smoke test.

    Feature: ``p2p_transport="edge"`` keeps same-peer duplex batching while
        routing each adjacent PP rank pair through its own two-rank group.
    Description:
        Launch the distributed overlap PoC using per-edge P2P groups.
    Expectation:
        No group-init or P2P matching deadlock; losses and grads match the sync
        baseline within ``rtol=1e-3, atol=1e-3``.
    """
    msrun_case(3, PP_OVERLAP_MOE_POC, "test_pp_overlap_moe_accuracy_edge_p2p",
