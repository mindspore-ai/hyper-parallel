...
def test_pp_overlap_moe_accuracy_edge_p2p() -> None:
    """Numerical equivalence with a fixed two-rank group for each PP edge.

    Feature: ``p2p_transport="edge"`` keeps the same coalesced per-peer batch
    ...
    Description:
        Run the overlap schedule with per-edge communication groups and compare
    ...
    """
    _test_pp_overlap_moe_accuracy_batched_p2p("edge", "edge_p2p")
