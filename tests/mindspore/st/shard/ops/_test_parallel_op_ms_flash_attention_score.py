...
def create_attention_mask(sparse_mode: Any) -> None:
...
def run_standalone_bsh(sparse_mode: Any=0, scalar_value: Any=SCALE, keep_prob: Any=1.0,
...
def bnsd_tensors() -> None:
    """Create BNSD format tensors."""
    def to_bnsd(data: Any) -> None:
...
def sbh_tensors() -> None:
    """Create SBH format tensors."""
    def to_sbh(data: Any) -> None:
...
def bsnd_tensors() -> None:
    """Create BSND format tensors."""
    def to_bsnd(data: Any) -> None:
...
def tnd_tensors(num_samples: Any=BATCH_SIZE, tokens_per_sample: Any=SEQ_LEN) -> None:
    """Create TND format tensors and cumulative sequence lengths."""
    total = num_samples * tokens_per_sample
    def to_tnd(data: Any) -> None:
        return np.ascontiguousarray(
            data.reshape(BATCH_SIZE, SEQ_LEN, HEAD_NUM, HEAD_DIM)
...
def scaled_dot_product_attention(q: Any, k: Any, v: Any, scalar_value: Any) -> None:
