...
# ============================================================================
"""MindFormers ST for npu_dense_lightning_indexer_grad_kl_loss custom op."""
import numpy as np
...
def _generate_inputs(layout="BSND"):
    ...
    qi_np = np.random.randn(*batch_seq1, N1INDEX, HEAD_DIM).astype(np.float16)
    ki_np = np.random.randn(*batch_seq2, N2INDEX, HEAD_DIM).astype(np.float16)
    ...
    return q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np
...
def _get_sm_stats(q_np, k_np, layout):
    ...
    fa_out = fa_op(Tensor(fa_q), Tensor(fa_k), Tensor(np.zeros(fa_k.shape, np.float16)),
                   None, None, None, None)
    sm_max_np = fa_out[0].amax(-1, True).asnumpy()
    sm_sum_np = fa_out[1].sum(dim=-1, keepdim=True).asnumpy()

    if layout == "TND":
        sm_max_np = sm_max_np.transpose(1, 0, 2, 3).reshape(N2, BATCH * S1, 1)
        sm_sum_np = sm_sum_np.transpose(1, 0, 2, 3).reshape(N2, BATCH * S1, 1)
    return sm_max_np, sm_sum_np


def _get_sm_index(qi_np, ki_np, w_np, layout):
    """Compute softmax index stats via npu_dense_lightning_indexer_softmax_lse."""
    if layout == "TND":
        qlen = Tensor(_GLOBAL_QLEN, dtype=ms.int32)
        klen = Tensor(_GLOBAL_KLEN, dtype=ms.int32)
        sm_max_idx, sm_sum_idx = npu_dense_lightning_indexer_softmax_lse(
            Tensor(qi_np), Tensor(ki_np), Tensor(w_np),
    ...
    return sm_max_idx.asnumpy(), sm_sum_idx.asnumpy()


def _foward_fn(q, k, qi, ki, w, sm_max, sm_sum, sm_max_idx, sm_sum_idx,
               qr, kr, **kwargs):
    out = npu_dense_lightning_indexer_grad_kl_loss(
...
def _run_standalone(q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np,
                    sm_max_np, sm_sum_np, sm_max_idx_np, sm_sum_idx_np, layout="BSND"):
    ...
    sm_max_idx = Tensor(sm_max_idx_np)
    sm_sum_idx = Tensor(sm_sum_idx_np)

    actual_qlen = Tensor(_GLOBAL_QLEN, dtype=ms.int32) if layout == "TND" else None
    actual_klen = Tensor(_GLOBAL_KLEN, dtype=ms.int32) if layout == "TND" else None

    fwd_out = npu_dense_lightning_indexer_grad_kl_loss(
        q, k, qi, ki, w, sm_max, sm_sum, sm_max_idx, sm_sum_idx, SCALE,
        query_rope=qr, key_rope=kr,
    ...
    if layout == "TND":
        grad = ms.grad(_foward_fn, (2, 3, 4))(
            q, k, qi, ki, w, sm_max, sm_sum, sm_max_idx, sm_sum_idx,
            qr, kr,
            actual_seq_qlen=actual_qlen, actual_seq_klen=actual_klen, layout=layout,
        )
    else:
...
def _assert_fwd(results, ref_out, tag):
    ...
    for i, (name, ref) in enumerate(zip(_FWD_NAMES, ref_out)):
        ...
        _atol = 5e-2 if name == 'd_key_index' else 1e-3  # pylint: disable=invalid-name
        assert np.allclose(dist, ref, atol=_atol, rtol=1e-3), (
...
def _assert_bwd(raw_grads, d_inputs, ref_grad, tag):
    ...
    for name, raw_g, d_inp, ref, g_placements in zip(
        ...
        _atol = 5e-2 if name == 'd_key_index' else 1e-3  # pylint: disable=invalid-name
        assert np.allclose(dist_g, ref, atol=_atol, rtol=1e-3), (
            f"{tag} backward {name} mismatch: max_diff={np.abs(dist_g - ref).max()}"
        )

def test_grad_kl_loss_bsnd_replicated():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss BSND replicated on 2-device mesh.
    Description:
    ...
    """
    q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np = _generate_inputs(layout="BSND")
    sm_max_np, sm_sum_np = _get_sm_stats(q_np, k_np, layout="BSND")
    sm_max_idx_np, sm_sum_idx_np = _get_sm_index(qi_np, ki_np, w_np, layout="BSND")
    ref_out, ref_grad = _run_standalone(q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np,
                                        sm_max_np, sm_sum_np, sm_max_idx_np, sm_sum_idx_np)
    ...
    sm_max_idx = distribute_tensor(Tensor(sm_max_idx_np), mesh, (Replicate(),))
    sm_sum_idx = distribute_tensor(Tensor(sm_sum_idx_np), mesh, (Replicate(),))
...
def test_grad_kl_loss_bsnd_dp():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss BSND with batch-dimension DP.
    Description:
    ...
    sm_max_idx = distribute_tensor(Tensor(sm_max_idx_np), mesh, (Shard(0),))
    sm_sum_idx = distribute_tensor(Tensor(sm_sum_idx_np), mesh, (Shard(0),))
...
def test_grad_kl_loss_bsnd_dp_cp():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss BSND with dp=2, cp=2.
    Description:
    ...
    sm_max_idx = distribute_tensor(Tensor(sm_max_idx_np), mesh, (Shard(0), Shard(2)))
    sm_sum_idx = distribute_tensor(Tensor(sm_sum_idx_np), mesh, (Shard(0), Shard(2)))
...
def test_grad_kl_loss_tnd_dp():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss TND with T-dimension DP.
    Description:
    ...
    """
    q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np = _generate_inputs(layout="TND")
    sm_max_np, sm_sum_np = _get_sm_stats(q_np, k_np, layout="TND")
    sm_max_idx_np, sm_sum_idx_np = _get_sm_index(qi_np, ki_np, w_np, layout="TND")
    ref_out, ref_grad = _run_standalone(q_np, k_np, qi_np, ki_np, w_np, qr_np, kr_np,
                                        sm_max_np, sm_sum_np, sm_max_idx_np, sm_sum_idx_np,
                                        layout="TND")
    ...
    sm_max_idx = distribute_tensor(Tensor(sm_max_idx_np), mesh, (Shard(1),))
    sm_sum_idx = distribute_tensor(Tensor(sm_sum_idx_np), mesh, (Shard(1),))
    qr = distribute_tensor(Tensor(qr_np), mesh, (Shard(0),))
    kr = distribute_tensor(Tensor(kr_np), mesh, (Shard(0),))
...
def test_grad_kl_loss_tnd_dp_cp():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss TND with dp=2, cp=2.
    Description:
    ...
    sm_max_idx = distribute_tensor(Tensor(sm_max_idx_np), mesh, (Shard(1), Shard(1)))
    sm_sum_idx = distribute_tensor(Tensor(sm_sum_idx_np), mesh, (Shard(1), Shard(1)))
...
def test_grad_kl_loss_tnd_replicated():
    """
    Feature: npu_dense_lightning_indexer_grad_kl_loss TND replicated on 2-device mesh.
    Description:
