...
IMPL_FILE = str(
    Path(__file__).resolve().parent / "_test_parallel_op_npu_sparse_lightning_indexer_grad_kl_loss.py"
)
...
def test_sparse_grad_kl_loss_group1():
    ...
    parallel_run([
        MindSporeCase(
            IMPL_FILE, "test_sparse_grad_kl_loss_bsnd_replicated",
            worker_num=2, local_worker_num=2, glog_v=2,
        ),
        MindSporeCase(IMPL_FILE, "test_sparse_grad_kl_loss_bsnd_dp", worker_num=2, local_worker_num=2, glog_v=2),
        MindSporeCase(IMPL_FILE, "test_sparse_grad_kl_loss_bsnd_dp_cp", worker_num=4, local_worker_num=4, glog_v=2),
    ])
...
def test_sparse_grad_kl_loss_group2():
    ...
    """
    parallel_run([
        MindSporeCase(IMPL_FILE, "test_sparse_grad_kl_loss_tnd_replicated", worker_num=2, local_worker_num=2, glog_v=2),
        MindSporeCase(IMPL_FILE, "test_sparse_grad_kl_loss_tnd_dp", worker_num=2, local_worker_num=2, glog_v=2),
        MindSporeCase(IMPL_FILE, "test_sparse_grad_kl_loss_tnd_dp_cp", worker_num=4, local_worker_num=4, glog_v=2),
    ])
