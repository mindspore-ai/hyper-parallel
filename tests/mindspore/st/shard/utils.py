...
def distribute_tensor(tensor: Tensor,
    ...
    mesh_shape = layout.mesh
    dev_mat_aliases = mesh_shape.alias_name
    sharded_dim_set = set()
    for dim, shard_alias in enumerate(shardmap_alias_names):
        if shard_alias != "None":
            sharded_dim_set.add(shard_alias)
            num_chunk = mesh_shape.get_device_num_along_axis(shard_alias)
            ...
            local_tensor = mesh_scatter(output, scatter_list, mesh_shape, shard_alias, src_data_rank)
