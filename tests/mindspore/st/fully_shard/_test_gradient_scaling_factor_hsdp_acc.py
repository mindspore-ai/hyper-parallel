...
# ============================================================================
"""MindSpore ST: set_gradient_scaling_factor under HSDP + grad accumulation
+ replicate_params (pure all-reduce path).

Mirrors the torch case. Validates that the scaling factor is applied exactly
once to every managed parameter's gradient -- including replicate_params whose
grads only go through all-reduce (no reduce-scatter) -- and that under gradient
accumulation the result is ``sum_i(g_i * factor)``.
...
for both sharded and replicate parameters (reduce-op agnostic ratio check).
"""
...
from hyper_parallel import SkipDTensorDispatch, init_device_mesh, DeviceMesh
...
class C(nn.Cell):
    ...
    def __init__(self, dim: int) -> None:
        super().__init__()
        self.lin_c = nn.Dense(dim, dim, weight_init="normal", bias_init="zeros")
...
class B(nn.Cell):
    ...
    def __init__(self, dim: int) -> None:
        super().__init__()
        self.lin_b = nn.Dense(dim, dim, weight_init="normal", bias_init="zeros")
        self.module_c = C(dim)
...
def _collect_grads(model):
    ...
    for key, grad in raw.items():
        ...
        out[key] = local.asnumpy().copy()
    return out


def _discover_replicate_params(model):
    """Params under module_b.module_c are kept replicated."""
    return {p for _, p in model.module_b.module_c.parameters_and_names()}
...
def _build_model(mesh: DeviceMesh, factor):
    ...
    expected = factor if factor is not None else None
    assert _get_gradient_scaling_factor(model) == expected
    return model
...
def _run_accumulated_grads(mesh: DeviceMesh, factor, micro_steps: int):
    ...
    dim = 8
    np.random.seed(84)
    inputs = [Tensor(np.random.randn(dim, dim).astype(np.float32)) for _ in range(micro_steps)]
...
def _run_multi_reduce_cycle_grads(mesh: DeviceMesh, factor, num_cycles: int):
    ...
    micro-batch accumulation in ``_run_accumulated_grads`` -- and is exactly the
    case the ``apply_reduced_grad`` note guards: the factor must scale each
    ...
    dim = 8
    np.random.seed(126)
    inputs = [Tensor(np.random.randn(dim, dim).astype(np.float32)) for _ in range(num_cycles)]
...
def test_ms_gradient_scaling_factor_hsdp_grad_accumulation():
    ...
    Description: 4-card HSDP mesh (2x2). module_c params are replicate_params
        ...
        every cycle reduces and the sharded grads accumulate across reduce cycles
        (apply_reduced_grad's += branch). Compares scaled vs baseline grads.
    ...
    micro_steps = 3
    baseline = _run_accumulated_grads(hsdp_mesh, None, micro_steps)
    for factor in (2.0, 0.5, 3):
        scaled = _run_accumulated_grads(hsdp_mesh, factor, micro_steps)
        _assert_ratio(baseline, scaled, factor)
    factor_tensor = Tensor(0.25, dtype=ms.float32)
    scaled_t = _run_accumulated_grads(hsdp_mesh, factor_tensor, micro_steps)
    _assert_ratio(baseline, scaled_t, 0.25)

    _run_multi_cycle_check(hsdp_mesh)

    if rank_id == 0:
        print("ms gradient_scaling_factor HSDP grad-accumulation case passed.")
