...
# ============================================================================
"""Contract tests for the iterable placements ``hp_build_tp_grad_info`` returns.
...
def _build(model, plan, *, unwrapped: bool = True):
    """Call ``hp_build_tp_grad_info`` with the parameter unwrap stubbed.
    ...
    try:
        return runtime.hp_build_tp_grad_info(
...
def test_codegen_path_delegates_to_the_native_builder(monkeypatch):
    ...
    result = runtime.hp_build_tp_grad_info(
        model, plan, DENSE_SOURCE_MESH, EXPERT_SOURCE_MESH
    )
