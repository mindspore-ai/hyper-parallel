...
class TestPlanTargetInference(unittest.TestCase):
    ...
    def test_inferred_and_explicit_produce_identical_artifact(self):
        ...
        # The EP guard and the expert group come from the instance-attribute
        # channel the runtime binds at install time — no module-level accessor.
        self.assertIn("if not self.ep_enable:", inferred_spec.body_template)
        self.assertIn("ep_group=self._hyper_ep_group", inferred_spec.body_template)
        self.assertNotIn("get_parallel_state", inferred_spec.body_template)
        self.assertIsNone(re.search(r"\bps\.", inferred_spec.body_template))
