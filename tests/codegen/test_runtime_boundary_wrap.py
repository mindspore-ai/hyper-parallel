...
(``hp_install_boundaries``) and binds the ``InstalledBoundary`` to the
module instance; the generated forwards call
``self._hp_boundary.redistribute_inputs/outputs`` with no per-call
...
class FakeEpGroup:
    """Process-group double: reports a multi-rank expert-parallel group."""

    def size(self):
        return 2


class FakeExpertMesh:
    """Expert-mesh double exposing the ``ep`` group an inlined EP shell reads."""

    def get_group(self, name=None):
        return FakeEpGroup() if name == "ep" else None
...
def test_install_wraps_unlowered_boundary(monkeypatch):
    ...
    Description: When ``_compile_boundary`` cannot lower (returns identity),
        ``hp_install_boundaries`` wraps the module's forward with the
...
def test_install_binds_lowered_forward_without_wrapping(monkeypatch):
    """A lowered boundary binds ``_hp_boundary`` without extra wrapping.

    Feature: boundary-install
    Description: When the module already has a lowered forward (``_forward_impl``),
        only the ``_hp_boundary`` attribute is bound; no wrapper is added.
    Expectation: No ``_codegen_boundary_wrapped`` tag; ``_hp_boundary`` is set
    ...
    assert isinstance(model.self_attn._hp_boundary, TagBoundary)
    assert model.self_attn._hp_boundary.module is model.self_attn


@arg_mark(plat_marks=["cpu_linux", "cpu_windows"], level_mark="level0",
          card_mark="onecard", essential_mark="unessential")
def test_install_does_not_bind_rewrap_for_region_boundaries(monkeypatch):
    """Region FQNs (``local_compute_fn``) do not get ``module._hp_rewrap``.

    Feature: boundary-install
    Description: For local-region boundaries, production generated forwards keep
        compute outputs local and let the boundary exit own any lowered or
        fallback redistribution.
    Expectation: ``_hp_boundary`` is installed, but no stale
        ``_hp_rewrap`` helper is bound.
    """
    model = Root()
    monkeypatch.setattr(runtime, "InstalledBoundary", TagBoundary)
    ...
    assert isinstance(model.self_attn._hp_boundary, TagBoundary)
    assert not hasattr(model.self_attn, "_hp_rewrap")
...
def test_install_routing_covers_runtime_conditional_branches(monkeypatch):
    ...
    Runtime-conditional mesh routing must stay in ``hp_install_boundaries``
    ...
    assert not compiled
    installed = model.embed_tokens._hp_boundary
...
def test_installed_redistribute_inputs_takes_pair_payload(monkeypatch):
    ...
    func = FunctionInfo(
        name="forward", body_start=0, body_end=0,
...
def test_install_static_tp_collective_binds_operators_on_match(monkeypatch):
    """A form match binds ``_hp_tp`` and leaves the static forward in place.

    Feature: boundary-install
    Description: When the emitted marker, the live-mesh re-classification,
        and the TP lowerer all agree, ``hp_install_boundaries`` binds the
        bare-operator runtime (``module._hp_tp``) and does NOT touch the
        generated static forward.
    Expectation: ``_hp_tp`` is a ``TPOperators`` carrying the lowerer; the
    ...
    assert isinstance(model.self_attn._hp_tp, runtime.TPOperators)
    assert model.self_attn._hp_tp._lowerer is lowerer
...
def test_install_static_tp_collective_falls_back_when_lowerer_unavailable(
    ...
    Description: ``create_tp_collective_lowerer`` returns ``None`` when the tp
        ...
        the baked ``self._hp_tp`` calls would then crash, so the install
        path must replace the forward with the generic redistribute engine.
    Expectation: ``_hp_tp`` is not bound; the instance carries a replaced
...
def test_install_static_tp_collective_falls_back_when_live_axes_disagree(
    ...
    Expectation: ``_hp_tp`` is not bound; the forward is replaced.
    """
    from hyper_parallel.distributed._builder import tp_collective_lowering
    ...
    assert not hasattr(module, "_hp_tp")
...
def test_install_leaves_unmarked_lowered_boundary_alone(monkeypatch):
    ...
    Description: Generic / region lowered forwards (no
        ``_hp_boundary_form`` marker) never enter the static validation —
        they already route through ``self._hp_boundary``.
    Expectation: No ``_hp_tp`` binding and no instance-level forward.
    """
    from hyper_parallel.distributed._builder import tp_collective_lowering
...
@arg_mark(plat_marks=["cpu_linux", "cpu_windows"], level_mark="level0",
          card_mark="onecard", essential_mark="unessential")
def test_bind_inline_state_binds_the_ep_channel(monkeypatch):
    """The inlined EP shell gets the one channel its body reads.

    Feature: codegen-external-state
    Description: A class the artifact declared in ``meta.external_state_classes``
        has its forward replaced by the inline EP strategy body, so the
        boundary / compute / inner-wrapper installers skip it; the parallel
        state that body reads (``self.ep_enable`` / ``self._hp_ep_group``)
        must come from ``hp_bind_inline_state``, bound on the instance from
        the live expert mesh.  No TP / CP channel is bound: a class that is
        *not* an inline strategy shell keeps the emitted boundary form, whose
        TP operators and CP wrapper come from the shared install paths.
    Expectation: The declared instance carries the resolved EP group with
        ``ep_enable`` True and no ``_hp_tp`` / ``tp_enable`` /
        ``_hp_cp_mesh`` / ``cp_enable``; an undeclared class of the same
        generated module is left untouched, a bind without an expert mesh turns
        the guard off, and no mesh is resolved when nothing matches.
    """
    generated = types.ModuleType("hyper_parallel_generated.inline_state")
    declared = type(
        "InlineMoeShell", (nn.Module,), {"__module__": generated.__name__}
    )
    undeclared = type("OtherBlock", (nn.Module,), {"__module__": generated.__name__})
    runtime.register_external_state_classes(generated, ("InlineMoeShell",))

    expert_mesh = FakeExpertMesh()
    calls: list[Any] = []

    def fake_resolve(mesh_context, mesh_dim_names=None):
        calls.append((mesh_context, mesh_dim_names))
        return object(), None, expert_mesh, ("ep",)

    monkeypatch.setattr(runtime, "_resolve_runtime_meshes", fake_resolve)

    model = nn.Module()
    model.declared = declared()
    model.undeclared = undeclared()
    runtime.hp_bind_inline_state(
        model,
        mesh_context="mesh",
        mesh_dim_names=("ep",),
    ...
    assert isinstance(model.declared._hp_ep_group, FakeEpGroup)
    assert model.declared.ep_enable is True
    for absent in ("_hp_tp", "tp_enable", "_hp_cp_mesh", "cp_enable"):
        assert not hasattr(model.declared, absent)
    assert not hasattr(model.undeclared, "ep_enable")
    assert calls == [("mesh", ("ep",))]

    # No expert mesh on the live topology (or a single-rank group): guard off.
    monkeypatch.setattr(
        runtime, "_resolve_runtime_meshes", lambda *a, **k: (object(), None, None, ())
    )
    runtime.hp_bind_inline_state(
        model,
        mesh_context="mesh",
    ...
    assert model.declared._hp_ep_group is None
    assert model.declared.ep_enable is False

    # A model with no declared instance never resolves the mesh at all.
    calls.clear()
    monkeypatch.setattr(runtime, "_resolve_runtime_meshes", fake_resolve)
    untouched = nn.Module()
    untouched.block = undeclared()
    runtime.hp_bind_inline_state(
        untouched,
        mesh_context=None,
        mesh_dim_names=("ep",),
...
class RenderedAttention(nn.Module):
    """Rendered component class: the emitted boundary form owns its forward."""
...
class Qwen3MoeSparseMoeBlock(nn.Module):
    """Inline strategy shell: its forward was replaced by the inline EP body.

    The class name is the EP archetype's block class — the structural fact the
    artifact's ``external_state_classes`` record carries (see
    :func:`_component_module`), not a convenience label.
    """
...
#: The EP compute factory whose archetype names the inline shell class.
_QWEN_EP_FN = (
...
def _component_module():
    """A generated module carrying one rendered component and one EP shell.

    The artifact's own record of which class takes its state externally is
    derived the way generation derives it (the injections' resolved strategy
    specs), so the rendered component's absence from that record is the
    production fact these tests exercise.
    """
    generated = types.ModuleType("hyper_parallel_generated.rendered_component")
    for cls in (RenderedAttention, Qwen3MoeSparseMoeBlock):
        cls.__module__ = generated.__name__
    meta = SimpleNamespace(
        source={"architecture": "Qwen3MoeForCausalLM"},
        injections=[{
            "match": "model.layers.0.mlp",
            "local_compute_fn": {"_target_": _QWEN_EP_FN},
        }],
    )
    manager._record_inline_declarations(meta)  # pylint: disable=protected-access
    assert meta.external_state_classes == ["Qwen3MoeSparseMoeBlock"]
    runtime.register_external_state_classes(generated, meta.external_state_classes)
    return generated


_WRAPPED_TARGETS: list[str] = []


@inner_wrapper
def _recording_cp_wrapper(target_module, mesh, tp_mesh, cp_mesh, ep_mesh):
    """In-place inner-wrapper double recording the module it is applied to."""
    del mesh, tp_mesh, cp_mesh, ep_mesh
    _WRAPPED_TARGETS.append(type(target_module).__name__)


@arg_mark(plat_marks=["cpu_linux", "cpu_windows"], level_mark="level0",
          card_mark="onecard", essential_mark="unessential")
def test_rendered_component_reuses_the_compiled_boundary(monkeypatch):
    """A rendered component is compiled like any other boundary.

    Feature: codegen-external-state
    Description: The rendered component class (the fused attention the family's
        factories build) is not an external-state class, so
        ``hp_install_boundaries`` treats it like any other source-lowered
        boundary: it compiles and binds ``_hp_boundary`` without adding a
        second forward wrapper.  The inline EP shell of the same artifact is
        still skipped.
    Expectation: The rendered component owns a compiled boundary; the shell
        carries none; the component's lowered forward is left in place.
    """
    generated = _component_module()
    monkeypatch.setattr(runtime, "InstalledBoundary", TagBoundary)

    model = nn.Module()
    model.self_attn = RenderedAttention()
    model.mlp = Qwen3MoeSparseMoeBlock()
    runtime.hp_install_boundaries(
        model,
        {"self_attn": _entry(), "mlp": _entry()},
        mesh_context="mesh",
        mesh_dim_names=("tp",),
        generated_module=generated,
    )
    ...
    assert isinstance(model.self_attn._hp_boundary, TagBoundary)
    assert model.self_attn._hp_boundary.module is model.self_attn
    # The emitted forward already redistributes; no second wrapper is added.
    assert not hasattr(model.self_attn, "_codegen_boundary_wrapped")
    # The inline shell keeps its own channel instead of a compiled boundary.
    assert not hasattr(model.mlp, "_hp_boundary")


@arg_mark(plat_marks=["cpu_linux", "cpu_windows"], level_mark="level0",
          card_mark="onecard", essential_mark="unessential")
def test_apply_inner_wrapper_carries_cp_to_the_rendered_component(monkeypatch):
    """CP is installed by the shared inner-wrapper chain, not a codegen copy.

    Feature: codegen-inner-wrapper
    Description: The rendered component class is not an external-state class,
        so ``hp_apply_inner_wrapper`` reaches it and applies the declared CP
        wrapper through the applier's own resolution chain
        (``_wrap_inner_attention`` → ``_resolve_inner_wrapper`` →
        ``_apply_custom_inner_wrapper``).  The inline EP shell of the same
        artifact is still skipped.
    Expectation: The declared wrapper runs for the rendered component only.
    """
    generated = _component_module()
    monkeypatch.setattr(
        runtime,
    ...
    model.self_attn = RenderedAttention()
    model.mlp = Qwen3MoeSparseMoeBlock()
    runtime.hp_apply_inner_wrapper(
        model,
        [
            {"match": "self_attn", "inner_target": "self",
             "inner_wrapper": _recording_cp_wrapper},
            {"match": "mlp", "inner_target": "self",
             "inner_wrapper": _recording_cp_wrapper},
        ],
        mesh_context="mesh",
        mesh_dim_names=(),
        param_plan={
            "self_attn": {"region_dispatch": False},
            "mlp": {"region_dispatch": False},
        },
        generated_module=generated,
    )
...
class LoweredBiased(nn.Module):
    ...
    guarantees (redistribute → ``hp_to_local_if_dtensor`` → re-add,
    ...
    def forward(self, x):
        args, kwargs = self._hp_boundary.redistribute_inputs(((x,), {}))
...
def test_install_deferred_bias_pair_on_imported_class(monkeypatch):
    ...
    Description: An entry with ``deferred_bias_params`` installs the native
        ...
        plus the ``_hp_deferred_bias`` exit hook; the boundary forward
    ...
    assert callable(model.self_attn._hp_deferred_bias)
...
def test_install_deferred_bias_pair_on_lowered_boundary(monkeypatch):
    ...
    Description: A lowered boundary (``_forward_impl``) with a deferred child
        bias only binds ``_hp_boundary`` and the suppression pair — the
        emitted forward's own ``self._hp_deferred_bias`` line performs the
    ...
    assert not hasattr(model.self_attn, "_codegen_boundary_wrapped")
    assert isinstance(model.self_attn._hp_boundary, BiasExitBoundary)
    out = model.self_attn(torch.tensor([[1.0, 1.0]]))
    assert torch.equal(out, torch.tensor([[7.0, 9.0]]))
    installed = model.self_attn._hp_boundary
    assert installed.sides == ["in", "out"]
...
def test_install_deferred_bias_self_owned_wraps_impl_not_forward(monkeypatch):
    ...
    Description: When the boundary's OWN bias is deferred, a source-lowered
        ...
        ``self._hp_deferred_bias(outputs)`` reads it at the exit — the
...
def test_install_rejects_deferred_bias_on_external_state_inline(monkeypatch):
    ...
    Expectation: ``hp_install_boundaries`` fails fast with a
...
def test_static_tp_fallback_preserves_deferred_bias(monkeypatch):
    ...
    Description: When the live mesh rejects the static ``tp_collective``
        ...
        ``self._hp_deferred_bias`` line.  The suppression pair is
        installed before the fallback wraps, so the fallback re-applies the
        hook after its exit redistribution.
    Expectation: ``_hp_tp`` is not bound, the forward is replaced, and the
