...
def test_install_does_not_bind_rewrap_for_region_boundaries(monkeypatch):
    """Region FQNs (``local_compute_fn``) do not get ``module._hyper_rewrap``.
    ...
    Expectation: ``_hyper_boundary`` is installed, but no stale
        ``_hyper_rewrap`` helper is bound.
    """
    model = Root()
    monkeypatch.setattr(runtime, "InstalledBoundary", TagBoundary)
    ...
    assert isinstance(model.self_attn._hyper_boundary, TagBoundary)
    assert not hasattr(model.self_attn, "_hyper_rewrap")
...
def test_bind_inline_state_binds_the_ep_channel(monkeypatch):
    ...
    Description: A class the artifact declared in ``meta.external_state_classes``
        ...
        state that body reads (``self.ep_enable`` / ``self._hyper_ep_group``)
        must come from ``hyper_bind_inline_state``, bound on the instance from
    ...
    Expectation: The declared instance carries the resolved EP group with
        ``ep_enable`` True and no ``_hyper_tp`` / ``tp_enable`` /
        ``_hyper_cp_mesh`` / ``cp_enable``; an undeclared class of the same
    ...
    runtime.hyper_bind_inline_state(
        model,
        mesh_context="mesh",
        mesh_dim_names=("ep",),
    ...
    assert isinstance(model.declared._hyper_ep_group, FakeEpGroup)
    assert model.declared.ep_enable is True
    for absent in ("_hyper_tp", "tp_enable", "_hyper_cp_mesh", "cp_enable"):
    ...
    assert model.declared._hyper_ep_group is None
...
def test_rendered_component_reuses_the_compiled_boundary(monkeypatch):
    ...
    Description: The rendered component class (the fused attention the family's
        ...
        ``hyper_install_boundaries`` treats it like any other source-lowered
        boundary: it compiles and binds ``_hyper_boundary`` without adding a
    ...
    assert not hasattr(model.mlp, "_hyper_boundary")
...
def test_apply_inner_wrapper_carries_cp_to_the_rendered_component(monkeypatch):
    ...
    Description: The rendered component class is not an external-state class,
        so ``hyper_apply_inner_wrapper`` reaches it and applies the declared CP
    ...
    model.self_attn = RenderedAttention()
    model.mlp = Qwen3MoeSparseMoeBlock()
    runtime.hyper_apply_inner_wrapper(
        model,
        [
