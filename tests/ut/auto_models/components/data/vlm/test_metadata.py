...
from hyper_parallel.auto_models.components.datasets.batching import FixedBatchDataLoader, build_dataloader
...
class TestVLMMetadata(unittest.TestCase):
    ...
    def test_base_forwards_explicit_metadata_callback(self) -> None:
        ...
        with patch(
                "hyper_parallel.auto_models.trainer.base.build_dataloader", return_value=((None,) * 3, (None,) * 3),
        ) as build:
    ...
    def test_omitted_load_balance_keeps_native_loader(self) -> None:
        ...
        target = Target(
            ...
            target_path="hyper_parallel.auto_models.components.datasets.batching.FixedBatchDataLoader",
        )
        with TemporaryDirectory() as directory:
            dataset = build_image_corpus(directory, size=4)
            loaders, samplers = build_dataloader(
