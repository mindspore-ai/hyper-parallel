...
# ============================================================================
"""CPU unit tests for Hyper-RL objectives, KL, and value loss."""
...
def test_loss_reductions_supply_batch_invariant_dimension() -> None:
    """Actor and Critic retain explicit-axis sums required by Ascend kernels."""
    original_sum = torch.Tensor.sum

    def explicit_sum(tensor: torch.Tensor, dim: int, keepdim: bool = False) -> torch.Tensor:
        """Reject implicit reductions just like the batch-invariant operator."""
        return original_sum(tensor, dim=dim, keepdim=keepdim)

    values = torch.zeros((2, 3))
    mask = torch.tensor([[True, False, True], [False, True, False]])
    with patch.object(torch.Tensor, "sum", explicit_sum):
        for name in ("grpo", "ppo"):
            algorithm = build_algorithm({"name": name, "loss_aggregation": "token-mean"})
            actor = algorithm.compute_actor_loss(values, values, values, values, mask)
            torch.testing.assert_close(actor.valid_token_count, torch.tensor(3.0))
        critic = algorithm.compute_critic_loss(values, values, values, mask)
        torch.testing.assert_close(critic.valid_token_count, torch.tensor(3.0))
