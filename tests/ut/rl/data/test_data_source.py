...
def test_train_dataloader_uses_deterministic_distributed_sampler(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Training loader owns rank-local sampling and worker prefetch settings."""
    ...
    torchdata = ModuleType("torchdata")
    loader_module = ModuleType("torchdata.stateful_dataloader")
    loader_module.StatefulDataLoader = Loader
    torchdata.stateful_dataloader = loader_module
    monkeypatch.setitem(sys.modules, "torchdata", torchdata)
    monkeypatch.setitem(sys.modules, "torchdata.stateful_dataloader", loader_module)
