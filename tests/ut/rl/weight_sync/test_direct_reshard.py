    ...
    worker_descriptions = [
        {
            "tp_rank": tp_rank,
            "tp_size": 2,
            "tensors": adapter_module._native_qwen3_direct_tensors(  # pylint: disable=protected-access
...
def test_source_and_destination_metadata_build_complete_reshard_plan() -> None:
    ...
    class Mesh:
        ...
        @staticmethod
        def size(unused_mesh_dim: int) -> int:
            del unused_mesh_dim
            return 2
...
def test_source_metadata_requires_explicit_regions() -> None:
    """Source layouts reject incomplete metadata instead of inferring rank offsets."""
    ...
    with pytest.raises(ValueError, match="requires explicit regions"):
        resolve_source_layouts(descriptions)
