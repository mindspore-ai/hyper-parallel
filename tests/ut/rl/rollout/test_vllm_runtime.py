...
def test_vllm_router_respects_capacity_and_restores_row_order(
    ...
    async def request(unused_method: str, unused_route: str, payload: dict[str, Any]) -> dict[str, Any]:
        del unused_method, unused_route
...
def test_shared_vllm_endpoint_has_one_owner_and_normal_lifecycle(
    ...
    transfer = SimpleNamespace(
        publish=lambda _client, _snapshot: None,
        close=lambda: events.append("transfer-close"),
    )
    ...
    owner = VLLMGenerationEngine(_model(), {"vllm": config}, publisher=transfer)
...
def test_sync_generate_executes_complete_http_generation_pipeline(
    ...
    engine._weight_sync = SimpleNamespace(
        generation_version=lambda used_client: version_calls.append("version") or 4
    )
...
def test_tp_owner_generates_once_and_broadcasts_complete_result(
    ...
    owner._weight_sync = SimpleNamespace(
        generation_version=lambda _client: 3
    )
...
def test_http_client_executes_sync_and_async_requests(
    ...
    class AsyncResponse:
        """Provide the asynchronous response context expected by the HTTP client."""
        status = 200
    ...
    class AsyncSession:
        """Record HTTP requests and session closure without network access."""
        closed = False
...
def test_shared_server_builds_command_and_isolated_environment(
    ...
    transfer = SimpleNamespace(publish=lambda _client, _snapshot: None)
    monkeypatch.setattr(
        vllm_module,
...
def test_engine_computes_child_capacity_and_inprocess_results(
    ...
    class LocalClient:
        """Return deterministic sampled tokens and retain the sampling parameters."""
        def __init__(self) -> None:
            self.sampling: Any = None

        def generate(self, prompts: Any, *, sampling_params: Any, use_tqdm: bool) -> list[Any]:
            """Validate request options and return deterministic per-prompt completions."""
            assert not use_tqdm
    ...
    monkeypatch.setattr(vllm_module.dist, "get_world_size", lambda: 3)
    monkeypatch.setattr(vllm_module.dist, "get_rank", lambda: 1)
    engine = VLLMGenerationEngine(
        _model(),
...
def test_http_client_runs_and_closes_persistent_async_transport(
    ...
    class Loop:
        """Track event-loop start and stop state for service lifecycle tests."""
        def __init__(self) -> None:
            self.running = True
    ...
    class Thread:
        """Run the service target deterministically without creating an OS thread."""
...
def test_http_client_retries_readiness_and_completes_bounded_process_cleanup(
    ...
    class DelayedProcess:
        """Require escalation before reporting a stopped child process."""
        pid = 101
...
def test_http_client_exercises_current_weight_control_protocol(
    ...
    assert client.collective_rpc("get_policy_version") == [{"version": 3}]
    client.resume()
...
def test_http_process_group_helpers_detect_live_and_completed_groups(
    ...
    class Entry:
        ...
        def __truediv__(self, unused_name: str) -> Stat:
            del unused_name
...
def test_http_generate_tokens_routes_unseeded_and_batch_invariant_parents(
    ...
    async def generate_parent(request: Any, unused_settings_value: Any) -> Any:
        del unused_settings_value
...
def test_engine_exposes_weight_sync_strategy_and_streaming_stats() -> None:
    """Rollout forwards the retained publication status."""
