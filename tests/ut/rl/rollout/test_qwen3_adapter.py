...
from tests.ut.rl.rollout.vllm_test_support import CompilationMode, load_adapter_modules

adapter_module, common_module = load_adapter_modules()
...
def test_native_and_hyper_qwen3_select_expected_vllm_runtime(
    ...
    assert registered == {
        "HyperQwen3ForCausalLM": "rl.roles.rollout.models.qwen3.model:HyperQwen3ForCausalLM",
    }
...
def test_hyper_qwen3_adapter_preserves_forward_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class RecordingAttention(torch.nn.Module):
        ...
        def forward(
            ...
            """Return the CPU attention oracle for the supplied query, key and value."""
    ...
    # Initialize only the module shell; the real constructor requires an NPU runtime.
    torch.nn.Module.__init__(wrapper)  # pylint: disable=unnecessary-dunder-call
...
def test_qwen3_adapter_initializes_tiny_supported_model(
    ...
    class AttentionLeaf(torch.nn.Module):
        """Stand in for the paged-attention leaf while preserving module behavior."""
        def __init__(
            self,
            unused_attention: Any,
            *,
            vllm_config: Any,
            prefix: str,
            family: str,
        ) -> None:
            del unused_attention
            super().__init__()
...
def test_qwen3_adapter_loads_complete_tp_weights(
    ...
    # Initialize only the module shell; the real constructor requires an NPU runtime.
    torch.nn.Module.__init__(model)  # pylint: disable=unnecessary-dunder-call
...
def test_hyper_qwen3_accepts_partial_online_weight_buckets(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Online packed publication can call load_weights once per bucket."""
    model = object.__new__(adapter_module.HyperQwen3ForCausalLM)
    # Initialize only the module shell; the real constructor requires an NPU runtime.
    torch.nn.Module.__init__(model)  # pylint: disable=unnecessary-dunder-call
    model.first = torch.nn.Linear(2, 2, bias=False)
    model.second = torch.nn.Linear(2, 2, bias=False)
    ...
    with pytest.raises(ValueError, match="missing parameters"):
        model.load_weights([("first.weight", value)])
    loaded = model.load_weights(
        [("first.weight", value)],
        require_all=False,
    )

    assert loaded == {"first.weight"}
    torch.testing.assert_close(model.first.weight, value)
...
def test_qwen3_adapter_applies_tp_sharding_plan_and_tied_placement(
    ...
    class AttentionLeaf(torch.nn.Module):
        def __init__(self, unused_attention: Any, **_kwargs: Any) -> None:
            del unused_attention
            super().__init__()
