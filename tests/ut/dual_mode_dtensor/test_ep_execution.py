...
class TestRoutedEPExecution(unittest.TestCase):
    """Alternative transports preserve dynamic fallback and model isolation."""

    def test_bind_and_restore_preserves_arguments_and_other_models(self):
        """Binding one expert holder must not change another model's dispatch."""
        model = SimpleNamespace(experts=SimpleNamespace())
        other_model = SimpleNamespace(experts=SimpleNamespace())
        hidden_states = torch.ones(1, 2, 4)
        router_fn = Mock()
        group = object()
        custom = Mock(execute=Mock(return_value=hidden_states * 2))
        with patch.object(ep_utils, "_dynamic_ep_routed_forward") as dynamic:
            previous = ep_utils.bind_routed_ep_execution(model, custom)
            result = ep_utils.ep_routed_forward(
                model, hidden_states, router_fn=router_fn, ep_group=group
            )
            self.assertIs(result, custom.execute.return_value)
            custom.execute.assert_called_once_with(
                model, hidden_states, router_fn=router_fn, ep_group=group
            )
            dynamic.assert_not_called()
            ep_utils.ep_routed_forward(
                other_model, hidden_states, router_fn=router_fn, ep_group=group
            )
            dynamic.assert_called_once_with(
                other_model, hidden_states, router_fn=router_fn, ep_group=group
            )
            replaced = ep_utils.bind_routed_ep_execution(model, previous)
            self.assertIs(replaced, custom)
            ep_utils.ep_routed_forward(
                model, hidden_states, router_fn=router_fn, ep_group=group
            )
            dynamic.assert_called_with(
                model, hidden_states, router_fn=router_fn, ep_group=group
            )
        router_fn.assert_not_called()

    def test_invalid_binding_preserves_previous_operation(self):
        """Reject a malformed transport before mutating the expert holder."""
        model = SimpleNamespace(experts=SimpleNamespace())
        custom = Mock(execute=Mock())
        ep_utils.bind_routed_ep_execution(model, custom)
        with self.assertRaisesRegex(TypeError, "callable execute"):
            ep_utils.bind_routed_ep_execution(model, object())
        previous = ep_utils.bind_routed_ep_execution(model, custom)
        self.assertIs(previous, custom)
...
class TestSwiGLUExpert(unittest.TestCase):
        ...
        for activation in (torch.nn.functional.silu, torch.nn.functional.gelu):
            with self.subTest(activation=activation.__name__):
                ...
                for fused in (False, True):
                    ...
                    actual = ep_utils.compute_swiglu_expert(states, weights, 1, activation)
    ...
    def test_empty_expert_retains_zero_parameter_gradients(self):
        ...
        output = ep_utils.compute_swiglu_expert(
    ...
    def test_default_logits_router_honors_normalization_switch(self):
        ...
        for normalize in (True, False):
            with self.subTest(normalize=normalize):
                ...
                indices, weights = ep_utils.MOE_ROUTER_ADAPTERS["default"](module, torch.ones(1, 1, 4))
    ...
    def test_native_routes_are_returned_without_reweighting(self):
        ...
        for output_type in (tuple, list):
            with self.subTest(output_type=output_type):
                ...
                actual_indices, actual_weights = ep_utils.MOE_ROUTER_ADAPTERS["qwen3moe"](
        ...
        for router_output in outputs:
            with self.subTest(router_output=router_output):
                ...
                with self.assertRaisesRegex(TypeError, "TopKRouter should return"):
                    ep_utils.MOE_ROUTER_ADAPTERS["qwen3moe"](module, torch.ones(1, 1, 4))
                module.gate.assert_called_once()
