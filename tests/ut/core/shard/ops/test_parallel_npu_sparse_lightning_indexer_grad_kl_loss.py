...
# ============================================================================
"""Unit tests for NpuSparseLightningIndexerGradKlLossDistributedOp."""
import os
import unittest
...
from hyper_parallel.core.shard.ops.parallel_npu_sparse_lightning_indexer_grad_kl_loss import (
    NpuSparseLightningIndexerGradKlLossDistributedOp,
    _normalize_sparse_grad_kl_loss_args,
    _to_local,
)
...
class TestNpuSparseLightningIndexerGradKlLoss(unittest.TestCase):
    ...
    @staticmethod
    def _get_op():
        return get_distributed_op("npu_sparse_lightning_indexer_grad_kl_loss")
    ...
    def test_normalize_args_positional_defaults_1(self):
        """
        Feature: _normalize_sparse_grad_kl_loss_args fills in default optional values.
        Description: Call with only the 9 mandatory positional args.
        Expectation: Optional args default to None/BSND/3/INT64_MAX; kwargs empty.
        """
        q, k, qi, ki, w = object(), object(), object(), object(), object()
        si, sm, ss, scale = object(), object(), object(), 1.0
        args, kwargs = _normalize_sparse_grad_kl_loss_args(q, k, qi, ki, w, si, sm, ss, scale)
        self.assertIs(args[0], q)
        self.assertIs(args[2], qi)
        self.assertIs(args[5], si)
        self.assertIs(args[8], scale)
        self.assertIsNone(args[9], msg=f"query_rope default should be None, got {args[9]}")
        self.assertIsNone(args[10], msg=f"key_rope default should be None, got {args[10]}")
        self.assertIsNone(args[11], msg=f"actual_seq_qlen default should be None, got {args[11]}")
        self.assertIsNone(args[12], msg=f"actual_seq_klen default should be None, got {args[12]}")
        self.assertEqual(args[13], 'BSND', msg=f"layout default should be 'BSND', got {args[13]}")
        self.assertEqual(args[14], 3, msg=f"sparse_mode default should be 3, got {args[14]}")
        self.assertEqual(kwargs, {}, msg=f"kwargs should be empty, got {kwargs}")

    def test_normalize_args_returns_17_elements_2(self):
        """
        Feature: _normalize_sparse_grad_kl_loss_args returns 17-element tuple.
        Description: Call with 9 mandatory args and verify tuple length.
        Expectation: len(args) == 17.
        """
        objs = [object() for _ in range(9)]
        args, _ = _normalize_sparse_grad_kl_loss_args(*objs)
        self.assertEqual(len(args), 17, msg=f"Expected 17 positional args, got {len(args)}")

    def test_normalize_args_layout_tnd_3(self):
        """
        Feature: _normalize_sparse_grad_kl_loss_args accepts layout keyword.
        Description: Pass layout='TND' as kwarg.
        Expectation: args[13] == 'TND'.
        """
        objs = [object() for _ in range(9)]
        args, _ = _normalize_sparse_grad_kl_loss_args(*objs, layout='TND')
        self.assertEqual(args[13], 'TND', msg=f"args[13] should be 'TND', got {args[13]}")

    def test_normalize_args_seq_lens_4(self):
        """
        Feature: _normalize_sparse_grad_kl_loss_args passes through seq len lists.
        Description: Pass actual_seq_qlen and actual_seq_klen.
        Expectation: seq len lists land at indices 11 and 12.
        """
        objs = [object() for _ in range(9)]
        qlen, klen = [4, 8], [4, 8]
        args, _ = _normalize_sparse_grad_kl_loss_args(*objs, actual_seq_qlen=qlen, actual_seq_klen=klen)
        self.assertIs(args[11], qlen, msg=f"args[11] should be qlen, got {args[11]}")
        self.assertIs(args[12], klen, msg=f"args[12] should be klen, got {args[12]}")
    ...
    def test_yaml_registration_7(self):
        """
        Feature: YAML loader registers NpuSparseLightningIndexerGradKlLossDistributedOp.
        Description: Call get_distributed_op with the op name.
        Expectation: Returns a non-None NpuSparseLightningIndexerGradKlLossDistributedOp instance.
        """
        op = self._get_op()
        self.assertIsNotNone(op, msg="npu_sparse_lightning_indexer_grad_kl_loss should be registered")
        self.assertIsInstance(
            op, NpuSparseLightningIndexerGradKlLossDistributedOp,
            msg=f"Expected NpuSparseLightningIndexerGradKlLossDistributedOp, got {type(op)}"
        )
    ...
    @patch("hyper_parallel.core.shard.ops.parallel_npu_sparse_lightning_indexer_grad_kl_loss.platform")
    @patch("hyper_parallel.core.dtensor.device_mesh.platform")
    def test_preprocess_ms_17_positional_args_8(self, mock_mesh_plat, mock_op_plat):
        """
        Feature: preprocess packs all 17 args positionally on MindSpore.
        Description: Patch platform type to MINDSPORE, call preprocess with 9 DTensor args.
        Expectation: local_args has 17 elements; local_kwargs is empty; cache_values has 4 elements.
        """
        ...
        dtsi = _mock_dtensor(layout_4d)   # sparse_indices
        dtsm = _mock_dtensor(layout_3d)
        dtss = _mock_dtensor(layout_3d)

        op = self._get_op()
        local_args, local_kwargs, cache_values = op.preprocess(
            (dtq, dtk, dtqi, dtki, dtw, dtsi, dtsm, dtss, 1.0), {}
        )

        self.assertEqual(len(local_args), 17, msg=f"MS path should pack 17 positional args, got {len(local_args)}")
    ...
    @patch("hyper_parallel.core.shard.ops.parallel_npu_sparse_lightning_indexer_grad_kl_loss.platform")
    @patch("hyper_parallel.core.dtensor.device_mesh.platform")
    def test_preprocess_torch_9_positional_args_9(self, mock_mesh_plat, mock_op_plat):
        """
        Feature: preprocess puts optional args in kwargs on PyTorch.
        Description: Patch platform type to PYTORCH, pass layout='TND'.
        Expectation: local_args has 9 elements; optional args are in local_kwargs.
        """
        self._setup_mock_platform(mock_mesh_plat)
        ...
        self.assertEqual(len(local_args), 9, msg=f"Torch path should have 9 positional args, got {len(local_args)}")
    ...
    def test_infer_layout_bsnd_replicated_four_outputs_10(self, mock_platform):
        ...
        Expectation: 4 outputs; d_q/d_k/d_w replicated; loss tensor_map=(-1,).
        """
    ...
    def test_infer_layout_bsnd_dp_only_inherits_input_layouts_11(self, mock_platform):
        ...
        Description: qi/ki sharded on B (dim 0); w sharded on B.
    ...
    def test_infer_layout_bsnd_outputs_are_independent_copies_13(self, mock_platform):
        ...
        Description: Verify all four result objects are distinct from each other and from inputs.
        Expectation: result[0] is not qi; all four objects are distinct.
        """
    ...
    def test_expand_impl_bsnd_cp_returns_callable_35(self):
        ...
        Description: query_index tensor_map[1] != -1 triggers CP path.
