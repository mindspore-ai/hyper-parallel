...
class TestParallelScatterUpdate(unittest.TestCase):
    ...
    def _make_2x2x2x2_mesh(self, mock_platform):
        ...
        return init_device_mesh(device_type="npu", mesh_shape=(2, 2, 2, 2), mesh_dim_names=("dp", "cp", "mp", "tp"))
    ...
    def test_scatter_update_data_parallel_1(self, mock_platform):
        ...
        assert op.get_expand_impl(None, output_layout, 
                                  (input_layout, indices_layout, updates_layout), None) is None, (
            f"get_expand_impl test failed. Expected None, "
            f"""got {op.get_expand_impl(None, output_layout,
                                      (input_layout, indices_layout, updates_layout), None)}"""
        )
    ...
    def test_scatter_update_three_dim_input_5(self, mock_platform):
        ...
        updates_layout = _build_layout(mesh, (Replicate(), Shard(3), Shard(4), Shard(5), Replicate(), Replicate()), 6)
    ...
    def test_scatter_update_error_input_first_dim_sharded_7(self, mock_platform):
        ...
        with self.assertRaisesRegex(ValueError, "first dimension of input cannot be sharded"):
            op.infer_layout((input_layout, indices_layout, updates_layout), None)
