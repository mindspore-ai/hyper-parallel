...
class TestMsFlashAttentionScore(unittest.TestCase):
    ...
    def test_flash_attention_tnd_softmax_wrap_output_rank_match(self, mock_platform):
        ...
        cache_values = self._make_cache_values(
            q_layout, k_layout, v_layout, input_layout="TND"
        )
        output_layouts, _ = op.infer_layout(cache_values)

        # MindSpore output order: softmax_max, softmax_sum, softmax_out, attention_out.
        local_outputs = (
        ...
        def _wrap_with_layout(local_tensor, device_mesh, placements, layout):
            ...
            assert len(local_shape) == len(layout.tensor_map)
        ...
        assert wrapped[SOFTMAX_MAX_IDX].layout.tensor_map == (1, 0, -1)
        assert wrapped[SOFTMAX_SUM_IDX].layout.tensor_map == (1, 0, -1)
        assert len(wrapped[SOFTMAX_MAX_IDX].to_local().shape) == 3
        assert len(wrapped[SOFTMAX_SUM_IDX].to_local().shape) == 3
