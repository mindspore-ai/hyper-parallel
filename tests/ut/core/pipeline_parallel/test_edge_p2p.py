# Copyright 2026 Huawei Technologies Co., Ltd
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ============================================================================
"""Unit tests for per-edge pipeline P2P groups."""

from types import SimpleNamespace
from unittest.mock import patch

import pytest

import hyper_parallel.core.pipeline_parallel.scheduler as scheduler_module
from hyper_parallel.core.pipeline_parallel.scheduler import ScheduleInterleaved1F1B
from tests.common.mark_utils import arg_mark


def _make_runtime(mode: str = "edge") -> ScheduleInterleaved1F1B:
    """Build a scheduler shell without initializing a distributed backend."""
    runtime = object.__new__(ScheduleInterleaved1F1B)
    runtime._p2p_mode = mode
    runtime._p2p_edge_groups = {}
    return runtime


@arg_mark(
    plat_marks=["cpu_linux"],
    level_mark="level0",
    card_mark="onecard",
    essential_mark="essential",
)
def test_init_p2p_edge_groups_uses_mesh_order_and_interleaved_wrap() -> None:
    """
    Feature: Per-edge pipeline P2P group initialization.
    Description: Build edge groups from the ordered PP mesh for an interleaved schedule.
    Expectation: The platform receives the PP ranks in mesh order with wrap enabled.
    """
    runtime = _make_runtime()
    runtime.stages = [SimpleNamespace(mesh=SimpleNamespace(rank_list=(0, 2, 4, 6)), pp_group="unused")]
    runtime.n_local_stages = 2
    expected_groups = {2: "edge-0-2", 6: "edge-0-6"}

    with patch.object(
            scheduler_module.platform,
            "create_p2p_edge_groups",
            return_value=expected_groups,
    ) as create_edge_groups:
        runtime._init_p2p_edge_groups()

    create_edge_groups.assert_called_once_with([0, 2, 4, 6], include_wrap=True)
    assert runtime._p2p_edge_groups == expected_groups, (
        f"Expected edge groups {expected_groups}, got={runtime._p2p_edge_groups}"
    )


@arg_mark(
    plat_marks=["cpu_linux"],
    level_mark="level0",
    card_mark="onecard",
    essential_mark="essential",
)
def test_batched_issue_passes_peer_edge_group_to_platform() -> None:
    """
    Feature: Per-edge pipeline P2P descriptor routing.
    Description: Issue a batched send through the group mapped to its peer.
    Expectation: The P2P descriptor receives the peer's group and returns one batch handle.
    """
    runtime = _make_runtime()
    runtime._p2p_edge_groups = {3: "edge-1-3"}
    tensor = object()
    descriptor = object()
    handle = object()

    with patch.object(scheduler_module.platform, "p2p_op", return_value=descriptor) as p2p_op, \
            patch.object(
                scheduler_module.platform,
                "batch_isend_irecv",
                return_value=handle,
            ) as batch_isend_irecv:
        handles = runtime._batched_issue([("isend", tensor, 3)])

    p2p_op.assert_called_once_with("isend", tensor, 3, "edge-1-3")
    batch_isend_irecv.assert_called_once_with([descriptor])
    assert handles == [handle], f"Expected one batch handle {[handle]}, got={handles}"


@arg_mark(
    plat_marks=["cpu_linux"],
    level_mark="level0",
    card_mark="onecard",
    essential_mark="essential",
)
def test_edge_transport_rejects_peer_without_matching_group() -> None:
    """
    Feature: Per-edge pipeline P2P descriptor validation.
    Description: Request an edge descriptor for a peer absent from the initialized mapping.
    Expectation: The scheduler raises before issuing an operation on the default group.
    """
    runtime = _make_runtime()
    runtime._p2p_edge_groups = {1: "edge-0-1"}

    with pytest.raises(RuntimeError, match="peer global rank 2"):
        runtime._p2p_op("irecv", object(), 2)


@arg_mark(
    plat_marks=["cpu_linux"],
    level_mark="level0",
    card_mark="onecard",
    essential_mark="essential",
)
def test_batch_transport_keeps_default_group() -> None:
    """
    Feature: Existing batch P2P transport compatibility.
    Description: Build a descriptor while the scheduler is using batch transport.
    Expectation: The descriptor keeps a null group and therefore uses the existing default group.
    """
    runtime = _make_runtime(mode="batch")
    tensor = object()

    with patch.object(scheduler_module.platform, "p2p_op", return_value=object()) as p2p_op:
        runtime._p2p_op("irecv", tensor, 5)

    p2p_op.assert_called_once_with("irecv", tensor, 5, None)
