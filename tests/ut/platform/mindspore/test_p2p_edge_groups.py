...
# ============================================================================
"""Unit tests for MindSpore PP edge-group creation."""
...
def test_create_p2p_edge_groups_initializes_only_local_edges() -> None:
    """
    Feature: MindSpore per-edge pipeline P2P groups.
    Description: Initialize edge groups for an interior rank in an interleaved PP ring.
    ...
    with patch.dict(
        ...
        local_groups = MindSporePlatform.create_p2p_edge_groups(
            [0, 2, 4, 6],
            include_wrap=True,
        )
    ...
    assert create_group.call_args_list == [
        call("(0, 2)", [0, 2]),
        call("(2, 4)", [2, 4]),
    ], f"Unexpected MindSpore edge-group creation order: {create_group.call_args_list}"
