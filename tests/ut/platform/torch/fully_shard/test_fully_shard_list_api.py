...
from hyper_parallel.core.fully_shard.api import (
    ...
    _validate_managed_params_tp_grad_infos,
...
from hyper_parallel.core.fully_shard.utils import MixedPrecisionPolicy, TPShardMetaInfo
...
class TestFullyShardListAPI(unittest.TestCase):
    ...
    def test_tp_grad_infos_validation_requires_complete_managed_parameter_map(self):
        ...
        metadata = TPShardMetaInfo(self._create_mock_mesh(), (Replicate(),))

        with self.assertRaisesRegex(ValueError, "cover every parameter"):
            _validate_managed_params_tp_grad_infos(managed, {first: metadata})
        with self.assertRaisesRegex(ValueError, "not managed"):
            _validate_managed_params_tp_grad_infos(managed, {first: metadata, second: metadata, external: metadata})
        with self.assertRaisesRegex(ValueError, "TPShardMetaInfo"):
            _validate_managed_params_tp_grad_infos(managed, {first: metadata, second: object()})
        origin_metadata = TPShardMetaInfo(self._create_mock_mesh(), (Replicate(),), origin_is_dtensor=True)
        with self.assertRaisesRegex(ValueError, "origin_is_dtensor"):
            _validate_managed_params_tp_grad_infos(managed, {first: metadata, second: origin_metadata})

    def test_tp_grad_infos_rejects_native_dtensor_mixing(self):
        """The explicit plain-parameter map cannot be mixed with native DTensor parameters."""
        parameter = nn.Parameter(torch.ones(2))
        metadata = {parameter: TPShardMetaInfo(self._create_mock_mesh(), (Replicate(),))}

        with patch(
            "hyper_parallel.core.fully_shard.api.DTensor",
            nn.Parameter,
        ), self.assertRaisesRegex(ValueError, "native DTensor"):
            _validate_managed_params_tp_grad_infos({parameter}, metadata)

    @patch("hyper_parallel.core.fully_shard.api._get_device_from_mesh")
    @patch("hyper_parallel.core.fully_shard.api.platform")
    def test_tp_grad_infos_is_forwarded_by_fully_shard(self, mock_platform, mock_get_device):
        ...
        with patch.object(HSDPModule, "hsdp_init") as mock_hsdp_init:
            fully_shard(module, mesh=mesh, tp_grad_infos=tp_grad_infos)

        self.assertIs(mock_hsdp_init.call_args.kwargs["tp_grad_infos"], tp_grad_infos)

    @patch("hyper_parallel.core.fully_shard.api._get_device_from_mesh")
    @patch("hyper_parallel.core.fully_shard.api.platform")
    def test_tp_grad_infos_none_is_forwarded_as_none(self, mock_platform, mock_get_device):
        ...
        with patch.object(HSDPModule, "hsdp_init") as mock_hsdp_init:
            fully_shard(module, mesh=mesh, tp_grad_infos=None)

        self.assertIsNone(mock_hsdp_init.call_args.kwargs["tp_grad_infos"])
