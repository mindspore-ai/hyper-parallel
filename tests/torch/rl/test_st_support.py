...
def test_acceptance_matrix_covers_matched_tp2_consistency_and_ppo() -> None:
    """Required feature combinations must remain discoverable with independent case IDs."""
    names = [case.name for case in CASES]
    assert len(names) == len(set(names)), f"Expected unique case IDs, got={names}"
    expected = {
        (False, "full_gather"), (False, "direct_reshard"),
        (True, "full_gather"), (True, "direct_reshard"),
    }
    actual = {(case.disjoint, case.strategy) for case in CASES if case.exact}
    assert actual == expected, f"Expected consistency deployment/strategy pairs={expected}, got={actual}"
    for case in CASES:
        if case.exact:
            config = prepare_config(case, 1, (8100, 8200), list(range(case.cards)))
            vllm = config["rollout"]["vllm"]
            assert case.tp == 2 and case.algorithm == "grpo", f"Expected GRPO matched TP2, got={case}"
            assert vllm["model_implementation"] == "hyper", f"Expected Hyper rollout, got={vllm}"
            assert vllm["enable_prefix_caching"] and vllm["enable_chunked_prefill"], (
                f"Consistency must exercise cache and chunked prefill, got={vllm}"
            )
            assert vllm["logprobs_mode"] == "raw_logprobs", f"Expected raw logprobs, got={vllm}"
    ppo = {(case.tp, case.strategy, case.resume) for case in CASES if case.algorithm == "ppo"}
    expected_ppo = {(1, "full_gather", False), (2, "direct_reshard", False), (1, "full_gather", True)}
    assert ppo == expected_ppo, f"Expected PPO acceptance combinations={expected_ppo}, got={ppo}"
        ...
        events.append({
            ...
            "payload": {"ordinal": ordinal, "request": {"messages": messages},
                        "response": {"prompt_token_ids": [1], "choices": [{
                            "token_ids": [2], "logprobs": {"token_logprobs": (
                                [] if damage == "logprob-length" else
                                [float("nan")] if damage == "nan-logprob" else [-0.5]
                            )},
