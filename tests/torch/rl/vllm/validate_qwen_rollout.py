...
# ============================================================================
"""Validate Qwen rollout through the unified native or Hyper vLLM interface."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", required=True, help="Qwen checkpoint directory")
...
def _resolve_model(args: argparse.Namespace) -> VLLMModelRegistration:
    registration = build_model_registration(
        {
            "model": {
                "registry_name": "qwen_rollout_validation",
                "name": "qwen",
    ...
    return resolve_vllm_model(registration, args.implementation)


def _create_llm(args: argparse.Namespace, rollout_model: VLLMModelRegistration) -> LLM:
...
def _validate_architecture(
    ...
    if architecture != rollout_model.architecture:
        raise ValueError(
            f"{rollout_model.implementation} rollout loaded {architecture}, "
            f"expected {rollout_model.architecture}"
        )
...
def main() -> None:
    ...
    rollout_model = _resolve_model(args)
    llm = _create_llm(args, rollout_model)
    architecture = llm.llm_engine.model_config.architecture
    _validate_architecture(rollout_model, architecture)
