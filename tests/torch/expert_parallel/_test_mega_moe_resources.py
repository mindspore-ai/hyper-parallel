...
from hyper_parallel.core import symmetric_memory as public_shmem
...
from tests.torch.expert_parallel import _test_mega_moe as baseline
from tests.torch.expert_parallel._mega_moe_utils import (
...
def _mixed_owner_close_order(public_first: bool) -> dict:
    """Exercise real public allocations and a live managed backward together."""
    start_shmem_lifetime()
    shape = baseline.MoeShape()
    ...
    public_tensor = public_shmem.empty((256,), torch.int32)
    public_tensor.fill_(baseline.RANK + 1)
    public_shmem.barrier()
    ...
    try:
        if public_first:
            ...
            _assert_invalidated([public_tensor])
            assert state.manager is manager and state.clients == 1, (
                f"rank={baseline.RANK}: public close finalized the managed owner."
            )
        output.backward(upstream)
        torch.npu.synchronize()
        ...
        if not public_first:
            assert state.manager is manager and state.clients == 1, (
                f"rank={baseline.RANK}: managed close finalized the public owner."
            )
            public_tensor.add_(1)
            public_shmem.barrier()
            expected = torch.full_like(public_tensor, baseline.RANK + 2)
            baseline.assert_close("surviving public allocation", public_tensor, expected)
            TorchSymmetricMemoryHandler.close()
            _assert_invalidated([public_tensor])
        assert not state.initialized and state.clients == 0, (
            f"rank={baseline.RANK}: last owner did not finalize: clients={state.clients}."
        )
        return {"public_first": public_first, "survivor_usable": True, "last_close_invalidated": True}
    finally:
        layer.close()
        TorchSymmetricMemoryHandler.close()
...
def test_mega_moe_shared_resource_acceptance() -> None:
    """Accept serial sharing and public/managed ownership with real NPU kernels."""
