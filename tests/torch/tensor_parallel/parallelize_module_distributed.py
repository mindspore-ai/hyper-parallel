...
via ``torchrun_case`` (same pattern as ``tests/torch/shard/base_shard.py``).
"""
import torch
...
def test_parallelize_module_returns_same_instance_npu():
    ...
    Description: init dist, 1-D mesh, Linear on NPU, call parallelize_module with verify style
    Expectation: return value is the same object as input module (out is m)
    """
    init_dist()
    mesh = _make_tp_mesh_1d()
    m = nn.Linear(4, 4).npu()
    style = _VerifyMeshParallelStyle()
    out = parallelize_module(m, mesh, style)
    assert out is m
