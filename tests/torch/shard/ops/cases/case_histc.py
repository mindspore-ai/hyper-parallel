...
# ============================================================================
"""Shard ops cases for ``torch.histc``."""
import torch
...
def _histc_bins50(x):
    return torch.histc(x, bins=50, min=-3.0, max=3.0)


def _histc_bins100_min0_max5(x):
    return torch.histc(x, bins=100, min=0.0, max=5.0)


def _histc_bins75(x):
    return torch.histc(x, bins=75, min=-5.0, max=5.0)


def _histc_bins100_min0_max3(x):
    return torch.histc(x, bins=100, min=0.0, max=3.0)


register(OpShardCase(
    name="histc_ops_data_parallel",
    fn=_histc_bins50,
...
register(OpShardCase(
    name="histc_ops_hybrid_parallel",
    fn=_histc_bins75,
