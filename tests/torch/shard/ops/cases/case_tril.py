...
def _tril_keyword_diagonal_one(x):
    return torch.tril(input=x, diagonal=1)
...
def _tril_diagonal_nine(x):
    return torch.tril(x, 9)


def _tril_diagonal_negative_seven(x):
    return torch.tril(x, -7)
