...
def _type_as_plain_other(x):
    """other is a zero-dim plain Tensor on the same device as x.
    Using a zero-dim tensor also proves that other.shape does not
    affect the result.
    """
    other = torch.empty((), dtype=torch.float16, device=x.device)
    return x.type_as(other)
