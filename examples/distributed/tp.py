...
def main():
    ...
    # 2. 应用分片（production：零 DTensor dispatch；validate_mode=True 可校验）
    ...
    print(f"[rank{rank}] TP={dist.get_world_size()} output matches "
          f"single-card reference; tp_grad_info entries: {len(tp_grad_info)}")
