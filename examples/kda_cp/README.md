...
With `KDA_TEST_SERIAL=1`, the long-sequence worker also records that serial-control
discrepancy separately from its same-layout P2P comparison. Passing compatibility is not a declaration of
native-FLA equivalence or large-scale training qualification.
...
```bash
pytest -q tests/ut/components/functional/test_kimi_delta_attention_cp.py
...
KDA_TEST_LENGTH=8192 KDA_TEST_LIFETIME=1 pytest -q tests/torch/context_parallel/test_kda_ag_hybrid.py -k layer_cp4
pytest -q tests/torch/context_parallel/test_kda_ag_hybrid.py -k layer_cp8
```

Run multi-card cases in an exclusive device window. `KDA_TEST_LIFETIME=1` adds two
live invocations with reverse backward and checkpoint checks. `KDA_TEST_DP=2`
uses interleaved DP2/CP groups; the layer case requires at least eight ranks in
that configuration. `KDA_TEST_OUTPUT` optionally saves per-rank metrics as JSON.
Single-pass times include compilation and are diagnostics, not throughput claims.
The optional serial control processes P times the local length on one rank;
it is a separate diagnostic and can require substantially more memory.
